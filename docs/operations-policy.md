...
Legacy audit review 的批准迁移结果是 `data/audit-reviews/legacy-audit-reviews.jsonl` 与 `data/audit-reviews/manifest.json`。它们必须通过 `npm run audit:legacy-reviews` 校验，且仍然只是 qualitative observations，不是 review receipt 或 `ACTUAL_RUN` evidence。不得重新生成旧逐文件 audit 布局来规避 repository file-count 信号。
