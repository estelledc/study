...
- status：writer epoch `complete`；已再 merge `origin/main`（含 #88）并重生成派生索引；PR https://github.com/estelledc/study/pull/294 待 review；未把本 PR merge 进 main、未 deploy。
- 起始 ref：`2b64a3ebffee19b72d570bfe70b8c0547069ae16`（当时的 `origin/main`，#71）。
- 当前 head：以 `git rev-parse HEAD` 为准；`d799f8959`（#88）已是祖先。
