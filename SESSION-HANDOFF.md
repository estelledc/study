...
## 2026-08-27 PARALLEL writer IU：mdast-util-frontmatter + micromark-extension-frontmatter
...
- objective：为缺失的 frontmatter-plugin 双子补齐 `mdast-util-frontmatter` 与 `micromark-extension-frontmatter` 两页，绑定可达固定 revision，证据上限 `STATIC_REVIEW` / `UNVERIFIED`。不绑定 micromark 核心（#171 / #165 已覆盖）。
- scope：两篇新项目页、共享审查文档 `docs/frontmatter-plugin-source-review-20260827-iu.md`、2 份 generation 1 receipt、taxonomy 两条 curated assignment、派生 atlas / site-state / 公开计数、本交接；本机 gitignored `research-worktrees/` 的 2 个 blob-filtered clone。未改 `micromark.md`，未新增 micromark 核心页，未安装上游依赖、未运行上游测试、未测 bundle。
- activated_by：`explicit-user-parallel-writer-iu-2026-08-27`。
- detector fingerprint：目录无这两个 slug；#283 / #263 已占 `remark-frontmatter` / `vfile-matter` / `gray-matter` / `front-matter`；#171 / #165 覆盖 micromark 核心；HM–ID 预留未使用 IU。
- external delta 计数：本 PR（push + PR 已授权；merge / deploy 未授权），D 轴不提升。
- 已完成切片：
  1. `micromark-extension-frontmatter` 绑定 tag `2.0.0` → `519a2880cab7d0065f534a70c851a38dd9b5a7f2`；tag / package / npm `gitHead` 一致。
  2. `mdast-util-frontmatter` 绑定 tag `2.0.1` → `d76616b88bdafdbbcd97247c4d3a8a41cc71ae48`；tag / package / npm `gitHead` 一致。
  3. 新增共享审查文档与两份 STATIC_REVIEW receipt；派生索引与公开计数按实时命令刷新。
- 验证结果：两页 `quality-gate.mjs` pass、0 advisory；`audit:content-contract` blocking 0；`audit:counts` / `audit:site-state` / `audit:wikilinks` / `git diff --check` 通过。`STUDY_CHANGED_FROM=0ffa894c35f0d01611c7c9dd4b6d7bda3c8a4465 npm run verify:ci` 全绿，含 23 Playwright a11y 测试。
- budget：2 个 blob-filtered clone + 2 页静态源码新建；单 writer。
- blocker：merge 与 Pages deploy 未授权；规模 baseline 仍超阈值，本轮未改 baseline。
- stop conditions：本 epoch 已完成；不得继续发明下一对；不得改 micromark 核心页。
- 下一次 wake 条件：owner review 本 PR，或另行授权 merge。
- 下一条命令：在 https://github.com/estelledc/study/pull/315 做 review；未授权前不要 merge。
- superseded_by：`none`
