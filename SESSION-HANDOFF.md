...
## 2026-08-27 PARALLEL writer FC：act + age 静态迁移

- status：Program `active`；本地 writer epoch `complete`。
- 起始 ref：`1f2917d90180638e3960be37de1a5df6884b989a`（`origin/main`，#75 merge）。
- objective：绑定 `act`（nektos/act）与 `age`（FiloSottile/age）两页到不可变源码 revision，补齐 `pinned_revision` / `evidence_boundary` / `self_test`。
- scope：两页正文、2 份 generation 1 static receipt、共享审查文档 `docs/act-age-source-review-20260827-fc.md`、本 handoff、派生 note-index / project-standard / site-state；2 个 gitignored blob-filtered worktree。未安装 Docker、未跑 workflow、未编译 age、未测体积或耗时。
- activated_by：`explicit-user-parallel-writer-fc-2026-08-27`。
- detector fingerprint：两页缺固定 revision / evidence / self-test；旧 act 把默认镜像写成 `catthehacker/ubuntu:act-latest`、把 `actions/cache` 写成无声跳过，并含未绑定 star；旧 age 把许可写成 MIT、只写 X25519，并含未绑定 star / 二进制体积。
- external_outcome：分支 `cursor/act-age-source-bind-0ca5` 准备 push + 一个 PR；未 merge、未 deploy。D 轴待 PR 成立后计。
- 完成切片：
