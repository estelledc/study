...
- detector fingerprint：开放 PR 未占用 `ajv` / `typebox` slug；本地 961 个项目页中不存在这两篇；JSON Schema 编译器与 TS 工厂对照缺口可独立验收。
- external delta 计数：本轮授权开 PR，不 merge、不部署；D 轴在 merge/deploy 前不提升。
- 完成切片：
  1. 用 GitHub metadata 绑定 `ajv-validator/ajv@0fba0b8e...` / `ajv@8.20.0` 与 `sinclairzx81/typebox@51e4b028...` / `typebox@1.3.19`。
...
- 下一次 wake 条件：owner 审查本 PR，或给出新的有限目标。
- 下一条命令：`git status --short --branch`；用 GitHub 查看本 PR，不要 merge。
- superseded_by：`none`。
