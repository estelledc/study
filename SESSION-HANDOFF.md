...
- 验证结果：两页 `quality-gate.mjs` pass、0 advisory；receipt digest/revision 一致且 `UNVERIFIED`；`audit:content-contract` blocking 0；`audit:counts` / `audit:wikilinks` 通过。`verify:ci` 在 push 后继续跑。
