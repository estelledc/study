...
- status：writer epoch `complete`；已再 merge `origin/main`（含 #85）并重生成派生索引；PR https://github.com/estelledc/study/pull/294 待 review；未把本 PR merge 进 main、未 deploy。
- 起始 ref：`2b64a3ebffee19b72d570bfe70b8c0547069ae16`（当时的 `origin/main`，#71）。
- 当前 head：以 `git rev-parse HEAD` 为准；`855573fce`（#85）已是祖先。
...
- acceptance checks：
  ...
  - `git diff --check`：通过。
  - `STUDY_CHANGED_FROM=origin/main npm run verify:ci`：接入 #85 后待再跑；定向 quality-gate / counts / site-state 已绿。#83/#84 接入后 GitHub CI 已绿；`d6699b82f` 在 main 前进后失败，已被后续 merge 取代。
