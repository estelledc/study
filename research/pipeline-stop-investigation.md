...
| 现象 | 根因 | 证据 | 严重度 | 修复建议 |
|---|---|---|---|---|
| cherry-pick 成功但 main 无文件 | Layer 2 gate **行数 <150** 拒 merge（v3 **≥150 无上限**，commit `50c40286`） | `quality-gate.mjs` L4、L95–99；`sync-and-merge-single.mjs` L77–84 | **P1** | Writer prompt 强调 ≥150；merge 失败写 graveyard（需创建 `graveyard.jsonl`） |
| 边界 150 行仍失败 | lame 恰好 150 行过 gate；<150 硬拒 | `pipeline-events.jsonl` 末条 `lines:150` success | **P2** | 目标 **170±10**（SKILL L97）留 buffer；>200 不再拒 merge |
