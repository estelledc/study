...
## 两条路径说明（⭐ Season 深度 vs ✅ v3 标准）

站点笔记分两类，在 atlas 质量列可以识别：

| 标记 | 路径 | 行数 | L4 要求 |
|------|------|------|---------|
| ⭐ Season | 人工深度研读，含 8 层完整流程 | 140–600 | H2「复现」+ `repro_evidence` 字段 |
| ✅ v3 | 零基础流水线，12 段标准模板 | 150–200 | H2「实践案例」含代码块（待补全） |
| 🗄 存量 | 早期/长文迁移存量 | 140–280 | 同 v3 |

**L4 复现**仍是 Season 路径的硬底线，v3 路径使用「实践验证/实践案例」降级，优先保证结构完整性。待填充的 L4 占位符记录在 `data/l4-backfill-queue.jsonl`。
