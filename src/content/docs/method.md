...
> 论文版 [状元篇 Checklist](/study/papers-method/) 的镜像，按 7 层挂钩，但有项目专属条目。
...
### Frontmatter + 核心信息表 (P0)

- [ ] `title: 项目名 — 一句话定位`（如 "shadcn-ui — 不是组件库，是代码分发协议"）
- [ ] `description` 1 行核心 trade-off
- [ ] 核心信息表 ≥ 8 字段：star / fork / 最近活跃 / commit hash + 读时日期 / 主语言 / 维护方 + 主要贡献者（前 3-5）/ License / 类似项目

### Layer 1 · Why (P0)

- [ ] 3-5 句话"前世界缺什么"，用自己的话
- [ ] 至少 1 处 manifesto / launch HN / 一作 blog 引用

### Layer 2 · 仓库地形 (P0)

- [ ] 顶层目录注释表（每行 1 句角色）
- [ ] 心脏文件清单（2-3 个，commit hash + 行数标注）
- [ ] commit 热点 top 10-20 输出（`git log --format='' --name-only | sort | uniq -c | sort -rn`）

### 架构图 / 状态机图 (P1：项目版 hero figure)

- [ ] ≥ 1 张：架构图 / 数据流图 / 状态机图（webp，13-15× 压缩）
- [ ] caption 详细：标注图中元素 + 关键节点 + 画风

### Layer 3 · 核心机制 (P0：≥ 3 段代码精读)

- [ ] 每段 GitHub 永久链接（commit hash 锚定，如 `https://github.com/<org/repo>/blob/<sha>/path#Lxx-Lyy`）
- [ ] 每段 30-100 行真实代码片段
- [ ] 每段 ≥ 5 个旁注子弹（状态机变化 / 关键 trade-off / 为什么不用更直接的写法）
- [ ] 每段尾 ≥ 1 个"怀疑 N"段（项目版可写"为什么不这么写" / "这个细节可能漂移"）

### Layer 4 · 改一处 Hands-on (P0)

- [ ] 30 分钟跑通命令清单（含 `git clone --depth 1` / `npm install` / `npm test` / 跑 example 等）
- [ ] 1 个具体改一处实验（"我改了 X，发生了 Y"）
- [ ] 给出实验输出（截图、数字、行为变化日志）

### Layer 5 · 横向对比 (P0)

- [ ] 维度对比表（≥ 4 维）
- [ ] 至少 1 个**哲学不同**的竞品（不是同流派下位替代）
- [ ] 选型建议段（场景 → 选谁）
...
- [ ] 3-5 个具体怀疑问题（追到行号级别，如 "useQuery 在组件 unmount 后 AbortController 在哪一行 abort？"）
- [ ] "接下来读哪 N 个文件"表
