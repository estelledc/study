...
- `upstream`：19 个均指向原项目。
- 父仓 `git check-ignore`：`research-worktrees/*/` 命中 `.gitignore`。
- 父仓没有追踪任何外部源码文件。
...
```bash
GIT_LFS_SKIP_SMUDGE=1 git -C research-worktrees/open-parse \
...
git clone --depth=1 --filter=blob:none --sparse --single-branch \
  ...
  research-worktrees/MinerU

git -C research-worktrees/MinerU remote add upstream \
  https://github.com/opendatalab/MinerU.git

git -C research-worktrees/MinerU sparse-checkout set --cone \
