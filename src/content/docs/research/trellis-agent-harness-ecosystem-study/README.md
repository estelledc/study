...
- 17/17 本地工作树 clean。
- 13 个 upstream 与 pinned commit 一致。
- 4 个漂移仓已做文件级审计：Trellis、Spec Kit、GSD Core、Compound Engineering。
- 已修正“Codex 默认 inline 是因为子 Agent 无法继承上下文”的错误结论；它是 Trellis 政策，不是 Codex 限制。
- Trellis task schema/phase 的 14 个定向测试通过。
- 最小教学 Harness 的 5 个状态/工件/验证/会话隔离测试通过。

先看[全量快照复核](11-2026-07-17-refresh.md)了解哪些结论变化；零基础读者随后完成[最小 SDD/Harness 实验](12-beginner-sdd-harness-lab.md)。
...
| 11 | [2026-07-17 全量快照复核](11-2026-07-17-refresh.md) | 17 仓漂移、旧结论修正和跨类别复用边界 |
| 12 | [零基础 SDD/Harness 实验](12-beginner-sdd-harness-lab.md) | 用状态、工件、验证和 session pointer 跑通最小闭环 |
...
- 初始研究未安装第三方依赖；重新验收只在隔离环境运行 Trellis task schema/phase 两个测试文件和本仓教学实验，不是完整 CLI 或宿主 E2E。
