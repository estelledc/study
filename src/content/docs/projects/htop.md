...
2. **平台抽象层**：核心只管 UI，进程数据从 `pl/`（platform layer）读——Linux 走 `/proc`、macOS 走 `task_for_pid`、FreeBSD 走 sysctl。这套抽象是 2020 年社区接管后重写的，也是 htop 能跨多个 Unix 的原因。
...
- **2006–2018**：Hisham 个人维护近 15 年，逐步加入树视图、鼠标支持、Setup 配置界面、IO 列等。版本号缓慢从 0.x 走到 2.x
- **2019**：Hisham 宣布无暇维护，把仓库标记为 archived；社区在 GitHub 组织 `htop-dev/htop` 下接手 fork。这次 fork 没有撕裂——Hisham 公开背书新组织，用户基本无缝过渡
- **2020 v3.0**：社区版第一个大版本——重写平台抽象层，正式支持 macOS / OpenBSD / FreeBSD / DragonFly，加入 cgroup v2 / Linux pressure stall 字段
...
2. **平台抽象比 UI 更难维护**——htop 早期只支持 Linux，跨平台是 2020 后社区花最大力气做的事；底层接口的差异（/proc 文件 vs Mach API vs sysctl）决定了多平台代码量
