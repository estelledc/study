...
5. **ya 不是 yazi**：`ya` 是 CLI 配套（发消息、装插件），`yazi` 是 TUI 主程序。装的时候两个都要。
...
- 官方文档：[yazi-rs.github.io](https://yazi-rs.github.io/) （配置、插件、键位映射全在这里）
- 插件市场：[yazi-rs/plugins](https://github.com/yazi-rs/plugins) （社区贡献的预览器、主题、键位包）
- 终端图片协议综述：[Kitty graphics protocol spec](https://sw.kovidgoyal.net/kitty/graphics-protocol/) （理解 yazi 怎么把像素送进终端）
