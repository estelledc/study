...
- 为什么 2022 年突然多一个"和 Redis 对标"的项目，云厂商跟进很快——单实例吞吐 25× 是真金白银省钱
...
2. **Dashtable**（替代 Redis Dict 的哈希表）：Redis 用的 Dict 是单线程时代的设计，扩容时会卡顿。Dashtable 是分段式哈希，扩容只动一段、不阻塞其他段。内存碎片也更少，所以省 30%。
...
差距是 **20-25×**。要在 Redis 上达到同样吞吐，得跑 Redis Cluster 16 个分片，运维复杂度立刻上一个台阶。
...
如果原来用 RDB / AOF 持久化，需要规划迁移：Dragonfly 有自己的 snapshot 格式（DFS），不能直接读 Redis 的 RDB 文件——这是替换时第一个要踩的坑。
...
很多中等规模的场景，单实例 Dragonfly 可以替掉 Redis 小集群，运维成本下一个台阶。

## 踩过的坑

1. **部分 Redis 7 命令未完全实现**：`FUNCTION` 系列、部分第三方 module（RedisJSON / RediSearch 等）在 Dragonfly 上行为不一致或不支持。换之前先把项目用到的命令清单跑一遍兼容性。
...
4. **BSL 许可的 4 年期**：BSL（Business Source License）规定"4 年后自动转 Apache 2.0"，但前 4 年内某些商业用途（比如把 Dragonfly 包装成竞品 SaaS 卖）受限。自用 / 内部缓存场景没问题，但公司法务最好评估。
...
- 单实例 Redis 已经吃满 CPU，但还不想拆 Cluster——直接换 Dragonfly 立刻有 10× 余量
...
- 需要和 Redis Enterprise 商业特性（Active-Active CRDT 等）对接的场景
