...
3. **文字识别（recognition）**：把每个框抠出来送进 **SVTR_LCNet** 骨干（PP-OCRv3 起：Transformer 的 SVTR + 轻量 CNN 的 PP-LCNet），输出这一框里的字符串；比早期“CNN + 序列模型 + CTC”更偏视觉端到端。

PP-OCRv4（2023）相对 v3，既有小结构改动，也靠 **训练策略** 抬精度：

- **CML（协同互学习）**：大学生模型和小学生模型互相教，不只单向灌
- **UDML（统一深度互学习）**：多个识别头对齐输出，逼细粒度一致
- **DSR（Dynamic Shrink Ratio）**：检测训练时动态调文字区域收缩比，不是另一套蒸馏口号
- **PFHead**：检测头加并行融合分支，照顾小字 / 密字

官方口径：中文端到端 Hmean 约 **+4.5%**（英文场景提升更大，约 +10%）；移动端整套仍约 10–16 MB。
...
```bash
# 1) PPOCRLabel 标 ~5000 行竖排 → 得到 train.txt
...
- 默认横排模型读竖排古籍会崩，必须在中文预训练权重上继续训
- `train.py` 吃配置文件；路径按你下载的 PP-OCRv4 rec 配置改
- `export_model.py` 导出推理图，再接到 PaddleInference / [[paddle-lite]]

这和商用 API 的差别是：**权重在你手里**，长尾场景还能继续叠。
...
- **2023**：PP-OCRv4 叠加 PFHead / DSR / CML / UDML 等，中文端到端 Hmean 约 +4.5%
...
3. **训练策略杠杆很大**：v3 → v4 中文端到端约 +4.5%，来自 PFHead / DSR / 骨干微调 + CML/UDML 等，不只是换一块更大的网络
