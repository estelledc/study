...
## 附录 F — 与 TanStack 全家桶的协同（≥ 20 行）

TanStack 矩阵的整体设计：

| 库 | 用途 | 与 Form 的关系 |
|---|---|---|
| TanStack Query | 异步数据 + cache | Form submit 后 invalidate Query；Query data 反过来 prefill Form |
| TanStack Table | 表格 / 数据网格 | 表格内嵌 inline edit 表单 |
| TanStack Router | 类型安全路由 | 表单跨页面状态保留（searchParams） |
| TanStack Virtual | 虚拟滚动 | 大表单字段虚拟化（field array > 1000） |
| TanStack Ranger | range slider | Form 内嵌 range 字段 |
...
1. TanStack Query 提供 server state，Form 提供 client form state，明确分工
2. invalidate 触发 refetch，无需手动同步
3. 与 RHF + tRPC + zod 模式相比，TanStack 全家桶给类似端到端体验，但每层都更显式

商业逻辑：Tanner Linsley 商业咨询路径，企业付费请教 TanStack 全家整套。这是开源 + service 的 Sustainable Open Source 模式参考。

## 附录 G — 学到补充（≥ 10 行）

补充 5 条更宏观的工程教训：

6. **state machine 抽象给 framework-agnostic 提供基础**：xstate / redux 等都用 state machine；TanStack Form core 类似
7. **subscriber selector 模式从 Redux + Zustand 演化而来**：手动 selector 比 Proxy 更可调试，但 DX 略差
8. **commercial open source 路径**：Tanner Linsley + TanStack 商业咨询是案例，与 Vercel / Linode / Supabase 单点 SaaS 不同
9. **standardSchema 接口标准化**：让生态库可互换，是健康开源的方向（vs 强绑定 vendor）
10. **多框架支持的护城河**：Solid / Vue / Lit 用户量小，但每个有强忠诚度。覆盖这些 = 跨平台公司客户（一份代码服务全员）的卖点
