...
- 为什么 2024 年起几乎所有 AI 工作流编辑器（Dify / Flowise / LangGraph Studio）都用同一个画布——它就是 React Flow
- 为什么"节点编辑器"这个 UI 模式 2010 年代主要在 ComfyUI、Blender 这种桌面软件里，2023 年突然在浏览器里遍地开花——React Flow 把门槛打到 30 行代码
...
3. **大图性能**：1000 节点以上，即便 v12 重写了 measurement layer，也要手动开 `onlyRenderVisibleElements` 和 `panOnDrag={false}`，否则每帧重排卡顿。
