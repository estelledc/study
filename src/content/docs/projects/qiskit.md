...
Qiskit 是 IBM 开源的一套量子计算软件开发工具包（SDK），用 Python 写量子程序。你可以把它理解成"量子世界的 LEGO 积木"——你搭好一套电路，然后交给真正的量子计算机去跑。
...
这是最简单的量子电路之一，产生"纠缠态"——两个量子比特同时为 0 或同时为 1。
...
## 代码示例二：用 Estimator 估算期望值

除了采样测量结果，Qiskit 还可以估算量子态的数学性质。这在量子化学、优化问题中非常有用。
...
from qiskit.provider.models import GenericBackendV2
...
1. **基门分解**：把高级门（如 CNOT）拆成硬件支持的基门（如 CZ、RZ）。
2. **路由映射**：根据量子比特的物理连接方式，插入 SWAP 门来移动量子信息。
3. **电路优化**：合并相邻的相同门、消除冗余操作。

## 运行在真机上

Qiskit 通过 Provider 接口连接各种量子硬件后端，比如 IBM Quantum、IonQ、Quantinuum 等。
