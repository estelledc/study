...
ANN-Benchmarks 是 Erik Bernhardsson（Spotify Annoy 的作者）2018 年开源的一套**给"近似最近邻算法"打分的统一擂台**。日常类比：你想知道哪款跑鞋跑步快，不是听厂商广告，而是把所有牌子拉到同一个塑胶跑道，同一双袜子、同一个天气、同一段距离，逐个测。
...
- **唯一公开 benchmark**：FAISS 论文、ScaNN 论文、DiskANN 论文、所有向量数据库（[[milvus]] / [[pgvector]] / qdrant）的官方对比，引用的都是这套数据
...
- 在线结果：[ann-benchmarks.com](https://ann-benchmarks.com)（每年定期更新一次）
