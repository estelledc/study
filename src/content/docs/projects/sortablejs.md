...
SortableJS 是一个**零依赖的 JavaScript 拖拽排序库**——你给它一个装着列表项的 DOM 容器，它就让用户能用鼠标或手指把里面的项拖来拖去重新排顺序。日常类比：像把固定书架改成可抽拉的滑轨书架——书还是那些书，你不用拆架子，只是用手把某一本拖到另一格，整排顺序就换了。
...
- 为什么 Vue 生态里最常用的拖拽适配层 `vue.draggable.next`（社区维护，非 Vue 核心团队）内部其实是 SortableJS——社区选择套壳而不是从零写
- 为什么同样是拖拽，dnd-kit 用 hooks 而 SortableJS 用 `new + 事件回调`——背后是 imperative DOM 思路 vs React 受控思路
- 为什么一个 30k+ 星、周下载数百万的库，主要 commit 集中在 2013-2018，之后还能继续被广泛使用——API 稳定、需求面窄、没什么可加的
...
- Vue 项目：通过社区适配层 `vue.draggable.next` 间接使用，是 Vue 生态拖拽事实标准
...
- 列表项上千且要虚拟滚动：SortableJS 直接绑全量 DOM，拖几千行会卡，应换虚拟列表方案
...
- **2015-2018 年**：黄金期。React 时代来临，社区做了 `react-sortablejs` 桥；David Desmaisons 等基于它写出 `vuedraggable` / 后来的 `vue.draggable.next`，几乎成了 Vue 应用拖拽的事实标准。
- **2019-2024 年**：作者 commit 节奏明显放缓，主要靠社区维护；同期 dnd-kit（2021）作为 React 时代的替代崛起；但 Vue 生态和 vanilla 场景里 SortableJS 仍是首选。
- **现在**：稳定在 1.15.x，~30KB min+gzip，星 30k+，周下载数百万，是「写完就不用怎么维护」的典型工具型库。
