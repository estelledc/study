...
- **2015 年**：Manish Jain 在 Google 做知识图谱，发现没合适的开源分布式图数据库，离职创业
- **2016 年**：Dgraph Labs 成立，开源 v0.1
- **2018 年**：v1.0 发布，生产可用
- **2020 年**：版本号改用年份命名（v20.03、v20.07）
- **2024 年**：被 Hypermode 收购，仓库迁移到 `hypermodeinc/dgraph`

Neo4j 是图数据库老大哥（2007），单机起家、企业版要钱、查询语言 Cypher。Dgraph 是后来者，主打"分布式 + GraphQL + 全开源"——两条路线各占市场。

## 学到什么

1. **数据模型决定查询效率**：同样的"朋友的朋友"，关系模型 JOIN 三次，图模型走三步边
2. **GraphQL 不只是 API 协议**：Dgraph 把它当数据库查询语言用，证明 GraphQL 的图遍历能力本来就强
3. **分片策略要贴近查询模式**：按 predicate 分片是因为图查询沿同一种边走

## 延伸阅读

- 官方文档：[Dgraph Docs](https://dgraph.io/docs)
- [[postgresql]] —— 关系数据库代表，对比图数据库差异
- [[chroma]] —— 向量数据库，与图数据库一样属"特化数据库"路线
