...
下面这些场景，Elasticsearch 是常见默认选型之一：

- **大型应用日志 / 站内搜索**：GitHub、Stack Overflow、Wikipedia 等站点**曾用或仍在用**它做搜索与日志检索（具体栈会随年代变）
- **全文检索 + 评分排序**：电商搜索"iPhone 13 256G 黑"要按相关度排，关系库做起来很吃力
- **可观测性（运维监控）**：ELK Stack（Elasticsearch + Logstash + Kibana）是开源运维监控常见组合
- **8.x 后内置向量搜索**：可以存 embedding 做语义搜索，与 Pinecone / Milvus 这类专门向量库正面竞争
- **2021 SSPL 许可争议**：Elastic 把许可从 Apache 2.0 改成 SSPL，AWS fork 出 OpenSearch——影响了之后开源数据库圈的许可证选择
...
- 全文搜索 / 模糊匹配 / 相关度排序（百万～百亿文档级常见）
- 日志与可观测性检索（ELK），写入吞吐高、查询以过滤+聚合为主
- 需要近实时索引（秒级可见），可接受最终一致而非跨文档事务

**不适用**：

- 需要多行事务 / 强一致主键更新 → 用 [[postgresql]]
- 只要精确 KV 快读 → 用 [[redis]]
- 数据量很小（万级）且只要简单 LIKE → 关系库或 [[meilisearch]] 更轻

## 历史小故事（可跳过）

- **2010 年**：Shay Banon 给妻子做菜谱搜索时，对 Lucene 做了分布式封装，开源命名为 Elasticsearch
- **2012 年**：Elastic 公司成立，商业化
- **2018 年**：Elastic 上市 IPO（NYSE: ESTC）
- **2021 年**：许可证从 Apache 2.0 改成 SSPL，不再算 OSI 意义下的"开源"
- **2021-04**：AWS fork 出 OpenSearch（保留 Apache 2.0），社区分裂
...
- 许可分叉对照：[[opensearch]] —— AWS 主导的 Apache 2.0 分叉
- 轻量替代：[[meilisearch]] / [[typesense]] —— 开发者友好的搜索引擎
- 底层引擎血缘：Lucene 倒排索引与 BM25 评分（Elasticsearch 的地基）

## 关联

- [[opensearch]] —— 2021 许可变更后的 Apache 2.0 分叉，运维概念高度同构
- [[meilisearch]] —— 更轻的搜索引擎，适合中小站内搜
- [[postgresql]] —— 主存储常用关系库；和 Elasticsearch 是互补不是替代
- [[redis]] —— 同样常和 Postgres 搭配，但解决的是"快读取"不是"全文搜索"
- [[kafka]] —— 日志管道里常作 Logstash/Beats 上游缓冲
- [[typesense]] —— 另一条高性能搜索引擎路线，对照选型
