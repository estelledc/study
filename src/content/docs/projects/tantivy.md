...
倒排索引：词 → 包含它的文档列表（postings list）。Tantivy 用 **FST**（Finite State Transducer，有限状态转换器——可以想成「压缩版字典树」，按字符一个个走状态）存词典；查找是 **O(词长)** 的状态转移，整表 mmap 后热词常落在已缓存页里，所以体感很快。
...
for (score, addr) in top {
    ...
    let snippet = snippet_gen.snippet_from_doc(&doc); // 命中处带 <b> 标记
    println!("{score} {}", snippet.to_html());
}
```

三步：parse → search 拿地址 → 取回文档；要高亮再挂 `SnippetGenerator`。
...
3. **BM25 在关键词场景仍很硬**：多数纯文本搜索里，BM25 + 良好 tokenizer 往往比纯向量检索更稳、更省算力；向量是语义补充，不是一键替代
