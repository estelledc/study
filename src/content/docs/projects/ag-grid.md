...
由英国公司 AG Grid Ltd 在 2015 年开源。今天打开一个网页银行的交易明细页、对冲基金的实时行情面板、电商后台的订单列表，下面跑的很可能就是 AG Grid。
...
- **金融行业的事实标准**：彭博终端、JPMorgan 内部交易系统、Goldman Sachs 风控面板——核心都是 AG Grid。投行 / 对冲基金对"百万行实时刷新不卡"的需求，没有第二个开源方案能扛
- **百万行流畅滚动**：原生 HTML table 渲染 1 万行就开始卡，10 万行直接崩。AG Grid 在百万行下还能保持 60 FPS——靠的是只渲染可见行（虚拟滚动）
- **跨框架兼容**：React / Vue / Angular / Svelte 全部官方支持。同一份配置代码（columnDefs / rowData）在四个框架下接口几乎一样
- **社区版免费 + 企业版付费**：基础功能（排序 / 过滤 / 分页 / 编辑）社区版够用；Pivot（数据透视）/ Master-Detail（主从表）/ Excel 导出 / 服务端行模型这些重型功能走企业版授权（约 $999/seat/年）
...
- **2015 年**：Niall Crosby 在伦敦投行工作，每天用 jQuery + DataTables 做交易系统的 grid，受不了性能。周末写了个 demo，发到 Hacker News，第二天被几百家投行索要
- **2016 年**：成立 AG Grid Ltd，开源社区版 + 商业化企业版。早期客户全是金融行业
- **2020 年**：年营收过千万美元，团队还是不到 30 人。"开源 + 付费插件" 模式被广泛模仿
- 今天：GitHub 13K star，npm 周下载 100 万+，是金融前端的事实标准
