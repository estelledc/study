...
2. **控制面 = istiod**：一个进程同时干"算路由（Pilot）/ 发证书（Citadel）/ 校验配置（Galley）"三件事——1.5 版本 2020 年合并掉了原来的 4 进程架构。istiod 用 xDS 协议把配置实时推给每个 envoy。
3. **核心抽象：4 个 CRD**（自定义资源）。VirtualService 写"流量怎么路由"，DestinationRule 写"目标服务有哪些子集 / 用什么连接池策略"，Gateway 写"南北向入口怎么暴露"，ServiceEntry 写"集群外的服务怎么纳入网格"。
