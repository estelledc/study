...
- **2001 年前后**：uClibc、BusyBox 和早期嵌入式 Linux 工具链推动了“从源码拼一个小系统”的需求。
- **2005 年以后**：Buildroot 逐渐稳定成 Makefile + Kconfig 风格，和 Linux 内核配置体验保持一致。
- **2010 年代**：Yocto 成为工业级复杂发行版路线，Buildroot 则继续主打简单、直接、全量重建可理解。
- **2020 年代**：软件包数量扩展到数千个，许可证导出、安全更新和 CI 固件构建变成常见使用理由。
