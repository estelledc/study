...
Pebble 是一个**用 Go 重写的嵌入式键值引擎**，由 CockroachDB（CRDB）团队在 2018 年起步，2020 年 v20.1 上线、v20.2 成为 CRDB 的默认存储引擎。

先把 **LSM** 说成人话：像记事本——新字先写在最新一页（内存 MemTable），页满了归档成旧册（磁盘上的 SST 文件），偶尔把几本旧册合并整理（compaction）。查一个 key 就从新页往旧册翻。Pebble 和 [[rocksdb]] / [[leveldb]] 同源这套写法，但**整个引擎是 Go 原生的**，不再走 cgo（Go 调 C++ 的"翻译柜台"）。

日常类比：之前 CRDB 像餐馆主厨只说英语（C++ RocksDB），点菜每次都得翻译；Pebble 等于**请了说同一种语言的主厨**——同一套菜（LSM），厨房和前台终于对齐。仓库 BSD-3，约 5.9k stars，当前 v2.x。
...
db.Set([]byte("key1"), []byte("value1"), pebble.Sync) // Sync=每次 fsync
val, closer, _ := db.Get([]byte("key1"))
defer closer.Close() // Get 返回的 closer 必须关，否则钉住资源
...
iter.Close() // 迭代器也要关，见踩坑
```

逐行：`Open` 打开（或创建）库；`Set` 写入；`Get` 读单 key；`NewIter` 扫描。API 比 RocksDB 瘦——**只暴露 CRDB 真用到的旋钮**。
...
- 博客：[Why we built CockroachDB on top of Pebble](https://www.cockroachlabs.com/blog/pebble-rocksdb-kv-store/)（Peter Mattis，动机与取舍）
- 源码导读：仓库 `docs/range_deletions.md`、`docs/format_major_version.md`
- GitHub：[cockroachdb/pebble](https://github.com/cockroachdb/pebble)
- [[rocksdb]] —— Pebble 的算法蓝本
- [[leveldb]] —— RocksDB 和 Pebble 共同的祖先
- [[badger]] —— 同期 Go LSM，value log 分离对照
