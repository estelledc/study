...
这一行做了五件事：发请求 / 缓存结果 / 同 key 去重 / tab 切回时自动重拉 / 组件卸载时取消订阅。整个库 **4.3KB gzip**，比 TanStack Query 小一个数量级。
...
- 为什么 SWR 4KB 就够用，TanStack Query 要 13KB——同一个问题两种哲学差在哪
...
function UserList() {
  return [1, 2, 3].map(id => <UserCard key={id} id={id} />)
}
```

**逐部分解释**：

- 三个 `UserCard` 同时挂载，每个调 `useSWR('/api/user/1')`、`/2`、`/3`
- 同一帧内 SWR 发现 key 相同的请求 in-flight，**只发一次**网络请求
- 拿到数据后广播给所有订阅同 key 的组件——三张卡同时更新

这就是"dedupe"：不需要你写 `if (loading) return` 防抖，SWR 在 cache 层兜底。
