...
它由 Sean Hughes 和 Adam McAskill 于 2021 年在 Y Combinator Summer 2021（S21）起步，主打 **"BI as Code"**：报表是源代码、git 是真相源、PR review 看的是数据可视化变更，跟分析师写 [[dbt-core]] 模型同一个工作流。技术栈是 [[sveltekit]] + DuckDB WASM，目前 GitHub ~6.5k stars。
...
- **2021**：Sean Hughes 和 Adam McAskill 在 YC S21 创业，最初想法是"分析师为什么不能像写代码一样写报告"；公开版本从一开始就建在 Svelte / SvelteKit 上。
- **2023**：引入 Universal SQL，把 DuckDB WASM 嵌进前端，自此跨源 join 不再需要后端；同年宣布 Seed 与 Evidence Cloud。
- **2024**：v0.40 一带组件 API 调整（如 LineChart props 改名），商业化托管继续推进。
