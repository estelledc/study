...
1. **底层：Lance 列存格式**（Rust 写）
   - Parquet 的设计目标是"顺序扫一大段"，LanceDB 反过来——AI 训练 / 检索更多是**随机点取**（取第 31427 条向量），Lance 把点取做到比 Parquet 快 10~100 倍

2. **中层：版本化对象存储**
   - 每次写入都是**新版本**，老版本数据复用，可以 `db.checkout(version=N)` 回到任一历史快照——零拷贝
   - 表数据可直接放在 `s3://my-bucket/table.lance/`，不用本地节点

3. **上层：向量库 API**
   - 自带 IVF-PQ / HNSW / IVF-HNSW 三种向量索引
