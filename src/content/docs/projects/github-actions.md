...
- **是 ADR1 选它当主平台的核心依据**：开源 + 企业双覆盖，hosted 给个人项目、self-hosted 给生产基础设施
...
`runs-on` 写一组 label，GitHub 调度器找匹配的 self-hosted 机器跑。这就是 ADR1 说的"双层"——hosted 跑公开测试，self-hosted 跑需要内网/GPU 的真实部署。
