...
- 为什么 rope 数据结构在现代编辑器里被广泛采用——xi-editor 是第一个把 rope 推到工程实践并公开写博客讲 tradeoff 的项目
- 为什么"前后端分离"能让编辑器打字永不卡——这个观点在 VS Code 之前的 Electron 编辑器里是反直觉的
...
3. **CRDT 化的文本模型**：xi-editor 后期探索了 CRDT（Conflict-free Replicated Data Type）来管理并发编辑。每次编辑操作带版本号和位置偏移，多个编辑可以无冲突合并。这个思路后来直接影响了 Zed 的实时协作设计。
...
这种简洁的协议设计让任何人都能用任意语言写一个 xi-editor 前端——事实上社区确实出现过 Swift/Cocoa、GTK/Rust、Electron/TypeScript、Qt/C++ 等多种前端实现。
...
- [[tree-sitter]] —— xi-editor 后期探索的增量解析方案，后来被 Zed / Neovim 等广泛采用
