...
Emacs 的设计可以拆成**三根支柱**（先记人话，源码文件名可后看）：

**1. Lisp 解释器是内核，编辑器是应用。** 大多数编辑器是「C/C++ 写编辑核心，插件 API 暴露有限能力」。Emacs 反过来：C 层只提供最底层原语（内存、显示、系统接口），几乎所有用户可见行为都用 Emacs Lisp 实现。你按下一个键，C 层捕获事件后立刻交给 Lisp 求值器。用户和核心开发者用同一种语言、同一套 API——没有「插件能做」和「核心才能做」的鸿沟。

**2. Buffer 是万物容器。** Buffer 不只是「打开的文件」。它可以显示文件、shell 输出、编译日志、邮件、网页、甚至图片。所有 buffer 共享同一套文本操作 API，所以你在文件编辑里学会的操作，看日志、读邮件时一模一样适用。

**3. 自文档是强制基建。** 每个 `defun` / `defvar` 都该带文档字符串。`C-h` 是帮助入口——`C-h k` 查按键、`C-h f` 查函数、`C-h v` 查变量。Emacs 因此成为自己的参考手册，不必离开编辑器查文档。

想对照源码时，可以按三层扫一眼（跳过也不影响跟案例）：

- **C 运行时**：`src/` 里 `lisp.h` 的 `Lisp_Object`、`eval.c` 求值、`alloc.c` GC、`bytecode.c` 字节码；Emacs 29+ 还有 `comp.c`（libgccjit 本地编译）。
- **显示引擎**：`xdisp.c` 把 buffer 变成字形矩阵，`dispnew.c` 做差分重绘；平台代码在 `xterm.c` / `nsterm.m` / `w32term.c` / `term.c`。
- **Lisp 应用层**：`lisp/` 里 `simple.el`、`files.el` 等；Major Mode（如 `python-mode`、`dired-mode`、`gnus`）决定 buffer 行为。

主循环一句话：捕获输入 → 查按键绑定 → 跑 Lisp → 改 buffer → 重绘。
...
- **2016 年**：Emacs 25.1 发布，引入动态模块支持，允许用 C 写可加载模块——四十年来第一次对 Lisp 之外的扩展方式开了口子。
- **2023 年**：Emacs 29 发布，集成 Eglot（内置 LSP 客户端）和 Tree-sitter，这是对现代编辑器标配功能的正面回应（开发工作在 2022–2023 推进）。
...
(fido-vertical-mode 1)                  ; 内置模糊补全（Emacs 28+）
