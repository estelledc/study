...
- 为什么有人说「Emacs 是一个操作系统，缺一个好的文本编辑器」——它用 Lisp 运行时替代了 shell 脚本 + GUI 应用的生态
- 为什么 Org-mode、Magit 这类工具在其他编辑器里难以复制——它们深度依赖 Emacs 的 buffer/hook 模型
- 为什么「编辑器战争」（Emacs vs Vim）持续 40 年——两者代表了完全不同的哲学：可编程 Lisp 运行时 vs 模式化高效击键
- 为什么 LSP、Tree-sitter 等现代工具最终都在 Emacs 里找到了家——Emacs 的扩展接口足够底层，任何协议都能接入
