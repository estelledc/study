...
5. **多线程 + libevent**：一个 main 线程监听端口，把新连接 round-robin 派给 N 个 worker 线程，每个 worker 自己跑事件循环。2003 年初版是单线程；约 2006 年底合入多线程后，才真正吃到多核。
...
def get_user(uid: int, db):
    key = f'user:{uid}'
    raw = cache.get(key)
    if raw is not None:
        return json.loads(raw)          # hit：直接反序列化
    row = db.query('SELECT * FROM users WHERE id=%s', uid)
    cache.set(key, json.dumps(row), expire=300)  # miss：回填，5 分钟
    return row
```

这是『Cache-Aside』模式的最小写法——读先查缓存，miss 再查数据库并回填。`db` 是你自己的查询句柄；生产里还要在写库时 invalidate 对应 key，否则会读到旧数据。
...
- **2006 年**：Facebook 的 Steven Grimm 合入多线程支持（`--enable-threads`）：main 派连接，worker 各自跑 libevent 循环
