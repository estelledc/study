...
仓库：`github.com/ScoopInstaller/Scoop`，**24.2k stars**，**MIT 协议**，当前版本 **0.5.3（2025-08）**。默认安装到 `%USERPROFILE%\scoop`（也就是 `C:\Users\你\scoop`），**不要管理员权限**，**不弹 UAC**。
...
想同时有 Node 18 和 Node 20：

```powershell
scoop install nodejs-lts@18.20.0
scoop install nodejs-lts@20.10.0
scoop reset nodejs-lts@18.20.0   # 当前用 18
scoop reset nodejs-lts@20.10.0   # 切到 20
```

`reset` 改的是 shim 指向哪个抽屉。比 nvm-windows 干净。
