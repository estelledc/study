...
- 为什么 D3 火了 12 年后，作者亲自做了一个**反 D3** 的库
...
`facet.x: "cut"` → 按切工值拆成 N 张子图，每张子图共享 `y` scale 但 `x` 各自独立。

**为什么有用**：传统 dashboard 要画 5 张子图你得手写 5 遍循环 + 5 个 SVG 容器 + 5 套 axis。Plot 一行 `facet`，库自己拆。
