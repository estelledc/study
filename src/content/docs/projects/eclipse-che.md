...
- 为什么 Che 特别强调 Kubernetes：每个 workspace 都能映射成 DevWorkspace CR、Pod、Service、PVC 这些集群资源。
- 为什么本地电脑再轻也能参与大项目：编译、依赖下载、语言服务器和终端都在集群里跑，本地只负责浏览器。

## 核心要点

1. **DevWorkspace 是工位登记表**：类比前台登记一张工位申请单。Che 把 devfile 转成 Kubernetes 里的 DevWorkspace 自定义资源，DevWorkspace Operator 再创建 Pod、Service 和存储。

2. **devfile 是开工说明书**：类比实验课的器材清单。它写清楚用哪个容器镜像、要多少 CPU/内存、有哪些命令、哪些端口要暴露；仓库带着它走，新人就少读一大页手工安装文档。
