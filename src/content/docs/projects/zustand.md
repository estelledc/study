...
- 为什么 Stack Overflow 2024 调查里 Zustand 用户量**反超 Redux**——5 年时间从无名到第一
...
1. **不传 selector 直接用 `useStore()`**：会订阅整个 store，**任何字段变都触发当前组件 rerender**。永远传 selector，哪怕只是 `s => s`。
...
5. **selector 写在组件内引用不稳定**：`useStore(s => s.count)` 里 `s => s.count` 每次 render 都是新函数。Zustand 内部用 `useCallback([api, selector])` 包装——selector 引用变 = 重新订阅一次。小项目无感，10k 组件订阅时会塌陷。把 selector 提到组件外（`const selectCount = s => s.count`）能避免。
...
- **2024 年**：Stack Overflow 调查显示 Zustand 用户量首次反超 Redux。
