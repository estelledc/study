...
- 为什么 kops / RKE 这类发行版和很多自建集群长期选择 Calico——它出现得早、稳得久；OpenShift 更常见的是自家 SDN/OVN-Kubernetes，Calico 是可选集成
...
2. **Felix 是数据面编排器**：每节点一个 felix agent，监听 K8s API → 把 NetworkPolicy / HostEndpoint 翻译成 iptables 规则（默认）或 eBPF 程序（新模式）；只有开启 eBPF service handling 时，Calico 才会接管 Service 转发。这一层是"控制面 → 数据面"的翻译官，跟 [[cilium]] 的 cilium-agent 角色对应。

3. **三种数据面可选**：iptables（默认、最稳）、eBPF（新、性能接近 [[cilium]]）、Windows HNS（少见但支持）。同一控制面、不同执行后端，这是 Calico 跟 [[cilium]] 最大的工程差异——后者是 eBPF-first 的路线。
...
- 2024：Tigera 发布 Calico Open Source 3.28，继续主打"BGP + eBPF 双模"，是少数长期同时维护这两条路线的方案之一。
