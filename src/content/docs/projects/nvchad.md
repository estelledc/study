...
此外 NvChad 还自研了两个关键 UI 组件：NvChad UI（statusline + tabufline + cheatsheet）和 base46（主题引擎），这两个是 NvChad 区别于其他预配置方案的标志性特征。GitHub 星标已超过 2.5 万，是 Neovim 预配置方案里最受欢迎的之一。
...
在 Neovim 中按 `<leader>th`（默认 leader 是空格），弹出主题选择器：用 `j/k` 预览约 90 套主题，`Enter` 确认。想下次启动就用某套主题，改 starter 里的 `lua/chadrc.lua`：

```lua
-- ~/.config/nvim/lua/chadrc.lua
local M = {}
M.base46 = { theme = "onedark" }  -- 也可改成 catppuccin / gruvbox / tokyonight
return M
```

**逐部分解释**：

- `<leader>th`：打开 base46 主题选择器，改的是当前会话配色。
- `M.base46.theme`：把默认主题写进用户层配置；更新 NvChad 主仓库时不会被覆盖。
- 一份主题数据会同时驱动 statusline、tabufline、nvim-tree、telescope，不用每个插件单独改色。
...
- **2025 年**：星标突破 2.5 万，要求 Neovim 0.11+，成为 GitHub 上最受欢迎的 Neovim 预配置方案之一。社区活跃在 Discord、Matrix 和 Telegram 上。
