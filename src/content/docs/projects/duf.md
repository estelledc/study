...
- 不理解它，调试 "为什么 df 显示 100% 但还能写"（Linux ext4 默认给 root 留约 5% reserved blocks）时容易误判——duf 和 df 一样按表面 used/avail 显示，不会单独拆出 reserved
...
```bash
duf --json | jq '.[] | select(.type=="local") | {mp:.mount_point, used:.used, total:.total}'
```

`--json` 输出每个挂载的全部字段，适合 pipe 给监控脚本。字段是 **snake_case**：`mount_point` / `fs_type` / `device_type` / `total` / `used` / `free`（见源码 json tag），写成驼峰会拿到 null。
...
```bash
duf --only local --hide-mp '/System/Volumes/Data,/var/lib/docker/*'
```

逐步解释：`--only local` 先丢掉 network/special；`--hide-mp` 再按路径通配藏 firmlink / 容器残留。容器或 macOS 上默认输出太吵时，先用这两层过滤，再谈颜色主题。
...
4. **JSON 字段是 snake_case**：`--json` 输出 `mount_point` / `fs_type`，不是 `mountPoint`；照着别的工具习惯写驼峰会拿到 null
5. **看到 100% 但还能写**：Linux ext4 默认给 root 留约 5% reserved blocks；df / duf 都只显示表面 used/avail，**没有单独的 reserved 列**——别指望 duf 替你拆账，要用 `tune2fs -l` 看 Reserved block count
...
- [[dust]] / [[ncdu]] —— 目录递归占用；duf 看挂载点，它们看"哪个目录最大"
