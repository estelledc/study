...
( , int256 principal, , , , , ) = comet.userBasic(msg.sender);
require(principal < 0, "should be borrower now");
```
...
// 2) 协议吸收坏账（清算人不需垫资）
comet.absorb(msg.sender, [target]);
// 3) 折扣买走被吸收的抵押物
comet.buyCollateral(WETH, 0, 1e18, msg.sender);
```
