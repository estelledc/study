...
代码层面 v3-core 的中心是 `Pool` 合约：所有 deposit / borrow / repay / withdraw / liquidationCall 都从这一个入口走。每种资产有一组配套合约——计息凭证 `AToken`、可变利率债务 `VariableDebtToken`、稳定利率债务 `StableDebtToken`、利率策略 `DefaultReserveInterestRateStrategy`，外加全局共享的 `PriceOracle` 与 `ACLManager`。
...
function getReserveData(address asset) external view returns (
    DataTypes.ReserveDataLegacy memory  // 含 liquidityIndex / variableBorrowIndex
...
清算成功能多拿到 `liquidationBonus`（一般 5%~10%）作为奖励。`closeFactor` 默认 50%，意思是单笔最多还掉对方一半债务；只有当 HF 跌到极深（< 0.95 这种）才会一次清完。要做清算 bot 就得盯着每个用户的 HF 与 mempool 里的喂价更新，先到先得。
...
6. **closeFactor 50% 误以为能一次清完**：刚入门做清算 bot，容易写成「HF<1 就一次还完所有债」，结果 revert——除非 HF 跌穿 0.95 阈值进入 95% closeFactor，多数情况只能分两次清。
...
- [[paxos-1998]] —— 共识算法，区块链共识层与 Aave 的应用层正交但底层依赖
- [[tcp]] —— RPC 调用 Aave 节点最终走 TCP，理解链下到链上调用栈
- [[boehm-gc]] —— Solidity 没 GC 但有引用计数式存储槽，对照学习内存模型
- [[skip-list-1990]] —— Aave 没用，但其它 DeFi 协议（Curve）的 tick / 区间索引借鉴这种分层查找
