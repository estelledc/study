...
- 上线前要保证 Core Web Vitals（LCP / CLS / TBT）这些 Google 排名指标合格，没工具量化就是凭感觉
...
2. **量化指标**：从 trace 里算出几十个数字——LCP（最大内容多久画出来）、TBT（主线程被阻塞多久）、CLS（页面元素跳了多少次）等。每条按经验阈值映射成 0-100 分。

3. **加权汇总**：性能类别不是简单平均，而是按指标对体感的影响**加权**——TBT 占 30%、LCP 占 25% 等。这就是为什么改一处不一定提分多。
