...
- 为什么国产 MCU 厂商（兆易、华大、国民技术、灵动）的板级支持包（BSP）**往往更贴合 RT-Thread**——本土社区跟厂商距离近，不是说全球生态已超过 FreeRTOS
- 为什么 2006 年起步后，在中文 IoT/MCU 社区常与 FreeRTOS（2003）/ Zephyr（2016）**并列讨论**——450+ 软件包 + 近 200 块开发板的厚度（全球装机 FreeRTOS 仍更广）
- 为什么从 1.2 KB 的 Nano 到带 lwIP / 文件系统的 Standard 是**连续可调**的——三层架构按需裁剪
- 为什么很多中文 IoT 教程默认 RT-Thread——中文文档、FinSH 命令行、AT 组件接 4G 模组都开箱即用
...
跟做四步：① 初始化串口上的 AT 客户端；② 发 `AT+CGATT?` 查网络附着；③ 让组件解析应答与 URC（**模组主动上报的消息**，如来电/掉线）；④ 再挂 lwIP + MQTT 上云。
...
AT 组件还管超时重发与状态机。把 IoT 协议栈在 KB 级 RAM 上跑通的关键拼图。
...
- 需要确定性短延迟响应（常在毫秒内，视主频/中断配置可达更短）+ 多任务并发
...
- 跨大陆社区协作、英文生态优先 → FreeRTOS / Zephyr 通常更厚
- 安全认证刚性场景（车规 ASIL-D 等）→ 用经过严格认证的商业 RTOS；硬实时还要关多余中断并测最坏响应

## 历史小故事（可跳过）

- **2006 年**：熊谱翔在中国发起 RT-Thread，定位极简内核（后来叫 Nano）
- **2014 年**：扩展为带组件 + 软件包生态的 Standard 版
- **2016 年**：Zephyr 由 Linux 基金会发起；此后中文社区常把 FreeRTOS / Zephyr / RT-Thread 并列讨论
