...
```bash
# 克隆 Poky（git:// 不通时改用 https://git.yoctoproject.org/poky）
git clone -b scarthgap git://git.yoctoproject.org/poky
cd poky
source oe-init-build-env          # 创建 build/，默认 MACHINE=qemux86-64
bitbake core-image-minimal        # 首次约 1-2 小时
runqemu qemux86-64                # 在虚拟机里启动刚编出的镜像
```

`core-image-minimal` 是预定义最小镜像，只含 busybox + 基础启动脚本，产出约 10 MB。

### 案例 2：用 .bbappend 给已有包打补丁

不 fork 上游 `meta` 层，只在自己的层里「追加」一步：

1. 建目录 `meta-mycompany/recipes-core/busybox/files/`，放入 `fix-my-bug.patch`
2. 同级写 `busybox_%.bbappend`（`%` 匹配任意版本）：
...
`FILESEXTRAPATHS` 告诉 BitBake「补丁在本配方旁边的 files/」；`SRC_URI +=` 把补丁挂进 busybox 的下载列表。构建时自动合并，你的层与上游解耦。

### 案例 3：自建发行版并换板子

```bash
# 1) meta-mycompany/conf/distro/mydistro.conf
DISTRO = "mydistro"
...
# 3) 树莓派还需克隆 meta-raspberrypi，并在 bblayers.conf 的 BBLAYERS 里加上该层路径
```

`distro.conf` 定特性集（systemd / wayland / SELinux）；`MACHINE` 定板子。换板子通常要加对应 BSP 层，业务配方可不动。
...
- **2003**：OpenEmbedded 起步——嵌入式开发者共享一套构建配方，不再每个项目从零搭系统
- **2010**：Linux 基金会发起 Yocto Project 伞项目，打包 OE-Core、BitBake 与参考发行版 Poky，提供标准化发布
- **命名**：Yocto 取自最小 SI 前缀 yocto-（10^-24），寓意给最小设备做 Linux；Poky 常被解释为 Pocket 变体
- **版本节奏**：约每半年一个代号版本；LTS 是指定发布（如 Kirkstone 4.0、Scarthgap 5.0），不是「偶数号就 LTS」
- **到 2026**：5.x 线仍在演进，生产环境常见做法是钉住一个 LTS 代号长期维护
