...
- MeiliSearch 像 Spotify 那种应用——下载、运行、搜，5 分钟就有可用结果
...
4. **Open Source + SaaS 双轨**：自托管免费（MIT 协议早期，现在 Server Side Public License），托管版叫 MeiliCloud。和 [[supabase]] / Mongo 走的是同一类商业路径。
...
### 3. Ranking rules（5 层排序）

搜索结果排序按这 5 层依次比较，前一层比不出胜负才看下一层：
...
这 5 层是默认顺序，可调。理解这套规则，调搜索质量就有抓手。
...
- **2022 年**：v1.0 发布，API 稳定，宣布生产可用。商业化 MeiliCloud 上线。
