...
Piper 是 Rhasspy 团队（Michael Hansen 主导）开源的**本地神经网络 TTS（文本转语音）**：把火热的 VITS 端到端 TTS 模型导出为 ONNX，再用 C++ 二进制驱动推理，**专为树莓派这种 5 美元小板子优化**。日常类比：像把一台**录音棚级的合成器塞进鞋盒**——不联网、不烧电、还能在树莓派上比说话还快地把文字读出来。
