...
- 为什么同一种代币（如 USDC）能在 10 条链上"同一个总供应量"流转——这是 OFT 标准做的
- 为什么 Stargate / Radiant 这类应用敢说"无中间链跨链"——它们底层就是 LayerZero
- 为什么 Wormhole 和 LayerZero 都做跨链但安全模型完全不一样——前者一条链验证、后者多 DVN 共识
- 为什么 2024 年后跨链桥事故率明显下降——可配置 DVN 让应用方能自己加保险
...
3. **DVN + Executor（验证 + 执行解耦）**：DVN 是链下守望者群组，每个独立地见证源链事件并签名——多个 DVN 都签了才算消息成立。Executor 是另一个角色，负责在目标链上花 gas 调 `lzReceive`。**这两件事谁都不能身兼**，避免单一作恶。
...
- [[uniswap-v3]] —— Uniswap 的跨链部署用 OFT 标准做流动性同步
- [[aave-v3]] —— GHO 稳定币用 LayerZero 跨链，是早期 OFT 大客户
- [[rollup]] —— 跨链消息的两端绝大多数是 Rollup
