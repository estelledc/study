...
1. **链下节点网络**：一组独立运营的 Chainlink Node（Go 语言写），订阅链上请求 + 拉外部 API + 跑共识。节点抵押 LINK，作恶可被罚没。

2. **OCR 共识（Off-Chain Reporting）**：节点先在链下 P2P（libp2p）网络互相对账，每轮选出 leader 收集所有节点的报告，排序去极值，把签名聚合成一份"已签名报告"——最后只发**一笔**交易上链。31 个节点从 31 笔交易压成 1 笔，成本砍 90%+。
...
VRF 节点用 BLS 签名生成随机数，链上合约能用公钥验签——**不可能事后调整**。区别于 `block.hash` 抽奖（出块矿工能操纵 / 重放），VRF 在密码学层面把作弊堵死。
...
2. **VRF v1 已不推荐**——v1 每次请求直接付费，v2 用订阅子账户预充 LINK。新项目直接上 v2，老教程用 v1 的别照抄。
