...
这套栈被 Coinbase 拿去做了 Base 链，也被 Worldchain / Mode 等采用；早期还有项目（如 Mantle）曾基于它再演进。2024 年它仍是最常见的 OP Stack 底盘之一。
...
- **2023 年 6 月**：Bedrock 主网升级，EVM 等价性落地，重写整套 client（op-geth + op-node 拆分）——这是它今天能做"开放栈"的工程基础。
