...
跑 `kubectl apply -k overlays/prod`，Kustomize 会读 base 的 `deployment.yaml`，把 `replicas` 字段改成 10，namespace 设成 prod，所有资源加上 `env=prod` label，再交给 kubectl apply。**base 文件一个字没动**。
...
1. **patch 语法两套混淆**：`patchesStrategicMerge` 按 K8s 字段语义合并（list 按 key 合并）；`patchesJson6902` 是 RFC 6902 JSON Patch（按路径增删改）。新人常用错——改 list 想覆盖某一项，strategic 是按 key 合并不是替换，结果 list 里多一项。
