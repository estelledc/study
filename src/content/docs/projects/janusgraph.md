...
- 为什么 Gremlin 是 TinkerPop 系图库的通行遍历语言（JanusGraph、部分云图库），而 Neo4j 主推的是另一条线 Cypher
...
| 后端 | 适合场景 |
|------|---------|
| [[cassandra]] / ScyllaDB（`cql`） | 写多读多、需要跨数据中心 |
| HBase | 已有 Hadoop 生态、单行强一致 |
| BerkeleyDB（`berkeleyje`） | 单机开发测试 |
| inmemory | 无持久化的快速试验 |
