...
1. **复用 syntect 生态**：syntax 规则用 Sublime Text 的 `.sublime-syntax`、主题用 `.tmTheme`（TextMate 格式），覆盖 200+ 语言。类比：delta 和 [[bat]] 像两家店共用同一套配料仓库，自己专心做好 diff 渲染。

2. **字级 diff（word-level diff）**：拿到一对 `-`/`+` 行后，delta 会继续比较行内文字，只把"真的改了的几个字符"染色，行内其余不变的字保留普通色。这是 default git diff 很难一眼看清的地方。
...
```bash
git -c delta.side-by-side=true diff HEAD~3
```

逐部分解释：

- `-c delta.side-by-side=true` 是临时打开 delta 的并排模式，不会把 `--side-by-side` 误传给 git 自己。
- `diff HEAD~3` 仍然是普通 git diff，delta 只是负责分页和渲染。
- 删除/新增并排两栏显示，每栏带独立行号、独立 syntax 高亮；**注意**：终端宽度 < 120 列时换行严重，别开。
...
- `git grep -n` 和 `rg -n` 都输出 `文件:行号:内容` 这种文本格式，delta 更容易识别。
- 不要把 `rg --json` 直接管给 delta；JSON 是给脚本读的，不是给人类分页器读的。
- 日常做"全仓找某关键字 + 看上下文"时，文件路径、行号、命中片段会比纯文本更好读。
...
- **2024-2026**：进入稳定维护期，重点在主题、导航、grep 输出和平台兼容性上打磨；具体 release 数和 star 数会随时间变化，应以 GitHub 页面为准
...
2. **字级 diff 是真功能差异**——default git diff 更偏行级展示；delta 往行内继续找差异，不只是配色调整
