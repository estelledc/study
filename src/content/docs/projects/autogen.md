...
1. **较早把"多 agent 对话"做成框架的项目**：之前各家多是单 agent + tool calling，AutoGen 把"agent 之间互相说话"抽象成一等公民。
...
新项目更建议看 v0.4 的 `agentchat` 入口，最小代码大致长这样：
...
async def main():
    ...
    result = await assistant.run(task="写一个 Python 求斐波那契数列")
