...
我（Jason）的工作主线是 video-eval-agent / quanzhiping，里面有大量"调用脚本"。目前我写脚本基本靠手撸 `process.argv.slice(2)` + 手动 split，遇到要加 `--dry-run` `--limit 10` `--config.path xxx.json` 就一团乱麻。先把 yargs 这层吃透，后面：
...
这条规则放到 video-eval-agent 也成立：6 件套 schema 验证不应该塞在 evaluator 内部，而是 evaluator 之外的独立 layer。
...
- **S31 收官**：选一个真实场景（video-eval-agent CLI），用 yargs 重构现有脚本
