...
AutoGluon 是 AWS 2020 年开源的 AutoML 工具箱。一行 `fit()` 把表格、时序、多模态三类任务自动训出强模型，起步时可以先不手动调参。
...
三行训练 + 预测，背后会按 preset 组合多种树模型、KNN、神经网络等基模型，再配合 k-fold bagging、stacking 和 weighted ensemble。

## 为什么重要

AutoML 圈一直主流是「搜超参」（HPO，hyperparameter optimization），代表是 Auto-Sklearn / FLAML / H2O。AutoGluon 的默认路线更偏 ensemble：HPO 可以开，但常用 preset 先把多个不同归纳偏置的模型攒到一起。
...
1. **Layer 1（base models）**：按安装依赖和 preset 跑 LightGBM、XGBoost、CatBoost、随机森林、KNN、FastAI 神经网络、Extra Trees 等候选模型。每个模型做 k-fold 交叉验证，留下 OOF（out-of-fold）预测。
...
为什么默认不把搜超参放在第一优先级？作者论点：在很多表格任务上，超参搜索的边际收益小于「把多个不同归纳偏置的模型混合」的收益。LightGBM 擅长的样本和 KNN 擅长的样本不同，混起来往往比只把单个模型调到极致更稳。
