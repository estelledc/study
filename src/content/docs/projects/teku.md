...
3. **管理验证者**：拿着私钥（或委托给 Web3Signer 远程签）按时给出投票，错过一次就少拿一份基础奖励（base reward 的零头）；连续不投会触发 inactivity 渗漏。类比：开会忘举手，那一轮工资就没了。
...
私钥永远不离开 Web3Signer 容器，beacon node 升级也不影响 validator 在线。
...
Teku 出块时会先问 builder 要"打包好的最优区块"，赚 MEV 收益。注意：`fee-recipient` 必须是 **20 字节有效以太坊地址**（如 `0x742d35Cc...`），写 `0xabc...` 这种占位会让 validator 启动失败。
...
1. **客户端多样性不是口号**——任何一种实现占比超 2/3 都会让网络在 bug 时分叉，所以"用 Teku"本身就是一种价值贡献
