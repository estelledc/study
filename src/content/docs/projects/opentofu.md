...
3. **OpenTofu 已反超 Terraform 的几个特性**：
   ...
   - **provider for_each**（v1.7）：可以对 provider 块迭代（同一个 AWS provider 跑多 region）
...
### 案例 3：provider for_each（OpenTofu 独占）

```hcl
# 同一个 AWS provider 同时跑 us-east-1 和 eu-west-1
provider "aws" {
...
Terraform 里 provider 块必须静态写死，要多 region 就要复制粘贴。OpenTofu 把这个限制解了。
...
- **2024-05**：v1.7 上 state 加密 + provider for_each，正式开始『反超』
- **2024 之后**：持续迭代，已成为 Terraform 的真正替代品
