...
先记住零知识在说什么：我能让你相信"这笔转账合法"，但你学不到谁、给谁、多少钱。日常类比：盲测可乐——猜对 10 次你信我能分辨，但仍不知道我靠什么分辨。zk-SNARK 把这套盲测压成一次非交互小证明。
...
转账四种组合：t→t（=比特币）、t→z（进屏蔽池）、z→z（**完全隐私**）、z→t（出屏蔽池）。**只有 z→z 是真正零知识**，其他三种至少泄漏一端——这是最常被误解的地方。

### 案例 2：一次 z→z 转账在节点视角看到了什么

```
区块新增一笔交易，节点看到：
  - 几个"消费的 nullifier"（一次性哈希，无法反查回 commitment）
  - 几个"新增的 commitment"（秘密金额的哈希）
  - 一份 zk-SNARK 证明（Sapling 用 Groth16，证明本身约 192 字节）
  - 矿工费（整笔交易里唯一必须明文的字段，否则矿工没法收费）
```

节点验证 zk-SNARK 通过 → 接受交易。**金额是多少？发给谁？节点不知道。** 也没法知道。除矿工费外，发送者、接收者、金额都不在明文里。
...
Zcash 用**多方仪式（MPC ceremony）**生成：多国参与者各出一份随机数、各自销毁、最后聚合。**只要至少一人真的销毁了，整个系统安全**。仪式过程是公开的——飞机舱、汽车后备厢、燃烧硬盘的视频都有。

Orchard 升级换到 **Halo 2**，**不再需要 trusted setup**——这个历史包袱终于卸掉了。

## 踩过的坑

1. **shielded 池太小 = 隐私集太小**：上线后多年里大部分资金仍停在 transparent 地址，真正做 z→z 的人偏少，**对手集一小时内只有几百笔**时，时间戳和金额相关性就能戳穿。"隐私功能存在不等于你有隐私"。
