...
来源: https://github.com/ratatui/ratatui
日期: 2026-05-31
分类: cli
难度: 入门
---
...
它的前身叫 **tui-rs**（Florian Dehau 2016 年开始写），2023 年原作者停维护，社区在 ratatui-org（后迁到 ratatui）重启 fork，名字从"tui"换成 ratatui（一道法国炖菜，logo 也是只老鼠厨师）。截至 2026-05，约 20.8k stars，被约 15k 个 Rust 仓库依赖——[[gitui]] / [[bottom]] / [[bandwhich]] / [[dua-cli]] / atuin / iamb 全是它画的。
...
- 为什么 [[ink]]（Node.js / React 风格 TUI）和 ratatui 性能拉得开——ink 走 React reconciler diff 整棵组件树，ratatui 直接 diff 字符 buffer
...
1. **立即模式（immediate mode）渲染**：你不是"创建一个 Button 对象、改它的 .text"，而是每帧从 app state 重新构建整个 UI。代码长这样：
   ...
   每次 `draw` 都重新算一遍。控件没生命周期，状态全在你这边。（`f.area()` 是 ≥0.28 的写法；旧代码里的 `f.size()` 已弃用。）
...
3. **Layout + Constraint 约束求解**：把一块矩形按 `Length(N)` / `Percentage(N)` / `Min(N)` / `Max(N)` / `Ratio(a, b)` / `Fill(N)` 的约束切分成几块。底层是 Cassowary 风格求解器（现用 kasuari fork；macOS Auto Layout 同源算法）。写法像这样：
...
1. **切屏**：`Layout` 把终端切成 2×2（Status / Diff / Log / Branches）
2. **自定义 Widget**：每块实现 `render(area, buf)`，往矩形里画字符
3. **后台算**：git fetch / blame 扔给 tokio，结果经 channel 推回 UI 线程
4. **心跳 redraw**：UI 线程约 30ms 一拍重 `draw`——标准"后台算 + 立即模式画"
...
`recent_bytes_per_sec` 是滑动窗口（最近 60 秒）。每秒抓包 → 求和 → push → redraw。控件无状态，数据全在你的 `Vec<u64>`。
...
三块积木：raw mode 开关 / `draw` 闭包 / `event::poll` 心跳。gitui / bottom 骨架同此，只是 widget 多、state 大。
