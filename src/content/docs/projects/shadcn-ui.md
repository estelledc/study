...
CLI 去 registry 拉一段 JSON、过 Zod schema 校验，再把 `button.tsx` **直接写进**你项目的 `components/ui/`。没有 node_modules 里的 shadcn 运行时，没有 npm 升级冲突——因为根本**没有 npm 包**。
...
2. **CLI 是 runtime**：`npx shadcn add` 内部跑 9 步——fetch JSON、Zod 校验、解析 `registryDependencies`、Kahn 拓扑排序（保证 utils 先于 button 落地）、deepmerge `tailwind` / `cssVars` 字段、ts-morph AST 改 `tailwind.config`、写 `components/ui/<name>.tsx`、装 npm 依赖、跑 format。每一步都绑定一个心脏文件，可单独排错。
...
4. **`shadcn add` 默认会覆盖你本地修改**——需要 `--overwrite` 显式确认；魔改过的 button.tsx 无声无息被冲掉，是新人最常见的 PR 灾难。
...
- 想搭跨仓库的 team registry，用 SSO Bearer token 鉴权
