...
- 为什么 [[lmdb-2011]] 和 bbolt 长得这么像（都 mmap、都 COW、都 B+ 树）——bbolt 就是 [[lmdb-2011]] 的 Go 版精神后继
...
- **2013 年**：Ben Johnson 看了 [[lmdb-2011]]（C 写的、mmap + COW + B+ 树）想要一个 Go 等价物，开始写 BoltDB
...
1. **mmap + COW + B+ 树** 三件套是经典"读优先嵌入式 KV"的最优解，[[lmdb-2011]] / bbolt 殊途同归
...
- [[lmdb-2011]] —— bbolt 的精神祖宗，C 实现，思路几乎一致
...
- [[lmdb-2011]] —— bbolt 的设计原型，几乎一样的 mmap + COW + B+ 树
