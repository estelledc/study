...
db.Update(func(tx *bolt.Tx) error {
    bucket, _ := tx.CreateBucketIfNotExists([]byte("kv"))
    return bucket.Put([]byte("/service/api/leader"), []byte("node-3"))
})
```

逐步看：`Update` 开一个写事务；`CreateBucketIfNotExists` 相当于建一个命名空间；`Put` 把 raft 已确认的状态写进单文件数据库；函数返回 nil 时事务才提交。
...
逐步看：`View` 是只读事务，可以和其他读事务并发；`Cursor` 利用 B+ 树的有序性；`Seek(prefix)` 直接跳到前缀开头，再顺序 `Next()`，这就是 bbolt 范围扫描快的原因。

### 案例 3：作为 Go 服务的本地配置/缓存

```go
func SaveToken(db *bolt.DB, token string) error {
...
很多 Go 工具（CLI、桌面应用、轻量服务端）只需要"重启不丢的本地存储"。这段代码的关键不是复杂 API，而是**进程内调用 + 单文件落盘**：不用起服务，不用连端口，备份时复制那一个 `.db` 文件就行。
