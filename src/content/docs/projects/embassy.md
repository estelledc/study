...
1. **必须用 nightly Rust**：Embassy 依赖 `type_alias_impl_trait` 等不稳定特性来实现任务的静态分配。这意味着你不能用 stable 工具链，而 nightly 偶尔会因为上游改动导致编译失败。解决方法是在项目里用 `rust-toolchain.toml` 锁定一个已知可用的 nightly 版本。
