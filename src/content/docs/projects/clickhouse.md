...
- 为什么字节、阿里、Uber 都在用它跑日志分析、监控、用户行为分析
...
1. **列式存储**：同一列的数据连续存放在磁盘上。查"今天 amount 之和"时，只读 amount 这一列的字节，跳过其他所有列。类比：图书馆按"作者"分柜，找某作者的全部书不用翻整馆。
...
- [[starrocks]] —— StarRocks — Doris 分叉出来的向量化 CBO 国产 OLAP
