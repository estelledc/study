...
- **2016 年**：Kubernetes 社区推进 CRI，把 kubelet 和具体 runtime 解耦；Red Hat 开始做 CRI-O，目标是只实现 Kubernetes 需要的那一层。
- **2017 年**：Kubernetes 1.6 引入 CRI beta，CRI-O 能作为 Docker 之外的运行时选项出现。
- **2019 年**：OpenShift 4 选择 CRI-O 作为默认 runtime，项目从“可选实验”进入企业发行版主路径。
- **2022 年**：Kubernetes 移除 dockershim，CRI-O 和 containerd 成为最常见的两条 CRI 路线。
- **2024 年**：CRI-O 从 CNCF incubating 毕业，定位稳定为 Kubernetes 专用 runtime。
...
- [[podman]] —— 同用 containers/* 库，但面向本地开发和命令行体验
- [[buildkit]] —— CRI-O 不覆盖镜像构建，构建侧通常交给这类工具
