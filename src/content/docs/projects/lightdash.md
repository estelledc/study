...
Lightdash 的 join 声明寄生在 model 的 meta 里。前端拖维度时，customers 表的列也会出现，Lightdash 自动 join。这条 join 关系可以和 dbt 的 `relationships` 测试对应——一处描述、双重作用。

### 案例 3：CI 里检查 metric 没坏

```bash
dbt parse                           # 重生成 manifest.json
lightdash compile --project-dir .   # Lightdash 解析 manifest，验证所有 metric 能编译
```

PR 改了 model 列名忘改 metric 的 sql 字段，`lightdash compile` 会报错。这条流程把 BI 层的"运行时才暴露"问题前移到 CI。
...
- **2021 年**：Oliver Laslett（前 Airbnb analytics engineer）和联创看到 dbt 已经把 transformation 标准化，但 BI 这一头还在被 Looker 锁定 LookML。他们提出"既然 dbt 已经描述了数据，BI 就不该再造一套 DSL"，开源了 Lightdash。
- **2022 年**：Y Combinator + 种子轮融资，社区从几百 star 涨到几千。同期 Cube、Evidence、Rill 也在抢"现代 BI / 语义层"位置。
...
- [[snowflake-2016]] —— Lightdash 最常见的 warehouse 后端
