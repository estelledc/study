...
- 为什么大厂的 ML 训练集群、CI runner、Spark 作业 90% 跑在 Spot 上
- 为什么"中断率 5-10%/天"是行业里讨论 Spot 风险的基准刻度
...
3. **中断率分桶**：Advisor 把每个池标成 `<5%` / `5-10%` / `10-15%` / `15-20%` / `>20%`（按天）。**5-10% 这一档**意味着平均每 10-20 天被收一次——对带 checkpoint 的训练任务可接受，对在线服务要谨慎。
...
本项目的中断率分档（`<5%` / `5-10%` / `10-15%` / `>20%`）和"跨 AZ + 跨 instance family 双重分散"两条原则的**唯一权威源**就是 Spot Instance Advisor。其他三方文章（博客、Medium、Stack Overflow）都是在引用或转述 AWS 的数据，要看一手数字必须打开 advisor 页面。

实操上，团队定的"中断率 5-10%/天可接受"红线，对应 advisor 黄色档位——平均每 10-20 天中断一次，配合 checkpoint + 自动重启对训练任务影响极小。
