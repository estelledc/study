...
[README 顶部宣传语](https://github.com/makeplane/plane/blob/0acb32e6/README.md)：
...
![Plane 整体架构 — Web/Space → Proxy → API (Django) + Live (Hocuspocus) → Postgres + Redis + S3](/projects/plane/01-architecture.webp)

*图 1：Plane v1.3.1 / commit `0acb32e6` 的整体架构。左侧三个 client（apps/web 主站、apps/space 公开页、apps/admin 实例管理）走
[`apps/proxy`](https://github.com/makeplane/plane/tree/0acb32e6/apps/proxy) 反向代理。中间两条独立运行时：
[`apps/api`](https://github.com/makeplane/plane/tree/0acb32e6/apps/api) 是 Django + DRF（Python 24.9%，事务/CRUD/权限/migration 全在这里），
[`apps/live`](https://github.com/makeplane/plane/tree/0acb32e6/apps/live) 是 Express + Hocuspocus + Yjs（Node，富文本协同 + presence）。
...
| 文件 | 行数 | 角色 |
|---|---|---|
| [`apps/api/plane/db/models/issue.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/issue.py) | ~600 | **Issue 模型 + IssueManager + IssueAssignee/Label/Subscriber/...**——后端事务真相 |
| [`apps/api/plane/db/models/cycle.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/cycle.py) | ~200 | Cycle（sprint）模型——和 Issue 一对多 |
| [`apps/api/plane/db/models/module.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/module.py) | ~200 | Module（epic / roadmap 容器）——和 Issue 多对多 |
| [`apps/web/core/store/issue/project/issue.store.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/project/issue.store.ts) | ~180 | **ProjectIssues store**——前端 issue 列表的 mobx 入口 |
| [`apps/web/core/store/issue/helpers/base-issues.store.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/helpers/base-issues.store.ts) | 1500+ | **BaseIssuesStore**——分页 / group / filter / sort 全部基类逻辑 |
| [`apps/live/src/hocuspocus.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/hocuspocus.ts) | 67 | **HocusPocusServerManager**——realtime 入口 singleton |
| [`apps/live/src/extensions/index.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/extensions/index.ts) | 18 | extension 装配顺序（Logger → Database → Redis → TitleSync → ForceClose） |
| [`packages/types/src/issues/issue.d.ts`](https://github.com/makeplane/plane/tree/0acb32e6/packages/types) | — | TIssue 类型公约（前端 / live 共享） |
...
[`apps/api/plane/db/models/issue.py:96-185`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/issue.py#L96-L185)
...
[`apps/api/plane/db/models/issue.py:200-235`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/issue.py#L200-L235)
...
[`apps/web/core/store/issue/project/issue.store.ts:51-100`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/project/issue.store.ts#L51-L100)
...
[`apps/web/core/store/issue/helpers/base-issues.store.ts:1-60`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/helpers/base-issues.store.ts#L1-L60)
...
[`apps/live/src/hocuspocus.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/hocuspocus.ts#L1-L67)
...
[`apps/live/src/extensions/index.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/extensions/index.ts#L1-L18)
...
- **MobX 6 + makeObservable 的显式 action 声明模式**——任何 React 项目里替代 Redux/Zustand 时直接抄
  [`apps/web/core/store/issue/project/issue.store.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/project/issue.store.ts) 的 `makeObservable(this, { fetchX: action })` 写法。比 `makeAutoObservable` 更可控
- **Postgres advisory lock 做 per-tenant 序号生成**——任何"我要给某个范围内生成自增 ID 但不锁全表"的场景，
  抄 [`issue.py:200-235`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/issue.py#L200-L235) 的 `pg_advisory_xact_lock(convert_uuid_to_integer(scope_id))` 模式
- **Tiptap + Yjs + Hocuspocus 三件套做协同编辑器**——别自己写 OT/CRDT。
  抄 [`apps/live/src/hocuspocus.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/hocuspocus.ts) 的 67 行 singleton 模式 + extension 装配顺序
...
- **Django 的 ChangeTrackerMixin 做 audit log**——任何"客户要看 X 字段什么时候被谁改成了什么"的场景，抄 [`issue.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/issue.py#L98) 顶部 `TRACKED_FIELDS = ["state_id"]` 的字段白名单模式
...
| 顺序 | 文件 / 资源 | 回答什么 |
|---|---|---|
| 1 | [`apps/live/src/extensions/database.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/extensions/database.ts) | Yjs → Postgres 持久化的具体实现 |
| 2 | [`apps/live/src/extensions/title-sync.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/live/src/extensions/title-sync.ts) | "协同字段"反向同步到"事务字段"的桥 |
| 3 | [`apps/web/core/store/issue/helpers/base-issues.store.ts`](https://github.com/makeplane/plane/blob/0acb32e6/apps/web/core/store/issue/helpers/base-issues.store.ts) 完整通读 | group/sort/pagination 的真正复杂度 |
| 4 | [`apps/api/plane/db/models/cycle.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/cycle.py) + [`module.py`](https://github.com/makeplane/plane/blob/0acb32e6/apps/api/plane/db/models/module.py) | sprint vs roadmap 的 schema 区分 |
| 5 | [`apps/api/plane/bgtasks/`](https://github.com/makeplane/plane/tree/0acb32e6/apps/api/plane/bgtasks) | Celery 异步任务（webhook / notification / import） |
| 6 | [`deployments/kubernetes/community/`](https://github.com/makeplane/plane/tree/0acb32e6/deployments/kubernetes/community) | 生产部署形态（statefulset / pvc / pdb） |
| 7 | [`packages/editor/`](https://github.com/makeplane/plane/tree/0acb32e6/packages/editor) | Tiptap + Yjs binding 的封装 |
| 8 | [`turbo.json`](https://github.com/makeplane/plane/blob/0acb32e6/turbo.json) | 构建图（哪些 task 互相依赖） |
