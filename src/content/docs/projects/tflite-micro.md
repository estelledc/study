...
这段代码的意思是：模型已经被编译进程序，`arena` 是提前准备好的工作台，解释器只在这块固定空间里搬东西，然后执行一次推理。
...
- 电池供电、网络不稳定、需要低延迟响应的小设备。
- 关键词唤醒、简单手势、异常检测、低分辨率图像分类这类小模型任务。
- 已经能接受模型压缩、`int8` 量化、有限算子集合的项目。
- 需要把推理结果留在设备本地，降低上传原始数据需求的场景。
...
- 设备其实是 Raspberry Pi 这类嵌入式 Linux；这时普通 LiteRT / TensorFlow Lite 往往更省事。

## 历史小故事（可跳过）

- **2010s 后期**：手机端推理已经常见，但微控制器还受限于内存、功耗和碎片化硬件生态。
- **2019 年左右**：TensorFlow Lite for Microcontrollers 开始被 TinyML 社区广泛讨论，关键词是"几 KB 内存也能跑模型"。
- **2020 年**：TFLM 论文系统解释了为什么要无操作系统、无动态内存、解释器式执行，并展示低资源开销。
- **之后几年**：社区移植到 Arduino、ESP32、SparkFun Edge、Zephyr、Renesas、Silicon Labs 等平台，README 里也维护了这些入口。
- **现在**：仓库大约 2k+ stars，定位不是替代云端大模型，而是让最小的设备拥有一点本地判断力。
