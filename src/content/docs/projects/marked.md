...
最常见的扩展：覆盖 `renderer.link`（现行 API 接收 **token 对象**）：
...
**逐部分解释**：`tokens` 是链接内部的 inline token；要用 `this.parser.parseInline` 拼出可见文字，不能再假设旧版的 `(href, title, text)` 三参数。`marked.use()` 可多次调用合并配置，**同一个 renderer 键后注册的覆盖先注册的**。
...
3. **tokenizer 的 first-match vs renderer 的后注册覆盖**：Lexer 里规则**先匹配的赢**；但两个 plugin 都覆盖 `renderer.heading` 时，**后 `use()` 的赢**，前面那个直接消失。社区因此很少做「多 plugin 串联同一键」的组合。
...
3. **first-match-wins vs 后注册覆盖**：扩展模型要分清两层——tokenizer 顺序决定谁先吃输入，renderer 同名键则是后写覆盖。设计自己的 plugin 系统时别混用这两套语义。
