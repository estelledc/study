...
它最早由 Wercker 团队开源，后来转交给社区在 `stern/stern` 维护，Go 写成单个二进制，GitHub 11k+ 星。Kubernetes 运维和后端开发日常工具链里，它和 kubectl、k9s、kubectx 并列前四。
...
```sh
stern . -n api --include 'trace_id=abc-123' -o raw
```

`.` 匹配所有 pod；`--include` 行内正则；`-o raw` 不带 pod 前缀，方便管道：

```sh
stern . -n api --include 'trace_id=abc-123' -o raw | jq '.message'
```
...
- `cmd/stern/stern.go`：Cobra root command，把 CLI flag 解析成 config
- `stern/config.go`：运行时配置结构 + 默认值
- `stern/stern.go`：主 watch loop —— 调 client-go 的 PodInterface.Watch，事件驱动开关 tail
- `stern/tail.go`：单 pod 单 container 的 log stream goroutine —— 处理染色、行格式、断流重连

读这份代码能直接学到三件事：怎么用 client-go 的 watch、怎么用 goroutine 池管理动态资源、怎么把 ANSI 颜色封装成可关闭的 writer。
...
- 临时排错 / 滚动发布观察 / 多副本同时观察
- 学习 client-go watch + goroutine 模式的范例代码（11k 星 Go 项目里少有这么小巧的）
...
- 仓库：[stern/stern](https://github.com/stern/stern)（Go，11k+ 星）
