...
5. **扩展可以预装**：自定义镜像里可以安装 Open VSX（开源扩展目录，类似应用商店，但不是微软官方市场）上的扩展，也可以下载 `.vsix` 安装包后装入。
...
### 案例 3：下载二进制并用 token 保护入口

适合单人自托管或小团队内网（大约 1–10 人），不想先搭 Docker 时：

```bash
# 1) 从 GitHub Releases 下载对应平台的 tarball 并解压
# 2) 进入解压目录后再启动：
...
- 二进制来自项目 Releases，不是系统自带命令；解压后才有 `./bin/openvscode-server`。
- `--host 0.0.0.0` 允许非本机访问；默认只听 localhost 时，别的机器连不上。
- token 只是第一道门：浏览器 IDE 里有终端和源码访问权，仍应放在可信网络或 HTTPS 反向代理后面。
...
- 单人到小团队（大约 1–10 人）想快速自托管浏览器版 VS Code。
- Gitpod 类平台需要 IDE 组件，而不是整套工作区调度。
- 教学、培训、临时实验环境。
- 用 Docker 镜像固定开发工具链。

**不适用**：

- 需要完整多人租户平台、账号体系和细粒度权限（几十人以上协作平台）。
