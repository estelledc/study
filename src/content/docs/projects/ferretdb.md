...
1. **wire protocol 解析**：MongoDB 客户端发的是二进制 BSON 包，FerretDB 用 Go 实现一份 wire protocol 解码器，把 `insert` / `find` / `aggregate` 这些命令还原成数据结构。
...
```bash
# 一个 docker-compose 起 PostgreSQL + FerretDB
docker run -d --name pg -e POSTGRES_PASSWORD=pw -p 5432:5432 postgres:16
docker run -d --name ferret -p 27017:27017 \
  -e FERRETDB_POSTGRESQL_URL=postgres://postgres:pw@host.docker.internal:5432 \
  ghcr.io/ferretdb/ferretdb
mongosh mongodb://localhost:27017/test
```

进 `mongosh` 后写 MongoDB 命令——背后真正落到 PostgreSQL 里：
...
```bash
psql -h localhost -U postgres -d ferretdb
\dt+
```

会看到 FerretDB 自动创建的表，比如 `test._ferretdb_database_metadata`、`test.users_xxxxx`，每条 MongoDB 文档对应一行，BSON 内容存在 JSONB 或 documentdb 扩展提供的列里。**你可以直接用 SQL 查**：

```sql
SELECT _jsonb FROM test.users_xxxxx LIMIT 1;
```
...
3. **Sharding 不是原生**：MongoDB 的 sharding 拓扑（mongos + config server）FerretDB 不复刻——要水平扩展走 PostgreSQL 自己的方案（Citus / 读写分离 + 分库）。

4. **性能不是替代 MongoDB 的卖点**：相同硬件下原生 MongoDB 通常更快——FerretDB 多了一层翻译。选它是为了**协议兼容 + 真开源**，不是为性能。

5. **版本迭代快**：v1.x 用 JSONB 存储；v2 切到 documentdb 扩展，行为和性能都变。锁定版本别只看 latest。
