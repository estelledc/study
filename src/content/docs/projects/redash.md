...
- 为什么 2020 年 Databricks 花钱收 Redash——它需要一个能直接挂在 Lakehouse 前面给非工程师用的查询入口
...
你点『运行』 → Web 把 SQL 写进 Postgres 的 `queries` 表 → 发一个 Celery task → Worker 从 `query_runner` 找对应数据源适配器 → 连真实数据库执行 → 结果存进 Postgres 的 `query_results` 表 + Redis 缓存 → Web 轮询拿到结果 → 前端 React 渲染表格。

下次另一个用户跑**同样的 SQL**（精确文本一致），命中 Redis 缓存直接返回，不再打数据库。这就是为什么改一个空格、加个注释，就会触发新一次执行——key 是 SQL 的哈希。
...
加一个新数据源 = 在 `redash/query_runner/` 下加一个 Python 文件，继承 `BaseQueryRunner`，实现 `run_query()` 和 `get_schema()` 两个方法。Redash 自动发现并注册，重启后在 UI 数据源列表里就能选。

这是 Redash 能积累到 35+ 数据源的关键——社区贡献者只要会写一个连接器就能合并进去，不用改核心代码。
...
2. **元数据库 vs 结果数据库分离** = Postgres 装『query 是什么、谁建的、调度计划』，结果走 Redis 缓存，避免互相影响
3. **轮询 + 缓存** 是简单有效的『近实时』方案——不用引入 WebSocket / 流式系统，前端轮询拿 Redis 缓存就够小团队用
4. **Dashboard 是 query 的容器**，不是独立实体——这种『一等公民 + 容器』的建模思路在很多内部工具里通用
5. **OSS + 商业版双轨**也能稳定共存——前提是商业版深耕生态集成（Lakehouse 上下游），OSS 守住基础查询体验
