...
它由 [[dgraph-io]] 公司在 2017 年开源，作者 Manish R Jain。动机很直接：[[dgraph-io]] 是 Go 写的图数据库，需要嵌入式 KV 但不想引入 cgo（C++ 那一套交叉编译麻烦、调试痛苦）。Badger 是**唯一**纯 Go 的生产级 KV 引擎。
...
身份验证服务存 session token、TOTP 密钥。写入不算高，但要求**单二进制部署**——不带 C 依赖、不依赖外部 Redis。Badger 嵌进去就完事。
...
3. **不能两个进程同时打开**：Badger 用文件锁防止多开。子进程 fork 后还想读老 db 会直接 panic。生产里见过启动脚本误开两份的事故
