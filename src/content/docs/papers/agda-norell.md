...
- 为什么 Idris、Lean、Rust 的某些 trait、TypeScript 的某些泛型，越走越复杂——它们都在追依赖类型的影子。
- 为什么"数组下标越界"在 Agda 里**不可能编译过**——长度信息写在类型里。
- 为什么 Lean 4 能拿来证四色定理、形式化数学——它走的是 Agda 趟出来的路。
...
- **1990 前后**：Catarina Coquand（Thierry 的妻子）在 Chalmers 实现 Agda 1。
