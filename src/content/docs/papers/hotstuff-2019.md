...
- 为什么 Facebook 的 Libra（后来叫 Diem）、Aptos 等公链把共识层建在 HotStuff 变体上，Sui 等则在同源思路上继续改（如 Mysticeti）
- 为什么 BFT 共识在 2019 年之后突然能跑到几百个节点——以前 PBFT 卡在十几个就难以扩展
- 为什么"3-chain + 线性 view-change"成了新模板——Tendermint/Casper 是精神前身，Diem-BFT 等才是 HotStuff 变体
...
2. **3-chain rule（三连锁规则）**：一个块 X 要被"承诺"（commit，不可撤回），必须形成 **X→X'→X'' 共三块** 的连续 QC 链（含自身，不是后面再跟三块）。前两轮（prepare、pre-commit）是"锁定"，第三轮（commit）才真正承诺。**少一轮都可能丢安全性**——这是 HotStuff 论文最精妙的证明。
...
Aptos（Facebook 团队后续）的共识代码 `consensus/safety-rules` 里直接照搬 HotStuff 的状态机，每个区块带三个字段：`parent_qc`（父块 QC）/ `grand_parent_qc`（祖父）/ `great_grand_parent_qc`（曾祖父）。当这三层 QC 对齐成一条链时，**曾祖父那块**才被承诺——这就是 3-chain 的工程写法。Sui 的 Mysticeti 是同源思路的 DAG 化优化，承诺规则仍能对上。
...
- **2014**：Tendermint 提出 lock-on-PoLC 的两阶段思路，是 HotStuff 的精神前身（不是变体）
- **2018-03**：Yin 等人在 arXiv 发出 HotStuff 初稿，Facebook 团队同时在内部用它造 Libra
- **2019-06**：Facebook 公开 Libra，共识层是 LibraBFT = chained HotStuff
- **2019-07/08**：HotStuff 正式发表于 PODC 2019（Toronto）
- **2022-2024**：Aptos / Espresso / Monad 等基于 HotStuff 变体；Sui 等在同源路线上继续演进
