...
- 为什么后来所有 T2I 模型（Stable Diffusion 3 / Flux / DeepFloyd IF）都用 T5 而不是 CLIP 做文本编码器
- 为什么"扩散网络做大"比不上"文本编码器做大"——这反直觉，但 Imagen 用消融实验证明了
- 为什么 classifier-free guidance 能拉到 cfg=7~10 不"烧屏"——动态阈值这个工程技巧从 Imagen 出
...
Imagen 没赢市场（闭源），但**赢了思想**——后来所有人都改用 T5 类的语言模型做文本编码器。
...
后来事实证明 Imagen 的路线赢了：所有后续主流 T2I（Stable Diffusion 3、Flux）都用 T5 而不是 CLIP 做文本编码器。
