...
- 为什么 ChatDev / OpenHands / GPTeam 长得都有点像——它们站在 MetaGPT 抽象上做变体
...
- **启发整个赛道**——ChatDev / OpenHands / OpenAI Swarm 都借鉴了 Role + Environment 抽象
...
- **Alice (PM)** 写 `docs/prd.md`：用户故事、UI 草图、需求池
- **Bob (Architect)** 写 `docs/system_design.md`：模块划分、类图
- **Eve (ProjectManager)** 拆任务到 `docs/tasks.md`
- **Alex (Engineer)** 在 `2048_game/` 下写 Python 代码
- **Edward (QA)** 写 `tests/` 测试代码
...
- **2024-01**：ICLR 接收为 Outstanding Paper（提名前 5 之一）
- **2024 全年**：ChatDev / GPTeam / OpenHands 等 30+ 项目复用 MetaGPT 的 Role + Environment 抽象
- **2024-12**：MetaGPT X（视觉版）发布，加入屏幕识别 + 鼠标操作能力
- **2025**：和 OpenAI Swarm 形成两条路线——MetaGPT 走"重 SOP"，Swarm 走"轻 handoff"
