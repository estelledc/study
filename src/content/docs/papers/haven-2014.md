...
- 为什么云厂商能把"机密计算 / 少信任云"当卖点——Haven 证明了"未修改应用 + 硬件隔离"可行，是后续 enclave / confidential container 路线的重要学术先驱（注意：Confidential VM 走的是整机 TEE，与 LibOS-in-enclave 不是同一条实现）
...
论文的性能测试表明，SQL Server 在 Haven 里相对虚拟机（VM）基线的吞吐量惩罚约为 31-54%（相对 Drawbridge 约再慢 13%，再加上加密文件系统与 I/O 瓶颈），主要来自 enclave 进出的上下文切换（每次 EENTER/EEXIT 成本很高）以及原型文件系统/EPC 相关开销。对于中等负载的 OLTP 查询，作者认为这类开销在"换不信任云"的前提下可接受。
...
1. **性能开销不容忽视**：整个 LibOS + 应用都在 enclave 里运行，每次和外界交互都要经过 shield module 的验证逻辑，再加上原型文件系统与 SGX 相关开销，SQL Server 相对 VM 基线慢约 31-54%。这个开销让 Haven 更适合安全要求极高的场景，而非通用部署。优化方向后续由 SCONE（异步 syscall 批处理）和 Occlum（用户态多进程 LibOS）分别探索。
...
Haven 论文获得了 OSDI 2014 的 Jay Lepreau 最佳论文奖。它直接启发了后续的 Graphene-SGX（2017）、SGX-LKL（2019）、Gramine（Graphene 改名）、Occlum（2020，蚂蚁集团开源）等系统，把"未修改应用进 enclave"做成可落地路线。需要分清：今天云厂商的 Confidential VM（如基于 AMD SEV / Intel TDX 的整机加密）保护的是整台虚拟机，并不等于 Haven 那条 LibOS-in-SGX-enclave 路线；更接近 Haven 思想的工业化落地，是 Gramine / Occlum 这类 LibOS，以及云上的 enclave / confidential container 产品线。Haven 仍是机密计算从"保护小段代码"走向"保护完整应用"的关键学术先驱。
