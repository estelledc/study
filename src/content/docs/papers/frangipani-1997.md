...
2. **中层：分布式锁服务**。每段磁盘区间（一组 inode、一段 bitmap）有一把锁。机器要改之前先去锁服务申请，改完释放。锁服务自己用 Paxos 复制保证不丢锁。
...
这套接口和 6 年后 Google Chubby（2006）几乎一样——后者明确说『我们就是把 Frangipani 这层独立出来做成服务』。
...
把分布式 FS 拆成『底层共享虚拟磁盘 + 中层分布式锁 + 上层无状态服务器』三层叠盘，每台机器对等、靠锁同步、靠 log 互救——美得像理论模型，但真上 Web 规模就会被锁服务拖下来。

## 再深一层（看到这里说明你想再多挖一点）

Frangipani 这种『**对称、共享磁盘、强一致**』范式并没有死，它在两个方向上活得很好：

- **企业 SAN / 虚拟化**：VMFS 让一群 ESXi 主机共享同一块 LUN，本质就是『Petal + Frangipani』的工业版
- **数据库共享存储引擎**：Oracle RAC、Aurora、PolarDB 把『日志即数据库』倒过来用，但底下『多节点共享一份存储 + 协调器仲裁』的骨架还是这条路

互联网那一端（GFS / HDFS / Colossus）则把对称性拆掉换来了横向扩展。所以 1997 年这个分叉到今天还在影响每条系统的选型。
