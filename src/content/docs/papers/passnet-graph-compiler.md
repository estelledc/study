...
- 为什么主流模型上图编译器很强，但到长尾模型和奇怪算子组合时会突然变慢（论文 profiling：约 43% 真实子图在默认编译下端到端变慢）。
...
2. **数据来自真实模型图，不是玩具题**。类比：练车不能只在空停车场转圈，还要上真实街区。PassNet 从 100K 真实模型里抽取并去重出 18,086 个计算图，再拆成三类子图：fusible（可融合的算子链）、classical（经典优化模式）、single-operator（单算子改写）。

3. **评测同时惩罚错误和变慢**。类比：外卖员不能只看速度，还要看有没有送错餐、有没有洒汤。论文用 ES_t（Error-aware Speedup Score，把正确性、稳定性、速度合成一个分数）和 AS（Aggregate Score，多任务汇总分）放在同一把尺子里。

## 实践案例

### 案例 1：一个 compiler pass 长什么样

下面是教学示意（不是 PassNet 仓库里的真实 API），用来看清 matcher + rewriter 的分工：
...
- 需要一个能同时检查正确性、稳定性、速度的 pass 生成 benchmark（PassBench 约 200 个长尾 fusible 任务）。
