...
### 案例 3：openssl + Node.js 一段实战

```bash
# 命令行加密（AES-256-GCM）
echo "hello" | openssl enc -aes-256-gcm -k mypass -a
```
...
let ct = c.update('hello', 'utf8', 'hex')
ct += c.final('hex')
const tag = c.getAuthTag()             // 别忘了存 tag
```

注意：**iv 一定每次随机或递增**，重用 = 灾难（见踩坑 2）。
