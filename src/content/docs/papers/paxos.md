...
- 为什么 Google Chubby / Spanner 用 Paxos，而 ZooKeeper 用 Zab、etcd 用 Raft——同属多数派共识家族却不是同一个协议
- 为什么 [[spanner]] 能跨大洲做强一致事务
- 为什么 [[raft]] 自称"易理解版 Paxos"，业界等了 30 年才有这个易懂版
- 为什么 1985 年 FLP 之后，Paxos 与更早的 Viewstamped Replication（1988）同属**最早正确且实用**的异步共识协议
- 为什么 Cassandra 的轻量事务（LWT）也要跑一轮 Paxos，而普通读写却走最终一致

只要系统里有"多副本 + 不能脑裂"的需求，背后大概率就是 Paxos 或其徒孙（Raft / Zab）。
...
Chubby / Spanner 是这种 Multi-Paxos with leader；ZooKeeper 用 Zab、etcd 用 Raft，形态相近但协议不同。**论文里"重复运行 Paxos 实例即可"一句话带过，工业系统 100% 在这上面打补丁**。
