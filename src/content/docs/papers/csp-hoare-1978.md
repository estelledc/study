...
- 为什么 Go 的口号是 "Don't communicate by sharing memory; share memory by communicating"——这是 CSP 1978 论文里那句话的直译
- 为什么 goroutine + channel + `select` 看起来又简单又强大——它就是 CSP 三件套
- 为什么 Erlang / occam / Ada 的 task / Rust async channel 都长得像兄弟——共同祖宗是这篇论文
- 为什么 Unix `cat | grep | wc` 这种管道能优雅串起来——pipe 就是退化版 CSP
...
`ch <- 5` 就是 CSP 的 `B!5`，`<-ch` 就是 `?`。`go func()` 起新进程。Go 把 1978 论文的语法换了符号，**语义几乎一比一**。
...
读：哪个 channel 先有数据就走哪条 case，等价于论文里 `[ A?x → ... [] B?y → ... ]`。

### 案例 3：素数筛（CSP 经典示范）

把每个素数当成一个进程，整数从 2 开始一路灌进流水线，每个进程把"能被自己整除的"过滤掉再传给下一个：

```
2 → [filter(2)] → 3 → [filter(3)] → 5 → [filter(5)] → 7 → ...
```

每个 filter 进程：从左 channel 读一个数 n，如果 n % 我 ≠ 0 就发给右 channel，否则丢掉。**用 N 个进程并发筛 N 个素数**，CSP 1978 论文用这个例子说明"流水线即并发"。
...
- 进程边界清晰、通信路径少（pipe 模型 / actor 模型 / 流式处理）
- 需要可推理的并发——CSP 有完整代数（process algebra）能形式化验证
- Go / Erlang / Rust async 这种以 channel 为一等公民的语言

**不适用**：

- 需要大量共享只读数据（每次都 copy 进 channel 太慢）→ 用 [[hoare-logic]] + 共享内存 + 锁
- 需要异步消息（发出去就不等）→ 用 Erlang 风格 actor，CSP 是同步的
- 极致低延迟场景（rendezvous 强制等同步，开销大）→ 用 lock-free queue 或 SPSC ring buffer
- 进程拓扑动态变化频繁 → 1978 原版不支持，得用现代变种（Go / π-calculus）
