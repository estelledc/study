...
这套"在抽象域上算"的方法，是 Astrée（空客飞控验证）/ Infer / Frama-C / Rust borrow checker / TypeScript 类型推导背后的共同理论根。
...
- 为什么数据流分析、类型检查、模型检查表面差很多，背后却用同一套算法

## 核心要点

抽象解释的精髓可以拆成 **三步**。先约定两个符号：⊥（读作 bottom，意思是"一无所知"或"不可能"），⊤（读作 top，意思是"啥都可能"）。希腊字母 α / γ（读 alpha / gamma）只是给两个函数起的代号。

1. **造抽象域**：选一个"简化版"的值集合，叫格（lattice）。比如把所有整数压缩成 `{−, 0, +, ⊤, ⊥}` 五个抽象值。类比：地图把真实地形抽象成等高线。

2. **建 Galois connection**：装一对函数把现实压扁、再展开。抽象函数 α 把具体值投到抽象格（如 α(3) = `+`），具体化函数 γ 把抽象值还原成它代表的具体集合（如 γ(`+`) = 所有正整数）。这对函数保证"抽象计算的结果一定包含真实结果"——即 sound（不漏报）。

3. **算不动点**：把程序看作格上的单调函数 F，从 ⊥ 出发反复算 F(⊥)、F(F(⊥))…直到结果不再变。Tarski 定理保证完备格上单调函数必有最小不动点。格高度无限（如区间）时引入 widening 算子 ∇ 强制跳到更宽的值，几步内必收敛。

三步加起来：**抽象域 + Galois connection + 不动点迭代** = 一切静态分析的骨架。
...
分析器推：`i` 进入循环时是 `[0, 0]`，每次 +1 后是 `[1, 1]`、`[2, 2]`、…格高度无限，必须 widening。Cousot 给出的标准 widening：观察到 `i` 在变大但上界未知，直接跳到 `[0, +∞]`，再用循环条件 `i < n` 收紧到 `[0, n−1]`。结论：只要 `n ≤ array_length`，就不越界。
