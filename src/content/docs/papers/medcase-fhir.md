...
答案很刺眼：作者把 MedCaseReasoning 的临床病例转成 FHIR R4 bundle，82.5% 的可处理病例生成了有效 FHIR；但同一批模型在结构化 FHIR 输入上的诊断准确率，普遍低于纯文本输入。
...
2. **生成流程是“LLM 分步写 + 规则校验返工”**。类比：实习生先填病历，质控系统查药品码、检查码和字段关系，错了退回重填。作者固定三次 LLM 调用：抽取临床信息、合成 FHIR、检查诊断泄漏。
...
4. **82.5% 有效生成说明流程可用但还没稳到工程闭眼上线**：失败主要来自术语覆盖、幻觉码、过细描述和输入病例不适配。
