...
EDM 不是新模型，**它是一份扩散训练的"标尺"**。后来的 [[stable-diffusion]] 系改进、Consistency Models、Flow Matching 等大量借鉴它的 preconditioning 与采样思路；SD3 / SD-XL Turbo 等工程系统还会叠蒸馏、对抗或 flow 目标，不能说"整份配方原样照搬"。
...
- 为什么很多新采样器讨论 Heun / 二阶 ODE，而不只提 DDIM——Heun 是 EDM 推荐的二阶求解器（ODE = 常微分方程，把"去噪轨迹"写成连续方程再离散求解）
- 为什么训练时噪声 sigma 用 lognormal 分布而不是均匀分布——EDM 消融出来的最优分布
- 为什么 ImageNet-64 的 FID（生成图分布与真图分布的距离，越低越好）能从旧模型的 2.07 降到 1.36——主要靠配方，不是换大架构
...
代价：每步多一次网络前向。收益：步数可以从 100+ 降到 35。**总 NFE（Neural Function Evaluations，神经网络前向次数）反而少**——这就是 EDM 推荐 Heun 的原因。上面是教学伪代码，真正跑通还要自备 `denoise`/`F` 网络与完整 sigma 调度（见官方 `generate.py`）。
...
### 案例 4：论文数字怎么读（别合成一张假消融表）

EDM 的贡献要分两层看，数字来自论文摘要 / 主结果，而不是把采样与训练改动硬拼成一张逐步表：

1. **只换采样**：把已有 ImageNet-64 模型的 FID 从约 **2.07 → 1.55**（近 SOTA），说明 Heun + 时间表本身就很强。
2. **按 EDM 配方重训**：CIFAR-10 条件生成 FID **1.79**，ImageNet-64 到 **1.36**——这才是完整 preconditioning + 训练噪声分布 + 损失加权一起上的结果。

方法论要点仍是：**采样、训练噪声、preconditioning、损失加权可正交替换**；读论文时跟 Table 1/2 的 config，不要把不同实验轴的数字串成一条假因果链。
...
4. **消融表是论文的灵魂**。EDM 把采样改进与训练改进分开报告（例如 ImageNet-64：只换采样 ≈2.07→1.55，完整重训到 1.36），让读者看清每条轴值多少分。这是研究论文该有的样子。
