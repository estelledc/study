...
1. **几何概率**：如果包围盒 A 完全包在 B 里，那么"随便射一根光线穿过 B 时也穿过 A"的概率，正比于它们的表面积之比 `SA(A) / SA(B)`。这是 19 世纪几何概率学家 Mally 的老定理，但 1987 年第一次被搬到光线追踪里。
...
假设一个 1m³ 大盒子里装两个 0.1m³ 小盒子，分别藏在两个对角。
...
反过来，如果两个小盒子被强行放进一个 0.2m³ 的中盒，表面积 ≈ 1.92 m²，命中率 30%——光线频繁误入。SAH 会给后者打很差的分。
