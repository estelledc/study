...
- 为什么现代 [[cuda]] 的 `__shfl` warp shuffle、以及后来的 CUDA Graphs / MPS 多进程共享，都能在 Kepler 的 Hyper-Q 与 Dynamic Parallelism 上找到硬件前提

## 核心要点

Kepler 在 Fermi 之上的关键升级可以拆成 **四件事**：

1. **SMX 胖核**：Fermi SM = 32 CUDA cores + **2 调度器**；Kepler SMX = **192 cores + 4 调度器 + 8 dispatch units**。意义：单 SM 算力 6 倍，但**不靠提频**——靠并行更多 warp、靠工艺降耗（28nm）。这是"宽而慢"取代"窄而快"的开端。
...
3. **Dynamic Parallelism**：Fermi kernel 想再启 kernel 必须**回 CPU 派活**（毫秒级延迟）；Kepler kernel 可以**自己直接 launch 子 kernel**。意义：自适应网格细化、递归算法、不规则图算法——之前必须 CPU 控流的代码现在 GPU 自闭环。后来 CUDA Graphs 把"少回 CPU"做成可捕获的图，硬件上的早期一步就是这里。
...
- 没 **Dynamic Parallelism**，递归/自适应代码仍卡在 CPU-GPU 来回——后来 CUDA Graphs 走的仍是"少回 CPU"这条路
...
# AlexNet 此处为示意占位，实战用 torchvision.models.alexnet
import torch
...
逐部分解释：`cuda:0` 只看到 K80 的一半；模型搬到 GPU 后按当时 FP32 训。2014-2017 年标准配置——AlexNet 原版用两块 GTX 580（Fermi），K80 出来后约 **2-3 倍速**，**单侧 12GB** 装得下当时主流模型。
...
__inline__ __device__ float warpSum(float v) {
    // Kepler 原语是 __shfl_xor；现代代码用 __shfl_xor_sync（Volta+）
...
逐部分解释：`o` 从 16 减半到 1，做 butterfly 归约；`__shfl_xor` 让线程直接读同 warp 另一 lane 的寄存器。Fermi 要走 shared memory + `__syncthreads()`（**20+ 周期**）；Kepler 约 **5 条 shfl、~5 周期**。`softmax / layernorm` 受益。
...
3. **"少回 CPU" 从这开始**：Dynamic Parallelism 让 GPU 自闭环启子 kernel；后来的 CUDA Graphs / graph capture 仍是同一方向的软件化，硬件根子可追溯到 Kepler
