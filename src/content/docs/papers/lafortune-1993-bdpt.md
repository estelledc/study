...
- 双向：你出门，**同时**朋友也出门，两人各走几步，然后掏手机直接对接位置——一定接得上
...
3. **逐对连接**：对每个 $(s, t)$ 组合，把 $y_s$ 和 $z_t$ 之间连一根**阴影线**（shadow ray），如果**互相可见**，这条拼接路径就是一个完整的 light → eye 路径，可以贡献到像素。每条连接都要乘上两端的 BRDF 值 + 几何项 + 可见性。
