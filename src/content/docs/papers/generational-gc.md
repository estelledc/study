...
2. **跨代引用追踪**：minor GC 只扫 young，但 old 区可能有指针指向 young 对象，漏扫就会把活的当成死的。解法是 **write barrier**（写屏障）——每次写指针时，编译器自动塞两三条机器指令进去，把"老年代某个位置写过新生代指针"记到 **card table**（卡片表：堆按约 512 字节切成一格，每格对应表里 1 字节脏标记）。下次 minor GC 把脏卡当根集补扫。
...
4. **大对象绕过分代直进 old**——并非"超过约 8 KB 就进 old"。Serial/Parallel 可用 `-XX:PretenureSizeThreshold`（默认 0=关闭）指定；G1 则把超过 region 一半的对象当 humongous 直接进老区。频繁产生大数组/大字符串等于绕开分代，full GC 会飙高。
