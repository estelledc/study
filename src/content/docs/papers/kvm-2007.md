...
- 为什么 AWS 的 Firecracker（Lambda 底座）、Cloud Hypervisor / crosvm 能把「KVM + 极薄用户态」砍到百毫秒级启动
- 为什么"hypervisor"和"操作系统"在 Linux 世界已经融成一回事——`top` 里看到的 qemu 进程就是一台云主机
- 为什么 Intel VT-x / AMD-V 这些 2005 年的硬件扩展，是 KVM 能在 6 页论文里讲完的前提（WSL2 在 Windows 上走 Hyper-V；gVisor 默认是用户态内核，KVM 只是可选 platform——别和 KVM 用户态混为一谈）
