...
### 案例 3：微服务下单 saga（线性编排）

按步骤往前走；失败就**倒序**补，每步补偿只跑一次：

```python
def place_order(order):
    done = []  # 已成功提交的步骤
    try:
        reserve_inventory(order); done.append("T1")   # T1
        charge_payment(order);    done.append("T2")   # T2
        create_shipment(order);   done.append("T3")   # T3
    except Exception:
        # 倒序补偿：先 C2 再 C1（T3 未成功则没有 C3）
        if "T2" in done: refund_payment(order)        # C2
        if "T1" in done: release_inventory(order)     # C1
        order.status = "failed"
        raise
```

工业界两种实现风格：**编排型（orchestration）**——一个中心 service 当指挥；**协同型（choreography）**——各 service 听事件自己推进。前者好调试，后者更去中心。
...
**适用**：
- 长流程业务（订单/审批/批量结算），每一步都能找到补偿动作；常见单 saga **≤10 步**、单步 P99 **<1s**
- 跨多个独立系统/数据库，没法做 2PC（旅行预订、跨行转账、微服务）
- 吞吐和可用性优先，能接受短暂脏读；步骤松耦合，可单步重试或单步退回

**不适用**：
- 强隔离性要求的金融核心账务、证券撮合 → 用真正的分布式事务（2PC + 共识，或 [[spanner]]）
- 中间步骤完全不可补偿 → 得改业务流程，saga 救不了
- 短事务（毫秒级）单库 ACID 已够 → 不要为用而用，见 [[bernstein-1981-cc]]
- 业务不能容忍"中间脏态被外部读到"——隔离性弱于 ACID，硬塞会出投诉
...
- [[brewer-cap-2000]] —— CAP 理论；saga 常与最终一致/高可用取向搭配，但本身解决的是隔离性换吞吐
