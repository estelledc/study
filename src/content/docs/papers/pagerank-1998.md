...
1998 年 Stanford 两个博士生 Larry Page 和 Sergey Brin 写了这篇 6 页技术报告。同年他们成立了 Google。

## 为什么重要

不理解 PageRank，下面这些事都没法解释：

- 1998 年之前，搜索引擎（AltaVista / Yahoo / Lycos）都靠**关键词出现次数**排序，被刷词的垃圾页面灌满；PageRank 把"被多少人引用"变成排序信号，搜索质量翻倍
- 今天推荐系统的"图冷启动"（用户-商品二部图打分）用的是 PageRank 的孙子 Personalized PageRank
- 学术论文的影响因子、Eigenfactor、Twitter 大 V 排名、生物蛋白网络的关键节点……都是 PageRank 思想的搬运
- GNN（图神经网络）里"节点表示 = 邻居表示加权和"的递归式，与 PageRank 同源
- Google 的 1.7 万亿美元市值，根在这 6 页论文里
...
设 `d = 0.85`，初始 `PR = [0.25, 0.25, 0.25, 0.25]`。一次迭代后大致：

```
PR(A) = 0.15 + 0.85 × ( PR(C)/1 ) = 0.15 + 0.85 × 0.25 = 0.36
PR(C) = 0.15 + 0.85 × ( PR(A)/2 + PR(B)/1 + PR(D)/1 ) = 0.69
```

迭代几十次后会收敛。直觉上，C 被三个页面指向，分数最高；D 没人指向，分数最低（只剩 `1-d` 的底分）。
...
# M[i,j] = 从 j 指向 i 的概率（即 j 出链均分给 i）
...
注意：上面公式行写的是原论文 1998 版 `(1-d) + d × Σ(...)`，这里代码用的是**归一化版** `(1-d)/N + d × M @ r`——把整张图分数总和约束为 1，方便比较。两者数学上等价（差一个常数倍），现代库（NetworkX / igraph / GraphX）都用归一化版。

### 案例 3：为什么阻尼因子救命

考虑两个页面 X 和 Y 互相指向，外面没人引用它们。如果没有阻尼：

- 每次迭代 X 把分给 Y，Y 把分给 X
- 它们互相喂分数，越来越高，把整张图的分数全吸走

加上 `1 - d` 之后，每个节点都漏 15% 出去（随机跳走），小圈子吸不到所有分数，全图收敛到一个稳定分布。
...
4. **6 页论文 → 万亿美元**：理论 + 工程 + 时机三者凑齐的最佳案例之一。
