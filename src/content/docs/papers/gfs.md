...
来源: Sanjay Ghemawat, Howard Gobioff, Shun-Tak Leung, "The Google File System", SOSP 2003
论文年份: 2003
...
### 后作：SeaweedFS（Go 重写，2015-至今）

[chrislusf/seaweedfs](https://github.com/chrislusf/seaweedfs) 是 GFS / Facebook Haystack 思路的 Go 语言现代实现，定位"小文件友好的对象存储 + 文件系统"。架构对照 GFS：

- **Master server** = GFS master（保留 single master 思路，但元数据更轻——只管 volume 不管 chunk）
- **Volume server** = GFS chunkserver（一个 volume = 30GB 的 large append-only file，内部装无数小 needle）
- **Filer** = 上层加的 POSIX-ish layer（GFS 没有，是后来才有的需求）

可对照阅读源码（commit `f4e2a7d` 系列，2026-05-28 读时；star ~25k）：

- Master 调度 volume 分配：[github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/topology/topology.go](https://github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/topology/topology.go)（对应 GFS Section 4.3 chunk placement）
- Volume server append 路径：[github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/storage/volume_write.go](https://github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/storage/volume_write.go)（对应 GFS Section 3.1 mutation flow）
- Replication 协议：[github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/storage/store_replicate.go](https://github.com/chrislusf/seaweedfs/blob/f4e2a7d/weed/storage/store_replicate.go)（对应 GFS Section 3.1 lease + primary-secondary）

**关键差异**：SeaweedFS 把 chunk 概念替换成 needle（< 1MB 小对象）+ volume（30GB 大文件）两层，专门优化"图片 / 头像 / 短视频缩略图"等小文件场景——**正面回应 GFS 64MB chunk 不适合小文件的批评**。

### 后作：Ceph（2006 OSDI，对手路线）

[ceph/ceph](https://github.com/ceph/ceph) 是 Sage Weil（UCSC）2006 年 OSDI 论文 "Ceph: A Scalable, High-Performance Distributed File System" 的实现，**直接挑战 GFS 的 single master 设计**。star ~14k。

核心反对论点：**metadata 也应该分布式**——通过 CRUSH 算法（pseudo-random hashing）让 client 直接计算对象位置，无需 master 查表。

源码可对照（commit `c8a3d1f`，2026-05-28 读时）：

- CRUSH 算法实现：[github.com/ceph/ceph/blob/c8a3d1f/src/crush/mapper.c](https://github.com/ceph/ceph/blob/c8a3d1f/src/crush/mapper.c)（这是 GFS 不存在的 component——把 master 的元数据查询完全消除）
- OSD（object storage daemon，类比 GFS chunkserver）：[github.com/ceph/ceph/blob/c8a3d1f/src/osd/OSD.cc](https://github.com/ceph/ceph/blob/c8a3d1f/src/osd/OSD.cc)
- Monitor（类比 GFS master，但是分布式 Paxos 集群）：[github.com/ceph/ceph/blob/c8a3d1f/src/mon/Monitor.cc](https://github.com/ceph/ceph/blob/c8a3d1f/src/mon/Monitor.cc)

**关键差异**：

| 维度 | GFS | Ceph |
|---|---|---|
| Master | single + shadow | 分布式 monitor 集群（Paxos） |
| 对象定位 | client → master 查 → 拿 chunk handle | client 直接 CRUSH 计算 |
| 一致性 | relaxed（at-least-once） | strong（per-object PG primary） |
| API | 类 POSIX + record append | 对象（RADOS）+ 块（RBD）+ 文件（CephFS）三层 |

Ceph 的"无 master 元数据"路线在 2010s 云存储兴起后更受欢迎——OpenStack Swift / MinIO 都借鉴了 CRUSH 思路。**这是 GFS 的"strong" 反对论点的成功实现**：metadata 也能去中心化。
