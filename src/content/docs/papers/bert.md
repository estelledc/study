...
- 为什么 Google 搜索 2019 年把 BERT 用进查询理解：它影响了约 10% 的英文查询，不是简单"提升 10% 分数"
- 为什么后来很多语言模型工作都绕不开 BERT 的经验：预训练、Transformer 表征、公开 checkpoint 和微调流程一起变成基础设施

引用数 100k+，是 NLP 史上最常被引用的一批论文之一。BERT 这个名字在 2018-2020 年几乎等同于「预训练语言模型」本身。
...
- 文本分类、句对匹配、抽取式问答、命名实体识别这类"读完整输入再判断"的理解任务。
- 需要稳定向量表示的检索和排序场景，例如双塔召回、cross-encoder rerank、企业语义搜索。
- 数据量不大但能拿到预训练 checkpoint 的业务，微调几轮就能得到强 baseline。

**不适用**：

- 长篇生成、开放对话、代码补全这类"一个 token 接一个 token 写下去"的任务，decoder-only GPT 路线更合适。
- 超长文档直接建模，原始 BERT 的 512 token 限制会迫使你切片或换 Longformer / ModernBERT。
- 低延迟边缘设备上的大规模在线推理，BERT-large 这类模型的 encoder 计算成本不低。
