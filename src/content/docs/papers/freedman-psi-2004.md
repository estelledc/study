...
接收方用 Paillier 同态加密发来加密的 xᵢ，发送方在密文上计算 P(xᵢ) 加上随机噪声，返回密文结果。接收方解密后：结果为零说明 xᵢ 在 Y 里；结果非零（因为噪声）说明不在。发送方全程没有看到明文 xᵢ，更不知道哪些查询命中了。
...
def blind_query(phone_numbers, server_public_key):
    """
    1. 把手机号哈希成大域元素
    2. 用服务端公钥做 Paillier 加密
    3. 发到服务端
    """
    encrypted = []
    for number in phone_numbers:
        h = int(hashlib.sha256(number.encode()).hexdigest(), 16)
        enc = paillier_encrypt(h, server_public_key)
...
def check_password_leaked(password, leaked_db_oprf_outputs):
    ...
    本地：去盲化后与服务端集合求交
    """
    blind_factor = random.randint(1, ORDER - 1)
    blinded = oprf_blind(password, blind_factor)
...
2. **半诚实假设不够用**：标准模型证明只挡"诚实但好奇"的对手；恶意对手（会篡改消息）需要 cut-and-choose 加固，通信量通常增加 10–40 倍；部署前必须对齐威胁模型。
...
- [[diffie-hellman-1976]] —— DH 假设是 Paillier 加密安全性证明的理论基础
