...
- **UnitPropagate**：F 中存在子句 `C ∨ l`，C 全在 M 里被假，则把 l 加进 M
...
- **Conflict / Explain / Learn / Backjump** —— 学习冲突子句并跳得更远
...
1. UnitPropagate：a, b, e 入 M
2. T-Propagate：LIA solver 看到 `b ∧ e`（即 `y≥0 ∧ y>3`），暗示 `y ≥ 4` —— 不是某个布尔文字，但更新 theory 状态
3. Decide：试 c（即 `x+y<2`）
4. T-Conflict：LIA solver 拿到 `x≥0 ∧ y≥4 ∧ x+y<2`，矛盾！解释 = `{a, e, c}`
5. T-Learn：加入子句 `¬a ∨ ¬e ∨ ¬c`
6. Backjump：撤销 c 决策，UnitPropagate 强制 d
...
早期 SMT（1990s CVC Lite）的 *lazy* 路线：

```
SAT 解出布尔模型 → theory 验证 → 矛盾就把否定加进 F → 重新解 SAT
```

每轮 SAT 都从头跑，theory lemma 只在外圈学。慢在两点：theory 等到尽头才上场；学到的 lemma 不能指导 SAT 更早剪枝。

DPLL(T) 的 *online* 路线：theory 与 SAT 同呼吸，每加一条文字 theory 就有机会喊"等等，矛盾"或"再加一条"。Z3 实测对工业实例快 10–1000 倍。

### 案例 3：你电脑里 DPLL(T) 在哪儿

- **Z3 内核**：`src/smt/smt_context.cpp` 的主循环就是抽象 DPLL 的实现，theory plugin 提供 T-Propagate
- **CVC5**：`src/theory/theory_engine.cpp` 把多个 theory solver 接到一个 DPLL(T) 框架
- **Bitwuzla**（位向量专用）：DPLL(T) + bit-blasting + propagation-based theory solver
- **Dafny / F\* / Boogie 验证程序**：底层调 Z3，每一步都跑 DPLL(T)
- **Rust borrow checker / Polonius**：内部约束求解就是 DPLL(T) 风格的 datalog 推理
...
- 短版 CAV 2004：*DPLL(T): Fast Decision Procedures*（10 页，看不下长版的可以先看这版）
- 教科书：Bradley & Manna *The Calculus of Computation*（2007，Chapter 11 专讲 DPLL(T)）
- 教学：Kroening & Strichman *Decision Procedures*（2nd ed 2016），实战配套
- 工业实现：Z3 源码 `src/smt/smt_context.cpp` 与 CVC5 `src/theory/theory_engine.cpp`
- [[dpll-1962]] —— 抽象 DPLL 抽象的就是它
- [[chaff-2001]] —— 抽象规则的工业基础（VSIDS / watched literals）
- [[minisat-2003]] —— 本框架里 SAT 引擎的典型实现
