...
3. **Cast 插入**：在动静交界处偷偷插入运行时检查。源码里你看不见这些 cast，编译器把代码翻译成内部语言时插上去。向 `?` 的向上 cast 通常不检查；从 `?` 落到具体类型（如 `⟨int⇐?⟩e`）才会在运行时验货。
...
let z = y * 2      // 运行时：Number("hello") 失败 → NaN（不是字符串拼接）
```
...
- TypeScript **没插入运行时 cast**（这是它和严格渐进类型的差别），所以错误不会立刻报；`*` 会把操作数当数字，得到 `NaN`（若写成 `y + 2` 才会因字符串拼接得到 `"hello2"`）
