...
3. **推理树（derivation tree）**：把规则串起来，得到一棵从『公理』（数字字面量直接 ⇓ 自己）到『最终结论』的树。这棵树**就是**程序的语义。一棵存在的有限推理树 = 程序终止；找不到树 = 程序不终止。

三步合起来：**判断 + 规则 + 树**，没有循环、没有变量、没有副作用，纯数学定义。
...
2. **big-step 和 small-step 是同一棵树的两种切片**——前者只看根和叶（结果），后者看每根边（步骤）
