...
- 为什么 Dapper 论文开头先讲一句"X-Trace 那条路太理想，我们退一步只做一家公司内"——X-Trace 是它的直接前辈
...
Dapper（2010）：
RootSpan
└─ChildSpan（parent_span_id 串起来，只有一种边）
```
...
Dapper 把 NEXT/DOWN 合二为一，因为它只追"一家公司内 RPC 调用树"——所有调用都是 DOWN 性质。X-Trace 想追"跨协议层"，所以必须保留两种边的区别。

### 案例 3：HTTP header 注入示例

X-Trace 通过 HTTP 自定义头携带元数据：
...
W3C `traceparent` 现在长的样子，几乎是这套结构的现代版本。
...
- 测试场景下的端到端验证（所有节点都受控，部署完整）

**不适用**：
- 跨公司端到端追踪（信任 + 部署完整性双重难题）
- 极致低开销场景（report 流量没采样兜底）
...
4. **基础设施型论文的影响力 = 后续系统继承了多少术语**。X-Trace 没活下来，但 traceparent / span / parent / report 这些词的语义全是它定义的。
