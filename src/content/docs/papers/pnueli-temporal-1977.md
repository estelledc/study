...
- 为什么 SPIN / NuSMV / TLC 这些模型检测器都吃"时序公式"——1981 起模型检测把 LTL/CTL 自动化，才能穷举跑
...
Lamport 把 Pnueli 的 LTL 扩展成 TLA+。Amazon 的 S3 / DynamoDB，以及微软的 Cosmos DB，在写代码前用 TLA+ 把协议跑一遍，找到过几个"代码 review 看不出但 TLA+ 几小时就抓到"的并发 bug。
...
- **1981 年**：Clarke-Emerson（CTL）与 Queille-Sifakis 各自独立发明**模型检测**，把时序公式自动化跑出来；LTL 模型检测随后也成熟。
...
- [[mcmillan-smv-1993]] —— 符号模型检测，把时序公式能验的状态空间再推一个数量级
