...
1. **跳表式分层**：每个新点的最高层号按指数分布抽样：归一化因子 `mL = 1/ln(M)`，层号 `floor(-ln(U)·mL)`（`U` 是 0–1 均匀随机数）。相邻层大约只留下 `1/M` 的点。类比：跳表（skip list）里高层节点稀疏——上层跳得远，下层搜得精。

2. **小世界图 + 贪心搜索**：每层是一个邻居图——每个点最多连约 M 条边（论文用启发式选边，不完全是"最近的 M 个"）。查询时从入口点出发，**只看邻居里离 q 最近的那个**，跳过去；不能再近了就停。这一步类比：在朋友圈里找"认识 Linus 的人"，每问一个朋友给你最接近的下一个介绍。

3. **ef 搜索宽度**：贪心容易卡局部最优。HNSW 同时维护一个"动态候选堆"，宽度由 `ef` 参数控制——`ef=1` 是纯贪心，`ef` 提到 100–200 时召回往往接近饱和，但代价也逼近暴力扫描。`ef` 越大召回越高、QPS 越低，是最重要的运行时旋钮。
...
- [[skip-list-1990]] —— 跳表是 HNSW 分层（mL=1/ln(M)）的直接灵感来源
