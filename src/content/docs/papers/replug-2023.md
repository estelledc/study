...
REPLUG 解决的就是这个问题：**只用 LLM 的输出概率作监督信号**，反向训练检索器。它证明了：黑盒 LLM 也能享受到"为它定制的检索器"带来的提升。这套思想后来被几乎所有商用 RAG 框架（LlamaIndex / LangChain 的 retrieval router）继承。
...
REPLUG 把训过的检索器接到 GPT-3（175B）上，在 Wikitext-103 / The Pile 等数据集上跑语言建模。结果：困惑度（perplexity）平均下降 6.3%，最大类别 Books 降 9.6%。**GPT-3 没动一行代码**，仅靠"换了个更懂它的检索器"就拿到提升。

### 案例 2：在 Codex 上提五-shot MMLU

MMLU（多学科问答）上，Codex 175B baseline 是 67.6 分；接上 REPLUG 后到 71.4 分（+3.8）。比"用现成的 Contriever 检索器"高 2 分以上——说明**为这个 LLM 定制的检索器**比通用检索器明显更好。
...
for batch in training_data:
    ...
    # 3. 让两个分布尽量接近（softmax 归一化后做 KL）
    loss = KL(softmax(scores_retriever) || softmax(scores_lm))
    loss.backward()  # 只更新 encoder 参数
```

注意：这里 LLM 只前向、不反向；它**只贡献一个概率数字**，不需要梯度。这就是"黑盒"二字的精确含义。换言之，REPLUG 把"训练 RAG"这件事的接口收窄到"LLM 能不能给我一个 logprob"——OpenAI / Anthropic 的 API 都能，所以它能落地到任何商用 LLM 上。

### 案例 4：与 In-Context RAG 的差距

普通 RAG 把 top-k 篇直接拼到 prompt 里：`[doc1][doc2]...[docK] Question: ...`。问题：

- context 长度被检索结果挤占，长上下文模型也吃力
- 多篇文档相互干扰，LLM 容易抓错重点
- 噪声文档（误检）会"污染"整段 prompt

REPLUG 的 ensemble 模式让每篇文档**独立提议一个答案分布**，再投票合并。论文实验在 NQ / TriviaQA 上证明：同样 K=10，ensemble 比 concatenation 高 1-3 个百分点。

## 踩过的坑

1. **LSR 训练成本不低**：每个 batch 要调 K 次 LLM API（K 通常 10-20），训一次跑下来 OpenAI 账单可能过万美元。论文里训了 1.6 万步，按 GPT-3 当时定价约 5 万美元。中小团队复现要么用蒸馏的小 LLM 当老师，要么只训少量步数。
...
- **2023-2024 年**：LSR 思想被 LlamaIndex / LangChain / Haystack 的"retrieval ranker"吸收，成为商用 RAG 的标配。
...
- 复现项目：[FlagOpen/FlagEmbedding](https://github.com/FlagOpen/FlagEmbedding) 里的 LM-Cocktail 是 LSR 思路的开源版
