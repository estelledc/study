...
function owner(keyPos) {
  const sorted = [...nodes].sort((a, b) => a.pos - b.pos)
  return sorted.find(n => n.pos >= keyPos)?.id ?? sorted[0].id
}
console.log(owner(40)) // C：从 40 顺时针碰到的第一个节点
// 若在 70 插入 E，只有 (60,70] 这段从 D 迁到 E，其余不动
```

**逐部分解释**：

- 这是环上定位，不是 `hash % N`；取模会在加减节点时几乎全量重排
- `owner(40)`：key 落在 B(35) 与 C(60) 之间，顺时针交给 C
- 新增节点只搬走它「前一段弧」上的 key，这才是一致性哈希少搬家的来源
