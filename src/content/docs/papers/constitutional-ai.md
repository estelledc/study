...
这个"AI 自己写宪法 + 自我批评"的方法，是 Claude 系列模型的核心训练范式。
...
- 为什么 Anthropic 敢公开"我们用什么标准训"——因为对齐目标是 12-16 句英文，不是黑箱权重
- 为什么大厂训 LLM 的标注成本能从"几亿美金"降到"几百万美金"——harmless 部分被自动化了
...
2. **RL-CAI（强化学习阶段）**：让 LLM 当 reward model（不用人工），它看两个候选回答 (A, B)，对每条原则算一次 logp，平均得到偏好分数，再用 PPO 训。

支撑这两个阶段的是 **宪法**：约 12-16 条英文原则文本，例如「请选择最不有害、不种族歧视、不性别歧视的回答」。这组原则取代了 [[instructgpt]] 里标注员的隐式偏好。
...
- **2025**：Claude 4 系列仍延续 CAI 思路，宪法版本迭代到第 N 代
