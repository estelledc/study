...
CoT 一次性写完三步，第二步选错就全错。ToT 每步留多个分支，相当于"棋谱搜索"。代价：论文测得 ToT 约 **5–100×** CoT 的生成 token，Game of 24 上 GPT-4 每题约 **$0.74**（接近跑 100 次独立 CoT 的 token 量，但正确率更高）。

### 案例 2：5×5 填字游戏，DFS 回溯救场

填字游戏没法 BFS（分支爆炸），ToT 用 DFS，一步填一个词：

```
线索 h1「展示」→ 候选 SHOWN
  Evaluator：其余线索仍可能 → 继续
线索 v1「堆积」→ 候选 HEAP? 但首字母已被 S 占住
  → 矛盾，标 impossible → 回溯，换 h1 的下一个候选
```

- CoT **词级**正确率约 **16%**（字级约 41%），ToT 词级 **60%**（字级 78%），并解开 4/20 盘
- 回溯是关键：论文去掉 backtrack 后词级掉到 20%
...
generate 和 evaluate 都是对 LLM 的额外调用——token 成本随分支数与深度线性涨。

## 踩过的坑

1. **搜索成本爆炸**：论文写明可比 CoT 多花 **5–100×** 生成 token。Game of 24 这种短题还能接受，长上下文任务几乎跑不动。

2. **Evaluator 会误判**：让 LM 自评并不稳。填字实验里，盘面其实已解出时，evaluator 仍可能把某些词标成"impossible"而剪枝——启发式剪枝本身会漏解。

3. **任务分步靠人写**：论文里每个 benchmark 的"一步是什么"都是手工定义的（24 点 = 3 步算术，填字 = 最多 10 步填词）。换个任务你得重新设计分步规则，没法对任意自然语言问题自动展开。
