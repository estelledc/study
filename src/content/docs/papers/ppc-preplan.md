...
在 LLM 推理里，传统 CoT（Chain-of-Thought，把中间推理写出来）是 question -> cot；计划型方法是 question -> plan -> cot。PPC 说中间还少了一步：question -> preplan -> plan -> cot。
...
- 为什么 PPC 能在不增加推理 token 的情况下提升 maj@16 和 pass@16（多采样里多数票 / 至少一次对的指标）
...
3. **奖励要逼 plan 真的听 preplan**。
   只奖励答案正确，模型可能把 preplan 写成装饰品。PPC 在 GRPO（一种相对排序的强化学习，不必另训 critic）里加入 plan-preplan adherence，让计划必须继承前面的问题理解。
...
```txt
<preplan>
这是「先认结构再动手」的题，重点是判断方程能不能因式分解。
需要关注：分解后每一支代表一类解，而不是一个孤立数字。
主要风险：一上来当普通求根题硬套公式，漏掉结构捷径。
</preplan>
```
...
- 只有 `score <= 2` 且最终答案正确的轨迹，才进入 SFT（监督微调）数据

### 案例 3：复合奖励不是只看答对

PPC 在 GRPO 里把奖励拆成四块（权重为教学示意，突出「答案仍最大、结构也要管」）：
...
4. **以为多一段就一定更慢**：原因是 preplan 会砍掉错误路线；论文报告在不增加推理 token 开销的前提下提升指标，而不是「必然更短」。
