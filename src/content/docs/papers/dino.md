...
待补充：写完 [[vit]]、[[clip]]、[[sam]] 的笔记后回来填反向引用。

候选反向引用方向：

- 任何讨论「视觉 foundation model」的笔记 → DINOv2 是 dense feature 一极
- 讨论 EMA / momentum encoder 的 SSL 笔记 → DINO 是 BYOL 之后 EMA 路线的 ViT 版
- multi-crop / view augmentation 主题 → DINO 把 SwAV 的 multi-crop 在 ViT 上发挥到位
- emergent / grokking 类「涌现现象」笔记 → DINO emergent attention 是工程级 emergence 的代表案例
