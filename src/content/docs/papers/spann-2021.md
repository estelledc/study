...
- 为什么 1B 规模向量检索的内存占用从 100+ GB 一路降到 GB 级——SPANN 把"完整向量"赶下了 SSD
...
1. **内存层（Centroid Index）**：把 1B 个向量聚成几万到几十万个簇，每簇取一个中心点，所有中心点放进一个内存索引（SPANN 用 SPTAG 的 BKT/KD-tree 混合）
2. **磁盘层（Posting Lists）**：每个簇的所有完整向量打成一个 posting list，落到 SSD

查询时三步：

1. 在内存索引里找最近的 **k 个**中心（典型 k=64）
2. 只从 SSD 读这 k 个对应的 posting list
3. 在读到的向量里精排，返回 top-N

听起来朴素，但要在 1B 规模真跑起来，SPANN 解了**三个工程难题**：

- **层次平衡聚类（Hierarchical Balanced Clustering）**：朴素 k-means 在 1B 数据上会让某些簇有几百万点、有些只有几百，磁盘读极不均匀。SPANN 用递归二分平衡聚类，强制每簇大小接近
- **边界点复制（Closure Boundary Expansion）**：聚类边界附近的点，可能它的真实最近邻在邻簇里。SPANN 把这些点**复制**到邻簇 posting list，代价是磁盘空间约 1.5×，换 recall 大幅提升
...
```text
HNSW (2018):   ~150 GB 内存 (图 + 全部向量)
DiskANN (2019): ~64 GB 内存 (压缩向量 + 部分图)
SPANN (2021):   ~10 GB 内存 (只有 centroids + 元数据)
```

向量数据本身按 fp32 存就 480 GB（1B × 128 维 × 4 字节）。SPANN 把 480 GB 全部赶到 SSD，内存只留"目录"。

### 案例 2：层次平衡聚类长什么样

```text
顶层：把 1B 个向量二分 → 两个 0.5B 簇
向下：每个 0.5B 再二分 → 四个 0.25B
...
递归到每簇约 1000-10000 点为止
```

最终得到几万到几十万个**大小相近**的叶簇。每个叶簇的中心进内存索引，叶簇本身落 SSD。
...
这一招让 SPANN 在 recall@10 = 0.9 这条线上比 FAISS-IVF 高出 5-10 个百分点。
...
2. **聚类数选不好两头吃亏**：簇太少 → posting list 太大磁盘读放大；簇太多 → 内存里 centroid 索引膨胀，失去意义。SPANN 论文给的经验是 1B 数据用约 64 万簇
3. **边界复制不是免费的**：磁盘空间多 1.5×，build 时间也变长。如果应用允许低 recall（例如 0.7），可以关掉这步省成本
...
- 召回率要求 0.85-0.95（top-k recall），延迟容忍 5-50ms
...
3. **复制换召回是值得的**：1.5× 磁盘空间换 5-10 个点 recall，工业上几乎总是划算
