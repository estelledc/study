...
PinSage 上线后 Pinterest 推荐相关指标显著提升（论文主报 offline hit-rate / engagement，常被转述为约 30% 量级增益）。这之后大厂图推荐架构基本都长成：**采样器 + 聚合器 + 训练框架**。
...
2. **训练聚合函数，不训练节点嵌入表**——归纳式仍训练 W 等参数；本质是把"每个节点一条可学习向量"换成"怎么从特征+邻居算出嵌入的函数"
