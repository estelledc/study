...
2. **Metropolis-Hastings 负责“抽难抽的分布”**。
   类比：你不知道整座城市每条路的总热度，但可以比较两条路线哪条更像常走路线。
...
```python
def accept(old_score, new_score):
...
- 真实算法还会乘上提议分布和切点分布的校正项。
- 这段只保留直觉：新轨迹更符合目标分布，就更容易被接受。
- 关键是 Entropy-Cut 改的是提议方式，不是偷偷换了最终要采样的目标。
...
- 数学、代码、科学问答这类有长推理链的任务。
- 想在推理时提升模型表现，但不想重新训练、标数据或训练 verifier 的场景。
