...
off_t size = /* 文件长度，可用 fstat */;
char *p = mmap(NULL, size, PROT_READ, MAP_PRIVATE, fd, 0);
if (p == MAP_FAILED) { /* 处理错误 */ }
char byte = p[i];  /* 像读数组一样读文件第 i 字节 */
```

逐部分解释：

- `open` 拿到文件描述符；在 MULTICS 里对应"打开一个段"
- `mmap` 把文件映射进进程地址空间，返回指针 `p`
- 之后 `p[i]` 直接触达文件内容，不必先 `read` 进 buffer
- MULTICS 1965 年默认就是这样：打开文件 ≈ 拿到一段虚拟内存

### 案例 2：ring 保护今天为什么只剩 2 级

按层次对照读一遍：

1. MULTICS 用 8 个 ring：内核（最内）→ 系统服务 → 受信用户库 → 普通用户（最外）
2. 每跨一层都走"调用门"检查参数，由硬件 descriptor bit 强制
3. x86 硬件其实也有 4 个 ring（0/1/2/3），但 Linux/Windows 都只用 0 和 3
4. 原因是软件复杂度——分层越多、调用门越绕；今天的 seccomp、capabilities、eBPF 是在**软件层**把分层补回来

### 案例 3：动态链接不是 Windows DLL 的发明

很多人以为 DLL 是 1990 年代 Windows 发明的。MULTICS 1965 年就有 runtime 按符号链接：程序写 `call sin`，**第一次跑到那一行**才解析地址。代价是符号表常驻、链接器是 OS 一部分。Unix 嫌重砍掉后，Linux 的 `ld.so` 看起来像"重新发明"。

### 案例 4：层级账户树 vs K8s ResourceQuota

MULTICS：根账户拥有全部 CPU/磁盘配额，只能委派**自己拥有的子集**，按微秒记账。把 MIT 改成 namespace、教授改成 sub-namespace，就是同款 ResourceQuota 思想（教学对照，不是字面继承）。
## 踩过的坑
