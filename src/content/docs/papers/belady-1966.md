...
更重要的是，他还顺手发现了一个**违反直觉的怪现象**——FIFO 异常。下面会讲。
...
现实中你看不到未来，怎么办？**赌一把**：刚才用过的，多半马上还会用；很久没用过的，多半短期不会再用。这就是 LRU（Least Recently Used）——踢掉**最久没访问**的那一页。

LRU 之所以在工程里好使，**不是因为它聪明**，而是因为大多数程序的访问有"局部性"——刚摸过的内存大概率马上再摸。LRU 是 OPT 的"过去版"，OPT 是"未来版"。

### FIFO 异常：内存加一倍，缺页反而更多

直觉告诉你：**内存越大，缺页应该越少**。Belady 1969 年和 Nelson、Shedler 在 CACM 上发表的后续工作发现了一个反例：FIFO 在某些访问序列下，**3 个槽缺 9 次，4 个槽反而缺 10 次**。
