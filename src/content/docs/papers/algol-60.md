...
- 为什么函数参数有"按值传递"和"按引用传递"两种说法——ALGOL 60 第一次形式化定义了这个区别
...
ALGOL 60 是 **第一门官方支持递归的高级语言**（FORTRAN I/II/IV 都不支持）。支持递归靠的是栈帧——每次调用都开一个新内存空间存参数和局部变量。这件事现在所有语言的运行时都在做。
...
3. **可被机器消费的形式**比"严格"更重要——BNF 的真正贡献不是把语法定得严密，而是让"语法"从人类对话变成机器可读的对象。这是从工艺到工程的关键一跃。

4. **17 页的密度可以击穿 60 年**——ALGOL 60 报告每一句都在做形式定义，没有一句废话。今天看 Naur 的写作仍然觉得 dense。**密度比页数重要**。
