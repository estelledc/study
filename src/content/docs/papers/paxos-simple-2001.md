...
- 为什么 proposer Phase 2 不能自由选值，必须 replay 已经被 accept 过的最高编号值
...
Lamport 在这篇里把整个推导链拆成 **三层**，每一层只解决前一层暴露的一个矛盾：

1. **safety 母条件 P1 + P2**：P1 要求"acceptor 必须接受第一个收到的提议"（保活）；P2 要求"如果 v 被 chosen，所有更高编号的 chosen 提议也必须是 v"（保一致）。两条像物理基本定律。
...
3. **从 P2c 推出 prepare/accept**：proposer 要满足 P2c，就必须先问多数派"你接受过什么"——这就是 prepare/promise；拿到回答后**只能 replay 最高编号值或自由提一个新值**——这就是 accept/accepted。**整个协议是 P2c 不变量的强制推论，不是设计选择**。

4. **最后才出现 leader**：Lamport 在 §2.4 Progress 才引出 distinguished proposer，并明确"safety 与 leader 选举的成败无关"。这是这份论文最常被工程师忽略的一句话。

5. **§3 才讲 SMR**：把 single-decree synod 套到一长串 log slot 上，每个 slot 跑一次 Paxos 实例。leader 一次发 Phase 1 给所有未来 slot，正常态下每条命令只走 Phase 2 一个 RTT——这是 Lamport 给的"理论极简骨架 + 工业必加优化"完整组合。
...
整个推导链 4 步，论文用 1 页讲完。这就是为什么 Lamport 说"follows almost unavoidably"——加强"任谁来读都只能想到这一种协议"的姿态。读者不是被告知协议，而是被牵着手一起推出来。
...
leader 一次跑无穷多个 slot 的 Phase 1（每个 acceptor 用一条短消息 OK 全部）。新 leader 上任时 replay 已知 chosen 值 + no-op 填空洞——这套是 Lamport 在 §3 给的，不是工程师后补。论文里写 "a leader can get α commands ahead"——α 是配置上限，决定 leader 失败时最多漏掉多少 slot。Spanner / etcd 的 pipelining 参数全在调这个 α。
...
- 理解 Multi-Paxos 与 SMR 的关系——§3 的 14 段叙述是工程蓝图
- 校准对 [[raft]] 的认知——Raft 的"易懂"标的就是这份 2001

**不适用**：

- 真正动手实现——细节缺失太多，必须配合 *Paxos Made Live* 与具体开源代码
...
- [[bigtable-2006]] —— 跑在 Chubby 之上的列式存储，证明 Paxos 抽象足以承载 PB 级业务
