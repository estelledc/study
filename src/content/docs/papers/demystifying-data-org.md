...
2. **训练顺序有四条原则**。类比：健身计划要热身、复习基础动作、动作难度平滑增加，还要避免每天只练同一块肌肉。论文把它叫 Boundary Sharpening、Cyclic Scheduling、Curriculum Continuity、Local Diversity。

3. **STR 和 SAW 是两种组合排队法**。类比：STR 像楼梯，在整体往上走时在台阶处回看基础；SAW 像锯齿，在过渡区用更平滑的来回走法。SAW 比 STR 多强调连续性，避免分数突然大跳。
...
- 这对应论文里的 FO：它比纯课程学习更像“边学新题，边复习旧题”。
...
3. **只看平均分不看训练过程**：FO 和 ZIG 的差别常体现在梯度是否突然尖峰，最终平均分掩盖不了优化稳定性问题。
...
4. **一点局部随机性是好事**：JIT 的价值在于别让每个 batch 都太单调，它像给整齐队伍保留呼吸空间。
