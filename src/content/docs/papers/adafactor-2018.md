...
它由 Google Brain 的 Noam Shazeer 提出，是 **T5（11B 参数）、PaLM（540B）** 这些大模型实际训练用的优化器。
...
- 为什么 T5 / PaLM 论文里反复提"用 Adafactor 优化器"——不是品味，是**显存逼出来的选择**
- 为什么 LoRA / DeepSpeed 这些"省显存"工具链背后还是离不开 Adafactor 思路
- 为什么 Hugging Face Transformers 默认给大模型推荐 Adafactor
...
2. **丢掉动量 m**：Adam 的一阶矩也是 O(d)，Shazeer 发现训 Transformer 时其实可以丢，再省一半
...
这是一种**秩 1 近似**——假设 V 大致可以写成"行因子 × 列因子"的乘积。Shazeer 证明在二阶矩这种"梯度平方均值"上，这个假设近似成立。
...
测试方法：在 WMT EN-DE 上跑 200K 步，Adafactor 不带动量的 BLEU 和 Adam 带动量的 BLEU 差距 < 0.3——在统计噪声范围内。
...
1. **二阶矩可以低秩近似**——不是所有"看起来高维"的量都需要全维度存。这思路后来直接启发了 LoRA（低秩微调）
