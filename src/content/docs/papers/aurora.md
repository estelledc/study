...
为什么是 4/6 而不是 5/6 或 3/6？4/6 容忍 "AZ + 1 节点"——一个 AZ 整挂、再加任意一个其他 AZ 内的 1 个节点挂，仍能写。再激进一点（3/6）持久性不够，再保守一点（5/6）写延迟变差。

论文 §2 的 "tolerate AZ + 1 node failure" 是 quorum 调参的精确锚点：副本数 6 × AZ 数 3 × 容忍 1 个 AZ + 1 节点的下界，唯一让 R + W > N 同时不暴露读路径的组合。
...
- [[bigtable-2006]] —— 共享日志 + LSM 存储的工业先驱，Aurora 是关系世界的对应物
- [[snowflake-2016]] —— 同期分离存算 OLAP 系统；Aurora 反过来用同思路做 OLTP
