...
落到硅片：**GP100 = 153 亿晶体管、TSMC 16nm FinFET、56 SM × 64 FP32 cores = 3584 FP32 + 1792 FP64 cores、16GB HBM2、720 GB/s 带宽、300W TDP**。代表卡：**Tesla P100（SXM2/PCIe）**、**DGX-1（8× P100 第一台 AI 整机）**。消费线 GP102/104（Titan X Pascal / GTX 1080）则是另一套设计，**没有** HBM2、**没有** NVLink、**没有** 1/2 FP64——这点很容易混。
...
- 为什么 **DGX-1** 这台 12.9 万美元的整机会成为后续所有 AI 服务器（DGX-2/A100/H100）的模板——**8 卡全连接 NVLink** 是它先做出来的
- 为什么 **HBM2** 后来成 AI 卡标配——P100 是第一张量产 HBM2 GPU，证明这个工艺能跑生产负载
- 为什么 **混合精度训练（AMP）** 在 2017-2018 才爆发——硬件 FP16 加速从 P100 起步，没它 AMP 是空中楼阁
...
1. **HBM2 取代 GDDR5**：不再走外圈 PCB 走线，**显存芯片堆叠到 GPU 同一封装**（2.5D interposer），4 个 stack × 4GB = **16GB / 720 GB/s**——是 M40 GDDR5 的 ~2.1 倍。意义：训练吞吐瓶颈从"算不过来"变成"算得过但喂不饱"——HBM2 把喂饱的天花板抬一倍。

2. **NVLink 1.0 打破 PCIe**：每张 P100 SXM2 有 4 条 NVLink，每条 **40 GB/s 双向**，合计 **160 GB/s**——**PCIe Gen3 x16 仅 16 GB/s**。意义：多 GPU 训练里的 all-reduce 不再被 PCIe 锁死，**8 卡训 ResNet-50 接近线性加速**。注意 PCIe 版 P100 **没有** NVLink。
...
NVLink topology: 混合立方网格（hybrid cube mesh）
每对 GPU 直连 NVLink，无需绕 PCIe，CPU
峰值：170 TFLOPS FP16、129000 USD
```

意义：**FAIR / OpenAI / DeepMind 2016-2017 主力训练机**就是 DGX-1。后面 DGX-2（16 卡 V100 + NVSwitch）/ DGX A100 / DGX H100 全是同一个套路：**N 卡全连接 + 高带宽 fabric + 整机出货**。

### 案例 2：FP16 packed 怎么写
...
`half2` 把 2 个 FP16 装进一个 32-bit 寄存器，`__hfma2` 一条指令完成两次 fused multiply-add——**理论 2× FP32 吞吐**。注意：必须**成对**操作，奇数个元素只能补齐 padding。
...
- 大模型训练前夜（2016-2018） —— ResNet-152 / Inception-v4 / 早期 Transformer 主力训练卡
...
- DGX-1 架构论文：[NVIDIA DGX-1 With Tesla V100 System Architecture](https://images.nvidia.com/content/pdf/dgx1-v100-system-architecture-whitepaper.pdf)
