...
- 提出"耦合拥塞控制"：多条子流共同竞争带宽时不能比单路径 TCP 更贪心，
  这个公平性约束写入了 RFC 6356。
...
**连接建立：** 第一条子流用正常 TCP 三次握手，SYN 里带 MP_CAPABLE 选项。
中间盒把选项剥掉？没关系，退化成普通 TCP。双方各生成 64 位 key，
用 key 的 hash（token）标识连接。加第二条子流时在另一个接口发起新握手，
SYN 带 MP_JOIN + token，HMAC 认证防伪造。

**数据传输：** 两层序列号——每条子流有独立 TCP 序列号（让中间盒看到合法 TCP），
MPTCP 在 TCP option 里加"数据序列号"（DSN）表示字节在整个连接中的位置。
接收端用 DSN 跨子流重组。子流层面伪装，连接层面还原全局顺序。

**拥塞控制：** 两条子流各自独立跑 CUBIC，在共享瓶颈上会抢两倍公平份额。
耦合算法把各子流 cwnd 增长绑在一起，总增长不超过单路径最优速率。
每收到 ACK，cwnd 增加量 = min(alpha * bytes_acked / cwnd_total, bytes_acked / cwnd_i)，
alpha 按所有子流 RTT 动态算，确保公平性。
...
**Apple Siri：** iOS 7 起 Siri 语音请求走 MPTCP。Wi-Fi 主子流 + 蜂窝备用子流，
从客厅走到电梯 Wi-Fi 消失，毫秒级切到 4G，不中断。这是 handover 模式——
不求带宽叠加，只求无缝切换。后来 Maps、Music 也启用。

**数据中心：** 论文在 EC2 实验显示两条子流走不同可用区间链路，
吞吐接近带宽之和。胖树拓扑下 MPTCP 与 ECMP 互补——
ECMP 按流分路径，MPTCP 在单连接内按包分路径。

**移动视频：** 地铁上 Wi-Fi 热点与 4G 反复切换，普通 TCP 每次要重建连接，
视频缓冲中断数秒。MPTCP 只需 MP_JOIN 加子流，连接不断缓冲不停。

**韩国 KT GiGA Path：** 2015 年全球首个消费级 MPTCP 带宽聚合商用服务，
手机同时聚合 Wi-Fi + LTE 下载，验证了带宽叠加的商业可行性。
...
- [tcp](/papers/tcp) — MPTCP 是 TCP 的扩展，序列号/ACK/拥塞控制是前提
- [quic](/papers/quic) — QUIC 多路径扩展面临类似挑战，在用户空间实现避开中间盒
- [jacobson-1988](/papers/jacobson-1988) — MPTCP 子流拥塞控制的基础算法
- [bbr-2017](/papers/bbr-2017) — 基于带宽估计的拥塞控制，与耦合控制对比
- [ron-2001](/papers/ron-2001) — 应用层多路径方案，与传输层方案形成对照
