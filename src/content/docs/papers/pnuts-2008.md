...
- 为什么 DynamoDB 全局表、Cosmos DB 五种一致性档位、Spanner 的外部一致性，都长得有点像它——它们都是 PNUTS 这个"按记录给一致性"思路的工业后继
...
- 为什么 YCSB（Yahoo! Cloud Serving Benchmark）这套基准是 PNUTS 团队产出——他们最先意识到要量化"读延迟 vs 一致性档位"的代价
...
# 如果有人抢先扣过，version 不匹配，写失败，重试
```

**关键点**：`test-and-set-write` = "写入时附带版本号断言"。这是 PNUTS 给跨记录事务缺失打的补丁——单条记录上的 CAS。
...
- **2008**：Brian Cooper 团队提出 PNUTS——多数据中心 + per-record 顺序。论文发表在 VLDB，同时开源 YCSB 基准。
- **之后**：Brian Cooper 离开 Yahoo!，去 Google 参与 Spanner 系列工作；PNUTS 思想在 Yahoo! 内部继续支持各产品到 2015 年前后。**没开源**，只剩论文，是工业界的"消失的大型系统"代表。
