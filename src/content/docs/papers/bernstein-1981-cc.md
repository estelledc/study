...
Bernstein 和 Goodman 干了一件简单但震撼的事：他们用一个统一模型（TM 事务管理器 + DM 数据管理器），把并发控制问题**拆成两个子问题**——读写同步（rw）和写写同步（ww）——然后说明许多看似不同的算法都能看成这两个子问题的解法组合。

整篇论文 51 页，目录 7 章，覆盖了锁、时间戳、多版本、乐观认证等主要路线——但读完你只需记住一张 2×N 的组合表。

## 为什么重要

不理解这篇综述，下面这些事都没法解释：

- 为什么数据库都说自己支持 serializable，底层却可能是锁、MVCC、时间戳或认证器的不同组合
- 为什么 Spanner 用 TrueTime 选安全快照来跑只读事务——它和时间戳排序同属"先定顺序再判定可见性"的路线
- 为什么"乐观并发控制（OCC）"在早期文献里常以 certifier（提交前认证器）的角色出现——这篇给了它清晰位置
...
3. **复杂算法常是这两条线的混搭**：基础 2PL、主拷贝 2PL、投票 2PL、集中 2PL；基础 T/O、Thomas Write Rule、多版本 T/O、保守 T/O；外加 certifier（乐观）这条旁支。论文的价值不是发明一个万能算法，而是给这些组合画出地图。
...
把每个事务用"成长期—收缩期"两阶段包起来，可以靠显式行锁表达出来：

```sql
-- T1（转账）
START TRANSACTION;                                  -- 成长期开始
SELECT balance FROM savings  WHERE id = 1 FOR UPDATE;  -- 加锁 1
SELECT balance FROM checking WHERE id = 1 FOR UPDATE;  -- 加锁 2（仍在成长期）
UPDATE savings  SET balance = balance - 1000000 WHERE id = 1;
UPDATE checking SET balance = balance + 1000000 WHERE id = 1;
COMMIT;                                             -- 收缩期：一次性放所有锁
```
...
- T2 此时若执行同样的 `SELECT ... FOR UPDATE` 会被阻塞，等 T1 提交后再继续；这是一种工程上常见的严格两阶段释放方式
...
- 写冲突多但事务短的场景：用显式锁或严格两阶段释放，接受等待和死锁重试
