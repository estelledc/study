...
整个 λ-演算只有**变量、抽象（写函数）、应用（调函数）** 三种构造。其他所有东西——数字、Boolean、列表、递归——都用这三种自己堆出来。Church 在 1932/1933 已提出这套记号；1936 年那篇 18 页 AJM 论文则用「λ-可定义性」证明判定问题不可判定——后来所有函数式语言（Lisp / ML / Haskell / Scala）的根，要连着这两步一起看。
...
- 为什么今天前端里的闭包 / 高阶函数（如 React 的 `useMemo` / `useCallback`）读起来不别扭——它们是 λ 里「函数当值传递」的工业用法
...
Church 早在 1932/1933 就写出了 λ-演算；1936 年春天，他用 λ-可定义性在 AJM 上发表 Entscheidungsproblem 不可判定的证明，比 Turing（同年稍后）的图灵机论文早几个月。Turing 当时正打算去普林斯顿读博，看到 Church 的论文后改投其门下。1937 年师徒俩证明两套模型可算的函数集相同，就是后来的 **Church-Turing 论题**。Church 的学生谱系包括 Turing、Kleene、Dana Scott、Michael Rabin——直接打开了「可计算性 + 类型论 + 模型论」三条线。
...
4. **现代特性都能追到早期 λ**：闭包、高阶函数、currying、引用透明——都能在 1930 年代这套记号里找到原型
...
- [[scott-strachey-denotational]] —— Scott-Strachey 指称语义
- [[simply-typed-lambda]] —— 加了类型的 λ
