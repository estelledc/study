...
更具体地说：[[lamport-1978]] 只能给出 a→b ⇒ C(a)<C(b) 的**必要条件**，无法检测并发。Fidge 给进程 i 维护 N 维向量 V_i：本地事件 V_i[i]+=1；发消息时 piggyback 整个 V_i；收消息时逐格 max 再把自己那格 +1。于是 a→b ⟺ V(a) < V(b)；a‖b ⟺ 两向量不可比。

40 年过去，Dynamo version vector、CRDT 的 dot、Riak causal context 都是直系后代；Jaeger 一类 tracing 的 parent/child span 只是「保留因果边」的远亲，并不带 version vector。
...
def write(cart, node_id, new_items):
    vv = dict(cart.vv)
    vv[node_id] = vv.get(node_id, 0) + 1
    return Cart(new_items, vv)

def dominates(va, vb):
    # V(a) < V(b)：逐分量 ≤ 且至少一处严格 <
    keys = set(va) | set(vb)
    less_or_eq = all(va.get(k, 0) <= vb.get(k, 0) for k in keys)
    strict = any(va.get(k, 0) < vb.get(k, 0) for k in keys)
    return less_or_eq and strict

def merge(a, b):
    if dominates(a.vv, b.vv):
        return [b]          # a 更旧，只留 b
    if dominates(b.vv, a.vv):
        return [a]
    # 并发分叉：逐节点取 max 合成 vv，并保留两份 sibling 给客户端 resolve
    keys = set(a.vv) | set(b.vv)
    vv = {k: max(a.vv.get(k, 0), b.vv.get(k, 0)) for k in keys}
    return [Cart(a.items, vv), Cart(b.items, vv)]
```

**逐部分解释**：
- `vv` 是稀疏向量（只记真正写过的节点），等价于 Fidge 的 V_i
- `dominates` 就是论文里的 V(a) < V(b)，用来识别 stale 写入
- 双向都不 dominate 就是并发分叉，Dynamo 把两份 sibling 都返回让客户端 resolve
...
- **1978**：Lamport 逻辑时钟只能判 a→b 不能判 a‖b
- **1988**：Fidge 在 ACSC 发表本文；同年 Mattern 独立提出 Virtual Time，思想几乎一致
- **1989**：Mattern 长文被社区更频繁引用
- **2007 / 2011**：Dynamo 把 version vector 推到工业；[[crdt-shapiro-2011]] 嵌进 OR-Set / RGA
- **2010s**：Yjs / Automerge / Riak DT 让协同编辑大规模用上变种

## 学到什么

1. **partial order 真正可判定**：向量时钟把 Lamport 的必要条件升级为充要条件
2. **空间换信息**：用 O(N) 空间换「是否真并发」的判定能力
3. **同时期独立发现**：Fidge 与 Mattern 几乎同时给出同一思想
4. **不是全序**：不是所有事件都可比——这是分布式系统诚实的时间观

## 延伸阅读

- 论文 PDF：[Fidge 1988](http://zoo.cs.yale.edu/classes/cs426/2012/lab/bib/fidge88timestamps.pdf)（11 页）
- Mattern 1989：[Virtual Time and Global States](https://courses.csail.mit.edu/6.852/01/papers/Mattern.pdf)
- 视频：[Kleppmann Lecture 5](https://www.youtube.com/watch?v=4VfPcCZuVmI)（vector clock 与 causality）
- 后续：[[crdt-shapiro-2011]]；Dynamo 论文 §4.4 写冲突
