...
这三条加起来，让 PostgreSQL、MySQL InnoDB、SQL Server、DB2 都能在崩溃后几秒钟恢复一个 TB 级数据库。
...
- `pg_wal/` 目录里的段文件就是 ARIES 的 log，文件名是 LSN 的高位部分
...
- 崩溃恢复时 PostgreSQL 跑的就是 Analysis → Redo → Undo 三阶段（PG 用 redo-only 简化版，没显式 undo log，靠 MVCC 让旧版本天然存在）
...
- 分布式数据库的跨节点提交 —— ARIES 解的是单节点恢复，跨节点要叠 [[lamport-1978]] 的 logical clock + 2PC，再升级到 [[spanner]] / [[calvin-2012]]
...
- [[foundationdb-2021]] —— 用 ARIES 思路 + 分布式事务，但日志结构更接近 LSM
