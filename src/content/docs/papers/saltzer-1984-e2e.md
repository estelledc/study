...
这篇 1984 年发表于 ACM TOCS 的短文（约 12 页，pp.277–288），是后来 TCP/IP **"哑网络 + 智能端点"** 架构的合法性来源。
...
4. **底层做可靠性也可能合理**——但只能作为**性能优化**：高丢包链路上让 TCP 端到端重传，每次都要重发一整个文件代价太大；底层加 FEC（前向纠错）/链路重传是为了把端到端重传次数压低，不是为了"承担正确性"。
...
**不适用 / 边界**：
- 严苛实时场景（VoIP / 视频）——丢包重传不可接受，反而要把 FEC（前向纠错）推到底层
- 高丢包链路——纯端到端重传常比链路层重传贵一个数量级以上，底层必须出力
...
- **被引最多的"设计原则"类论文之一**：十来页的短文撑起了整个互联网的架构合法性
