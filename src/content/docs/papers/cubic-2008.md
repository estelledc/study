...
只要机器是 Linux（包括 Android、容器宿主、绝大多数云服务器），默认就是 CUBIC。Mac 默认是 NewReno，iOS 14 起也切到了类 CUBIC 的算法。
...
意思：从减窗那一刻起，需要约 3.68 秒重新爬回 100。爬的轨迹：

| 时间 t | W(t) ≈ |
|--------|--------|
| 0 秒   | 80（凹底）|
| 1 秒   | 80 + 0.4*(1−3.68)^3 ≈ 80 − 7.7 ≈ 87（接近 W_max）|
| 3.68 秒| 100 |
