...
- **被主流引擎接住**：vLLM / TensorRT-LLM 等已支持 Medusa 类多头投机，作为可选加速路径
...
Vicuna-7B 默认：5 个 head + 每 head top-5 + 树拼成约 64 条路径。逐步看一轮解码：

1. **读当前 hidden state**：大模型对已生成前缀做一次前向，得到位置 t 的表示
2. **多头并行猜**：5 个 head 各自给出未来位置的 top-k token，拼成候选树
3. **一次验证前向**：用树形 attention mask 并行算整棵树在大模型下的真实概率
4. **按前缀留下**：从根往叶找最长可接受前缀；平均约接受 **2.5 个 token**，错的截断后下一轮重猜

吞吐近似翻倍，关键是步骤 2–3 用一次（加一次验证）前向换多个 token。
...
- `model`：底座 Vicuna；`speculative_model`：预训好的 Medusa head 权重
- `num_speculative_tokens=5`：对应 K=5 个 head，每轮最多多猜 5 个位置
- `generate(...)`：业务侧仍只调一次生成 API，**不用改提示词或后处理**

### 案例 3：树形 attention 的 mask 是什么样

假设 head1 候选 {A, B}，head2 候选 {X, Y}。prompt 末尾记作 P，一次前向输入：
...
mask 让「跟随 A 的 X」只看见 P+A，「跟随 B 的 X」只看见 P+B。一次前向拿到各候选在大模型下的真实概率，再和 head 候选对照裁决。相对 [[specinfer-2023]]：Medusa 用多头代替一群 draft 小模型，部署一份权重，接受策略用 typical acceptance 换工程简单。
...
- **2024 年中**：vLLM / TensorRT-LLM 等接入，成为生产推理的常见选项之一
