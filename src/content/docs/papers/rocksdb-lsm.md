...
- 为什么 CockroachDB / TiKV / YugabyteDB / Kafka Streams 底下都是 RocksDB（Cassandra 是自研 LSM，不是 RocksDB）
...
1. **put**：先写进 `memtable`（内存字典），不碰磁盘——对应「先丢进大筐」。
2. **满了就 `_flush`**：把整表按 key 排序后追加成一个 SST，再清空 memtable——对应「整筐装车」。
3. **get**：先查 memtable，再从**最新** SST 往旧扫——同一 key 可能有多版本，新的覆盖旧的。读慢就慢在「可能要扫多个文件」。
