...
- 为什么 **KLEE / 符号执行器**能解混合位向量 + 算术的路径条件 —— 底层 SMT 在跑 NO 式协作
...
> 只要两 theory 的**签名不相交**（不共享函数符号）+ 都是 **stably-infinite**（有解就能找到任意大的模型，像"旅馆永远加得开新房间"）+ 都是 **凸的（convex）**，那只交换共享变量间的**等式**就足以让组合算法完备。

凸的日常说法：如果理论只告诉你"x 等于 y₁ 或 y₂ 或…"这种**或**，它其实已经能推出其中**某一个**具体等式——不会永远停在模糊的"或"。线性算术、未解释函数是凸的；位向量、非线性算术不是 → 现代 SMT 要做 case split 补救。
...
- **Dafny / F\* 验证 `arr[i] + 1 > arr[i]`**：array.read（数组 theory） + 整数算术 → SMT 求解
- **KLEE / SymCC 符号执行**：路径条件混合 BV + LIA → STP/Z3
- **Boogie / VCC 等中间层**：把程序验证条件翻成 SMT-LIB，交给 Z3/CVC5
- **编译器等价性检查**（部分 LLVM/Alive2 流程）：算术化简是否合法 → SMT

注意：**`cargo-audit` 查 CVE 靠的是 RustSec 顾问库，不调 Z3**；日常 `rust-analyzer` / `clippy` 也不跑 NO。只有你显式接 SMT（验证器、符号执行、证明助手）时，背后才是这套协议。
...
- **2022**：CVC5 发布，继续把 DPLL(T) + theory combination（NO 后裔）工程化；SMT-LIB 交换格式本身更早，但多 theory 协作仍以这类协议为默认假设。
