...
- 为什么 2023-2024 端侧 LLM 社区常把 RWKV 当候选——没有 KV cache，1.5B 模型在树莓派上也能跑长对话
...
任务：在手机或树莓派上跑 1.5B 模型做长对话。分三步看差别：

1. **装模型**：两边都把权重放进内存（约几百 MB 量级）
2. **对话变长**：Transformer 每多一轮就要多存一份 KV cache，上下文越长占用越大；RWKV 每层只维护几个固定向量（state），对话再长也不涨
3. **结论**：长对话场景下，RWKV 的常驻显存近似「权重本身」，这是 2023-2024 端侧落地窗口；社区另一条主线是 rwkv.cpp（对照 Llama.cpp）

### 案例 2：流式语音 / 翻译生成

实时语音边听边翻，按 RNN 形态跟四步：

1. **存住**上一步的 state（固定大小）
2. **来一个**新 token（音频/文本片）
3. **更新** state（O(1) 读写）
4. **吐出**当前输出

Transformer 做 streaming 要分 chunk、配滑窗、决定多久重算 attention；RWKV 延迟稳定，代码就是「state 进 → state 出」。
