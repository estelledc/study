...
### 案例 2：TC-Strong 写者等寿命（对比 TC-Weak）

```
时刻 t=100：SM-A 读 X，line 寿命 = 200
时刻 t=150：SM-B 读 X，line 寿命 = 250
时刻 t=180：SM-C 想写 X（TC-Strong）
  → SM-C 等到 t=250（最长寿命），再写 L2，不发任何失效消息
时刻 t=251：SM-A / SM-B 再访问 X
  → 自检发现寿命过期，重新拉新值
```

TC-Strong：**零失效消息**，但写者可能长 stall。TC-Weak 反过来——写不等寿命，只在程序显式 fence / atomic 时用时戳对齐，把同步成本推到软件边界（论文主推的实用版）。
