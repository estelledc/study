...
DDIM 是当今扩散图像/视频生成加速采样器的祖师爷——Stable Diffusion 早期默认的 PLMS/PNDM，以及后来常见的 DPM-Solver、Heun 等，多属 DDIM / 概率流 ODE 这一脉派生。
...
1. **训练目标只看边缘分布，不看路径**。边缘分布可以想成「只问某一层楼梯长什么样，不问你怎么爬上来」。[[ddpm]] 训练让神经网络学 `q(x_t | x_0)` 这个边缘上的去噪。**它并不强制反向过程必须 Markov**——只要边缘对，反向怎么走都行。这是整篇文章的钥匙。
...
CIFAR-10 上 DDPM 1000 步 FID 约 3.17。DDIM（η=0）50 步 FID 约 4.67，100 步约 4.16，1000 步约 4.04，**少步数几乎追平**。CelebA 64×64 上 DDIM 100 步 FID 约 6.53，1000 步 DDPM 约 3.26。代价小、收益大，是 2020 年扩散模型上线生产的关键一步。
