...
**意义**：在 [[ranknet-2005]] 用神经网络做 pairwise 排序的两年后，GBRank 在 Yahoo 内部数据上**全面胜过 RankNet 和 RankSVM**——这是 2007 年"GBDT 在 IR 实战胜过神经网络"的早期重要证据，也直接催生了 2010 年拿下 Yahoo LTR 挑战赛冠军的 [[lambdarank-2006]] 的 GBDT 版（LambdaMART）。
...
2. **偏好对损失**：对一对 (i 优于 j)，定义带 margin 的 squared hinge：

   ```
   L_ij = (1/2) * max(0, tau - (s_i - s_j))^2
   ```

   tau 是 margin（论文取 0 或 1）。当 `s_i - s_j >= tau` 时损失为 0——已经分得够开，不动了；只对**违反 margin 的 pair** 算梯度。
...
注意第 3 步的设计：传统 boosting 让弱学习器拟合 loss 的负梯度（function-space gradient descent，Friedman 2001 GBM 的核心）；GBRank 在这里做了一个工程上的简化——直接给出"该修到哪"的目标值，让回归树去拟合。这种"绕开链式法则、直接写回归靶点"的思路，和后来 [[lambdarank-2006]] "跳过定义损失、直接写梯度"的做法精神相通。
