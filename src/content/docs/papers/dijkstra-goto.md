...
- 给 [hoare-logic](/papers/hoare-logic/) 公理化语义提供了讨论对象——Hoare 1969 给三件套写了推理规则；
...
Dijkstra 当时是 [algol-60](/papers/algol-60/) 委员会成员，刚做完 THE 多道操作系统，正在思考程序怎样可证明。letter 比 NATO Garmisch 软件危机会议早 7 个月，恰好踩在「software crisis」这个词被命名的前夜。

[knuth-taocp](/papers/knuth-taocp/) 作者 Knuth 1974 写了 40 页平衡论 "Structured Programming with go to Statements"，列举多重循环退出、错误清理、状态机三类合理 goto 场景，态度是「教条不要走极端」，不是反对 Dijkstra。
...
- Hoare 1969 公理化语义：见 [hoare-logic](/papers/hoare-logic/)，给三件套配推理规则；
...
- 数学根基：[turing-1936](/papers/turing-1936/) 给「可计算」奠基，三件套是它在高级语言里的投影；
- 同代家族：[algol-60](/papers/algol-60/) 是 Dijkstra 关心的语言，第一次有 if-then-else 但也仍允许 goto；
- 推理工具：[hoare-logic](/papers/hoare-logic/) 把三件套的可推理性形式化为推理规则；
- 工程参照：[knuth-taocp](/papers/knuth-taocp/) 作者后来写 Structured Programming with go to Statements 做平衡；
- 句式回声：后续 Pointers Considered Harmful、Null References Considered Harmful 一脉，都借这个句式。

## 反向链接

- 任何讨论控制流、结构化编程、形式化证明的笔记都会回到这里；
- 评审 C 代码时遇到 goto，先问「向前还是向后跳？目标是不是 cleanup label？」再决定要不要反对；
- 学一门新语言时把它的控制流构造（顺序/选择/循环/异常/并发）列出来，能立刻看出它和这条 1968 年线索的距离。

---

回到一句话：goto 不是因为难写才有害，是因为它让代码不再是「读到哪里 = 知道状态」的可推理对象。

这个洞察比「禁用 goto」这条工程禁令重要得多，也是 Dijkstra 留下的真正资产。

下次再读一篇「X Considered Harmful」的文章时，先把作者的层次、对手的具体场景、现代工具的调和方案三件事拆开，再判断要不要照单全收。
