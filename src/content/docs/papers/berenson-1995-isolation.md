...
也就是今天 PostgreSQL 默认 Repeatable Read、Oracle 默认隔离机制背后的核心思想。

## 为什么重要

不读这篇论文，下面这些现象说不清楚：

- 为什么 PostgreSQL 的 Repeatable Read 其实是快照隔离而不是锁实现
- 为什么 Oracle 声称的 SERIALIZABLE 其实只是 Snapshot Isolation，遇到写偏斜不回滚
...
**案例 1：写偏斜导致数据不一致。**
银行有两个账户 x=50、y=50，约束 x+y≥0。
事务 T1 检查 x+y=100，决定从 x 扣 60（写 x=−10）；
事务 T2 检查 x+y=100，决定从 y 扣 60（写 y=−10）。
在 Snapshot Isolation 下两个事务各自看到旧快照，
T1 写 x、T2 写 y，写集合不重叠，都提交成功——
结果 x+y=−20，约束被打破。
这就是 A5B 写偏斜，在真正的 Serializable 下不会发生。

**案例 2：丢失更新。**
用户 A 读到库存=10，下单减 1 写回 9；
用户 B 同时读到库存=10，下单减 1 也写回 9。
库存应该是 8 但变成了 9——少扣了一单。
这就是 P4 丢失更新。
Snapshot Isolation 用 first-committer-wins 能防住这个，
但 Read Committed 级别下如果不加应用层锁就会中招。

**案例 3：PostgreSQL 的 Repeatable Read 名不副实。**
你在 PostgreSQL 里设 `SET TRANSACTION ISOLATION LEVEL REPEATABLE READ`，
实际跑的是 Snapshot Isolation。
大多数场景没问题，但写偏斜场景下你以为安全却不安全。
PostgreSQL 9.1 之后提供了真正的 `SERIALIZABLE`（基于 SSI 算法），
需要显式设置才能用。
