...
来源: 'Anonymous, "MMSkills: Towards Multimodal Skills for General Visual Agents", arXiv 2026'
日期: 2026-06-01
分类: agents
难度: 中级
---
...
更直白点：以前的 skill 库像"只写菜谱不放图"，新人按文字试了 5 次都做不出蛋炒饭；MMSkills 是"菜谱 + 步骤照片 + 关键技巧三件套"，新手第一次就能做到 80 分。
...
### 案例 1：在 GUI 任务里"已经会"和"完全没见过"的差距

```text
任务：在购物 app 里把第二件商品加入购物车
没有 MMSkills：
  agent 先盯主页 5s → 误点 banner → 退回 → 再误点搜索框 → 8 步成功
有 MMSkills（已存"商品列表 → 点击商品 → 加入购物车"卡片）：
  state card 命中 → 直接 3 步成功
```

加快的不是模型本身的视觉能力，而是**模型不再每次都重新发现"加入购物车"这条动作链**。
...
这种卡片不止帮当前 agent，还能**导出给同游戏其他 agent 用**——人类玩家社区的 wiki 攻略本质上做的就是这件事，MMSkills 把它自动化了。

### 案例 3：和纯文字 skill 库的对比

| 场景 | 纯文字 skill 库 | MMSkills |
|---|---|---|
| 网页表单填空 | 凑合 | 一样 |
| 找一个图标 | 经常找不到 | 有 keyframe 直接定位 |
| 玩 RPG 战斗 | 几乎不可用 | 可用 |
| 跨主题色重做 | 直接失败 | keyframe + 文字双通道兜底 |

文字库在"语义清晰"的任务里没问题；一旦目标本质上是视觉的（图标位置、UI 布局、敌人血条），文字描述就会信息丢失。**MMSkills 的真正价值是让 agent 在"视觉问题"上也能像在纯文本任务里那样积累经验**。
