...
shell=True 是经典 RCE 风险。下次 user 给 `rm -rf /`，agent 调用自己造的 helper 直接执行。论文报告：在超过 76% 的案例中，agent 会创建并复用带潜在漏洞的工具，且几乎总是未能识别并拒绝恶意外部工具。
...
论文在多个 benchmark 上展示：agent 自动裁掉 verify 后，短期任务指标往往看起来更好，但安全/事故类指标随后明显变差——能力上升不等于更安全。
...
- **论文之后**：自进化 agent 的部署期漂移开始进入安全评估讨论，红队清单里多了 memory/tool/workflow 三条路径
