...
- 为什么后续 LightRAG / nano-graphrag 等变种都跟着做图检索——GraphRAG 把"图 + RAG"这条路走通了（多查询融合如 RAG-Fusion 是另一条线，别混成"都在做图"）
- 为什么 Microsoft 开源后 GitHub 拿到 30k+ Star、社区一年内出了几十个变种
- 为什么企业知识库 / 全库问答开始认真考虑图结构——它让 LLM 看到的是**概念关系**而不是**碎片文本**（产品是否上图因场景而异，不是全家默认）

## 核心要点

GraphRAG 流程拆成 **三步**（对应论文 From Local to Global 的 indexing → local/global query）：
...
三步加起来：把 retrieval 从"找一段相似文本"升级成"对整库做主题级摘要"。Local 吃图邻域，Global 吃社区摘要——同一张图，两种问法。
...
1. **索引贵**：1M tokens corpus 索引一次约 $30-50（GPT-4 turbo 价位）。每个 chunk 都要喂 LLM 抽实体——总调用次数 ≈ chunk 数 × 2（base + 二次询问"还有遗漏吗"）。临时一次性查询完全划不来；先用小样本估成本再全量跑。
...
> GraphRAG = LLM 抽实体建图 + 社区摘要 + Local/Global 两路检索。贵在索引、强在全局题；小语料或高频更新别硬上。

选它之前先问：你的问题是局部事实还是全局主题？后者才值得付索引账单。

## 延伸阅读

- 论文 27 页 PDF：[arXiv 2404.16130](https://arxiv.org/abs/2404.16130)（核心是 Figure 1 + Section 2 pipeline + Section 4 评测）
- 官方实现：[microsoft/graphrag](https://github.com/microsoft/graphrag)（30k+ Star，Python 包发到 PyPI）
...
- [[react]] —— ReAct（Yao et al.）：agentic 路线，运行时动态 retrieve，对照 GraphRAG 的预编译图
