...
- **1976 年**：IBM System R 已经把 2PL、锁粒度、WAL 跑在原型机上，Gray 是核心成员
- **1977 年**：Bayer 在慕尼黑办操作系统暑期学校，请 Gray 来讲数据库专题
- **1978 年**：讲义整理成 130 页，先发为 IBM RJ2188 报告，然后收入 Springer LNCS 60
- **1981 年**：Gray 在 VLDB 把"事务"升华成通用抽象（[[gray-1981-transaction]]）
- **1992 年**：Mohan 把 1978 的 DO/UNDO/REDO 工程化为 [[aries-1992]]，至今是 WAL 教科书

## 学到什么

1. **事务是数据库的"核心数据类型"**——所有内核功能（并发、恢复、分布式）都围绕它转
2. **并发 = 锁 + 协议（2PL）**；**恢复 = 日志 + 重放/回滚（WAL）**；**分布式 = 多机协议（2PC）**——三条主线
3. **粒度选择是性能折中**：粗粒度省开销少并发；细粒度多并发多开销
4. **2PC 的阻塞是设计代价**：原子性 vs 可用性的根本张力，46 年没变
5. **教学讲义反而成了开山之作**：Gray 没把它当论文写，反而给后人留了一份最完整的全景图
