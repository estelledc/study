...
加在一起叫 **tabulation 算法**，最坏 O(E·D^3)。
...
IFDS 把"被污染的变量"当作 D 的元素：

- `read_user_input()` → 给 data 这条事实建边
- `clean(data)` → 把 data 这条事实切断（流函数返回空集）
- 在 sink 处查图：是否有路径到 (sink, data)

**Heros / Phasar 直接用 IFDS 当 taint 引擎**——把"source/sanitizer/sink 三类节点"喂进去就出报告。
...
你想分析"y 在某点是常量几"。问题：流函数**不分配**——`y = x` 的输入是"x = 5"事实，输出是"y = 5"事实，不能拆成"对每个事实独立处理"。

IFDS 要求**分配**（distributive）。这种情况要扩到 **IDE**（Interprocedural Distributive Environment，Sagiv-Reps-Horwitz 1996），让边带权值（micro-functions），代价是更复杂但还是多项式。
...
- 想要"精确（MOP-on-realizable-paths）"而非"安全近似"
...
2. **summary edges 是上下文敏感的关键**——不靠枚举调用栈，靠"算一次缓存复用"
3. **分配性是 IFDS 的硬约束**——它换来了多项式复杂度和精确性；扩展到 IDE 要付出额外复杂度
4. **理论 → 框架 → 工具链**：从 1995 论文到 2010s 工业静态分析平台，IFDS 走通了从公式到产品的全链路
5. **精确解 vs 安全近似**：跨过程时强制 call/return 配对，把"理论上可走但实际不通的路径"剔除，比传统数据流的不动点解法更精确
