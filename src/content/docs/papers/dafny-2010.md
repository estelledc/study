...
链表反转里，你想证"反转后元素集合不变但顺序倒过来"。可以引入一个 ghost 序列 `s`，每次操作同步更新它，最后断言 `result.contents == reverse(s)`。运行时 `s` 不存在，但证明阶段它给了 Z3 抓手。

## 踩过的坑

1. **不变量要程序员写出来**。Dafny 不是"全自动证一切"，它需要你写出**正确强度**的循环不变量。太弱证不出后置，太强自己保持不住——这一步是真功夫，写对了证明几秒过，写歪了 Z3 跑半天超时。
2. **Z3 时间不稳定**。同一份代码今天 2 秒、明天 30 秒——SMT 求解器对公式形式敏感。社区有套"老司机经验"：少用乘法、多用 `seq` 而不是数组、断言切小步。
3. **错误反例难读**。Z3 给的反例是赋值表（x = 7, a = [3, 1, ...]），看不出"是哪条不变量第一次没成立"。新人常被淹没。
4. **frame 条件容易漏**。方法 `modifies this.x` 但实际改了 `this.y`——Dafny 拒绝。新人会觉得"我明明只是改了相邻字段"，但验证器要的是显式声明。
5. **学习曲线分三段**：先适应前后置条件（1 周）→ 再适应循环不变量（2 周）→ 最后是 frame 条件 + 数据精化（持续打磨）。
6. **并发支持弱**。论文 2010 版本是纯顺序语言，IronFleet 用的是上层"协议层 reasoning"，不是 Dafny 直接验并发。Dafny 4 才开始扩。
...
- **2022 年**：Dafny 完整开源（dafny-lang/dafny），脱离 Microsoft 内部工具身份。
