...
Magic3D 是 NVIDIA 在 [[dreamfusion-2022]] 之上做的工业级升级。同样是"一句文本生一个 3D 物体"，DreamFusion 大约 1.5 小时一只低分辨率 NeRF，Magic3D **40 分钟跑完一个高分辨率带纹理 mesh**——快两倍、清晰得多。
...
**3D 表示切换**：把 coarse hash grid 通过 marching cubes 转成 DMTet（Deep Marching Tetrahedra，可微分四面体网格）。DMTet 的关键性质是**形变和纹理都能反向传播梯度**，所以可以继续优化。
...
# Stage 2: Fine
mesh = marching_cubes(hash_grid)   # 拓扑固定下来
mesh = DMTet.from_mesh(mesh)       # 可微分版本
...
注意 Stage 2 里 SDS 是在**潜空间**算的，但梯度通过 VAE encoder 一路传回 RGB 图、再传回 mesh 顶点。
...
2. **Stage 1→2 转换的"质量断崖"**：marching cubes 提取 mesh 时如果 hash grid 还没收敛，会出现破洞、悬挂三角形。论文用了一个 occupancy 阈值 + 联通分量过滤。
