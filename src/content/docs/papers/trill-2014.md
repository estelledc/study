...
- 为什么 Trill 相对同期商用流引擎（如 StreamInsight 一类）吞吐能高 **2-4 个数量级**，并远高于 Storm 等（论文实测）
...
2. **可调 punctuation（标点）策略**：punctuation 是流里的"水位线"——告诉系统"在我之前的事件都到齐了，可以出结果"。Trill 让你自己调标点的频率：调密 → 延迟可压得很低但吞吐低；调疏 → 吞吐高（百万条/秒）但延迟高。**同一份 query，调旋钮就能在两端之间滑动**。这就是论文标题里"diverse analytics"的来源——一个引擎覆盖几个数量级的延迟谱。
...
1. `ToStreamable(...)`：把数据源变成 Trill 能吃的流；乱序策略与标点策略在这里定
2. `punctuationPolicy`：流模式每 500ms 发一次标点催结果；批模式 `None()` 表示攒齐再一次性 flush
3. `Where` / `HoppingWindow` / `Sum`：业务 query 两边一字不改——切换只动 ingress 那一行

这是工业界用产品级方式实现"批是流的特例"：一份测试覆盖两种 deployment。

### 案例 2：列式批让你看见 CPU 在干什么

传统流处理（每条事件一个对象）：CPU cache miss 高，分发开销大，每条事件跑 200-500 ns。Trill 改成三步：

```text
// 教学示意：一批点击事件的列布局
timestamps: [10, 11, 12, 13, ...]   // long[]
values:     [3.0, 120.0, 8.0, ...]  // double[]
bitvector:  [1, 1, 1, 1, ...]       // 1=保留, 0=被 filter 掉
```

1. **攒批**：约 80000 条打成一个 `Batch`，字段各成一列（连续数组）
2. **列循环**：`Where(e => e.value > 100)` 退化成扫 `values[]` 的紧凑循环，编译器可 SIMD
3. **bitvector 跳过**：不真删行，只把不合格位置标 0；下游 mask-skip，省内存复制

每条事件摊到约 **5-10 ns**。这是把 [[cstore-2005]]、[[monetdb-x100-2005]] 的列存招式搬到流上。

### 案例 3：渐进式交互（progressive）

用户在 BI 里拖 7 天销售曲线。传统做法：每拖一次重跑全部聚合。Trill 走增量三步：

```text
// 教学示意：窗口左边界滑出一条旧事件
insert:  (sale=50,  +1)   // 右边新进来
retract: (sale=30,  -1)   // 左边滑出去
// 下游聚合只做 +50-30，不重扫整窗
```

1. **窗口滑动**：只看左边界滑出、右边界滑入的增量，中间未变区间复用
2. **发 retract**：事件"过期"时不删，而是发 `(payload, -1)` 让下游做减法
3. **只算 delta**：聚合看见 `+1, +1, -1, +1` 累加成 +2——前端每帧看到结果在精化

retraction 不是 Trill 独创（见 [[self-adjusting]]），Trill 把它工程化进了流引擎。
...
- JVM 生态（→ Flink）
- 超低延迟（远低于毫秒）—— Trill 在 ms 量级最甜，再低就要专用硬件
...
- **2008 年**：Microsoft Research 启动 CEDR 项目，奠定"事件 = 时间区间元组"语义
- **2010 年**：CEDR 产品化为 StreamInsight——能跑但单线程性能瓶颈大
- **2014 年**：Trill 用 columnar batch + 可调 punctuation + 代码生成，吞吐比前辈高 2-4 个数量级；PVLDB 2014，VLDB 2015 见刊
- **2017 年**：开源到 github.com/microsoft/Trill，成为 .NET 流处理事实标准
- **2020 年代**：Materialize、Risingwave 等流式数据库多借鉴"批是流的特例 + 增量物化"
...
4. **嵌入式库 vs server 的取舍**——库简单部署但吞吐有上限；Trill 选前者并做到极致
5. **API 通用性 ≠ 引擎通用性**——引擎能在几个数量级延迟谱上滑动才算真"diverse"
...
- [[cstore-2005]] —— 列存 OLAP 的另一支祖先，启发了 Trill 的 batch 设计
