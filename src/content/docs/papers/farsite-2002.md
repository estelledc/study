...
- 把文件**剪碎、加密、撒到很多人手里**，少一两份还能拼回来
- 一份文件想被算"权威版本"，得**三分之二的人同时点头**才作数（拜占庭多数）
...
- **拜占庭容错（BFT）+ 加密分片**第一次在一个真系统里同时落地，不是论文里的玩具
...
但副本不是裸文件，而是**用文件 owner 的密钥加密 + 签名后**的版本。任何一台机器返回数据时：
...
1. 客户端在本地写好内容
2. 用 owner 密钥**加密 + 签名** hello.txt（密钥来自内容哈希，做 convergent encryption）
