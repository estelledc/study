...
Snowflake 2016 是一篇 **SIGMOD 工业 track 论文**，把一个 2014 上线的云数仓产品的内部架构第一次完整公开。它的核心主张：**传统 shared-nothing 数仓在云上不够灵活，把"存储"和"计算"完全解耦才是云原生的形态**。

日常类比：以前 KTV 的话筒、点歌机、音箱是绑死的整套；这篇论文证明你可以把"歌库放云端 + 自己带话筒进包间 + 用完话筒就退还"——而且**有 600+ 真实付费客户跑得起来**。
...
3. **Cloud Services 层 = 中央调度室**：管表 schema、micro-partition 列表、事务版本号、查询优化、权限。基于 FoundationDB 多副本。类比：图书馆总台和借阅卡系统——书在仓库、工人临时雇，但谁能借哪本书全在总台记一笔。
...
三个 VW 跑在不同 EC2 集群，**互不抢资源**，但读同一份 S3 数据。论文 §4.1 把这个特性叫 "multi-cluster shared data"——shared-nothing 数仓里要么塞死要么得复制三份。
...
实现：每次写入产生新 micro-partition + 新 metadata version，老版本不删。读查询固定一个 version，看到的是一致快照。代价：老 micro-partition 占 S3 空间（默认保留 1 天）。论文 §4.2 把这归到 "continuous availability" 一组特性里。
...
4. **Cloud Services 是元数据单点**：FoundationDB 多副本但跨 region 故障时所有 VW 一起瘫。2020 年 us-east-1 4 小时事故公开 RCA 把 metadata service 列为根因——这是论文里"集中状态"假设的实证代价。
...
- **2014 年**：产品 GA。第一个大客户来自广告分析行业，最痛苦于"加节点要停机一周"。
- **2016 年**：本论文发在 SIGMOD 工业 track，12 页，公开三层架构。同年 Aurora 论文也发表，两篇并称为"云原生数据库奠基"。
- **2020 年**：Snowflake IPO，首日市值 $700 亿，证明"存算分离 + 按秒计费"是可行的商业模式。
