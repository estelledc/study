...
- **谁发明的有点反常识**：以前优化器都是数学家手推（SGD / Momentum / Adam / AdamW 都是）。Lion 是**机器在 1 万小时算力里搜出来的**，开源后大家发现"它居然真的好"。
...
整个过程吃了 1 万 + GPU 小时，但产物是一个**人类几乎写不出来**的怪公式：beta1 ≠ beta2、用 sign、没 epsilon——每一步都和已有优化器不一样。
...
3. **单一指标会骗你**。Lion 在 ImageNet / 预训练上赢，**但在小 batch / 微调上输**。换基线前必须**在你自己的工作流上跑一次**。
