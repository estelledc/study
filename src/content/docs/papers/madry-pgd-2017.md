...
PGD 对抗训练是一套**让神经网络学会对抗最坏情况输入**的训练框架。日常类比：就像驾校考官故意在路考中安排最难的场景——绕桩、坡道、倒车入库——只有把这些最难情况都应付过去，才算真正会开车。

Madry 等人把"最坏情况"数学化为一个 **min-max 鞍点问题**：

```
min_θ  E[(x,y)~D][ max_{δ∈S} L(θ, x+δ, y) ]
```

外层 `min` 是模型训练（找最优参数），内层 `max` 是攻击者（在扰动集 S 内寻找让损失最大的扰动 δ）。FGSM 只走一步梯度，PGD 走多步，每步之后把 δ **投影回**扰动球内——因此称为"投影梯度下降"。
