...
1. **broker 重写成只追加文件**：传统系统把每条消息当独立可 ack/delete/TTL 的实体，broker 维护 per-message 状态。Kafka 反过来——partition 是一组顺序 segment 文件（如 `00000000.log`），写入是 O(1) 的 append，**不按单条消息原地改/删**（过期整段丢掉）。**类比**：传统是 1 万张索引卡的档案柜，Kafka 是只能贴纸条的电报纸带。

2. **offset 是 consumer 的状态，不是 broker 的**：传统 broker 必须记"谁读到哪、谁 ack 了"——这是复杂度根源。Kafka 把 offset（游标）还给客户端，broker 只暴露 `fetch(topic, partition, offset, max_bytes)`。**broker 对"谁读了什么"完全无知**——所以才能轻、快、多消费者扇出。

3. **page cache + sendfile 做零拷贝扇出**：Kafka 不维护应用层缓存，把"缓存什么"交给 Linux page cache（OS 记最近读过的磁盘页）。读时用 `sendfile(2)` 把内核页直接 DMA 到网卡——**不进用户态、不复制**。多个 consumer 订同一 topic 只需一份内存。
...
LinkedIn 需求：埋点发 `page_view`，离线推荐和在线 dashboard 都要同一份数据。下面是**示意伪代码**（对应论文 Figure 2 扇出模型）：

```python
# 示意：生产端埋点
producer.send("page_view", key=user_id, value=b'{"url":"/feed"}')

# 示意：两个独立消费组（group_id 在构造参数里）
...
1. **生产只追加**：埋点不管谁读，对应论文"broker 当日志文件"
2. **两个 group**：各自持 offset，互不抢——对应论文多订阅者扇出
3. **一份磁盘数据**：被读 N 次，broker 状态不随消费者数膨胀

### 案例 2：CDC（论文扇出模型的后世落地）

Debezium 等工具把 MySQL/Postgres binlog 写进 Kafka——**这不是 2011 原文实验**，而是同一套"一份日志、N 个下游"哲学的后世落地：

```yaml
# 示意：Debezium connector 配置
source: mysql
...
1. **抓变更流水**：connector 读 binlog，转成 Kafka 消息
2. **按表进 topic**：下游按需订阅
3. **可重放**：默认约 7 天 retention，迟到的消费者仍能追上
...
1. **同一 offset**：磁盘上只存一次
2. **三个游标**：快慢消费者各走各的
3. **成本恒定**：传统 broker 每多订阅者多一份状态；Kafka 只多几次 `sendfile`
