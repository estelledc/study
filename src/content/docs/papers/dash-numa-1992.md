...
- **GPT-3 训练用的 A100 集群、Grace-Hopper 共享 HBM、NVLink mesh** —— 跨设备 cache coherence **思想一脉**（directory / home agent），不是同一套实现照搬
...
2. **Directory 存什么**：每条 16 字节 memory line 在 home cluster 有 1 条记录 = **状态 2 bit + sharer bitmap 64 bit**。状态只有三种：没人缓存 / 多家共享只读 / 一家独占可写。日常说法：这张**前台登记表**记的就是"此刻谁手里有副本、能不能写"。
...
**适用**：
- 多 socket 服务器的硬件一致性（Epyc / Xeon / Power，常见 **2–8 socket**）—— DASH 是它们的拓扑祖先
- 跨 chiplet / 多 tile 的片上 NUMA（现代 CPU 把 DASH 树又加深一层）
...
- [[moesi-cache-coherence-1986]] —— 总线/目录之前的经典 cache 状态机，DASH cluster 内 snoop 的近亲
- [[gpu-cache-coherence-2013]] —— GPU 侧用时戳等手段做一致性，对照 directory 路线
- [[hydra-1974]] —— 更早的多处理器系统软件实验，对照"硬件一致性何时值得做"
