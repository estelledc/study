...
- 为什么 2024 年 CRAG / Adaptive-RAG 等 agentic RAG 都强调「按需查 + 查完再批判」——它们接的是 Self-RAG 把检索决策嵌进生成的思路（FLARE 更早，是 2023-04 的前驱，还没有 reflection token）
- 为什么 LangGraph / LlamaIndex 的"自反思 RAG"模板默认带"判断要不要查"这一步——这步是 Self-RAG 推成主流工程模板的
- 为什么 retrieval 这条线上「模型自己决定何时调检索器」成了标配样本——Self-RAG 用 reflection token 把它训进词表
...
|---|---|---|---|
| Llama2-chat-13B（无检索） | 20.0 | 55.9 | 38.4 |
| Ret-ChatGPT / ChatGPT | 50.8（Ret） | 71.8（ChatGPT） | 75.3（Ret） |
| **Self-RAG-13B** | **55.8** | **80.2** | **73.1** |

Self-RAG-13B 在 PopQA / FactScore 上超过检索增强的 ChatGPT 基线；ARC 上 Ret-ChatGPT 略高，但 13B 开源模型已接近。FactScore 比无检索 ChatGPT 高约 8 分。
...
### 案例 4：长文里「逐段 IsSup」三步

写传记类长文时不只开头查一次，而是每写一段都自检：

1. **写「出生地」** → Retrieve=Yes，拉 wiki 段，IsSup=Fully → 保留
2. **写「获奖经历」** → 再 Retrieve=Yes，IsSup=Partially → 标「待校核」
3. **写「个人评价」** → Retrieve=No（主观不必查）→ 直接写

最终输出能逐段标「有据 / 推测」，学术、法律、医疗长文特别有用。
...
3. **Retrieve 校准不准**：模型有时候该查不查（aleatoric 题——答案本身有随机性，却被误判成"常识题不用查"）、不该查乱查（"今天心情不好"也去查文档库）。本质是 GPT-4 标签和真实分布有 gap。
...
1. **把"控制信号"嵌进生成本身**——reflection token 是 retrieval 线上最早把「要不要查 / 支不支持」训进词表的成功样本。后来 o1 的 thinking token、extended thinking 也是「生成里带控制/推理痕迹」，可作对照，但机制与训练目标不同
2. **"按需"比"总是"经常更划算**——vanilla RAG 强制查反而引入噪声；让模型自己分流是更精细的工程取舍
3. **critic 蒸馏**（先用强模型当阅卷老师打标签，再把能力蒸进小模型）是做"自我评估"的标准路径
4. **agentic 不是必须有 multi-agent**——一个 LLM 通过 reflection token 也能做出「查不查、用不用」的 agent 行为
