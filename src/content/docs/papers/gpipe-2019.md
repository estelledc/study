...
L 层模型从"单卡装不下"瞬间变成"K 张卡每张装 L/K 层"，激活值再用重算（re-materialization）压一遍，6B 参数的 Transformer 就能在 16 张加速器上训起来。
...
- 网络切成 **K=4 段**（论文原文：divided into 4 partitions），分到 4 块加速器
- micro-batch M=32（M/K=8，已高于 M≥4K 的 bubble 阈值）
- 重算大约占 step time 的 ~20–23%，但**模型大了约 6.6 倍**——拿算力换准确率的典型一击
