...
- 为什么 ChatGPT / Pinecone / Milvus 这种 RAG 系统能在十亿级向量库上做到亚秒检索
- 为什么微软 Bing 搜索的语义检索能在单机服务百万 QPS——它的底层就是 DiskANN
- 为什么 HNSW（2018）那么火却没解决"超大规模"——HNSW 必须全内存，1B 点要 400GB 内存
- 为什么 2023 年后向量数据库赛道里"SSD 索引"成了关键词——DiskANN 验证了 SSD 路径可行
...
```python
def search(query, L=100):  # L 是候选集大小
    visited = set()
    candidates = [start_node]  # 固定起点
    while True:
        # 用内存里的 PQ 压缩向量找最近未访问的候选
        next_node = min(candidates - visited, key=lambda n: pq_distance(query, n))
        if next_node is None: break
        # SSD 读 4KB：拿到 next_node 的全精度向量 + 邻居表
        page = ssd_read(next_node)
        candidates.update(page.neighbors)  # 把邻居加进候选
...
5-10 次 SSD 读 = 5-10ms 延迟。

### 案例 2：实测数据（SIFT-1B 数据集）

- 数据：10 亿条 128 维向量（SIFT 特征，约 480GB 原始）
- 机器：单台 64GB 内存 + NVMe SSD
- 结果：召回率 95%@1，p99 延迟 5ms，单线程 200 QPS

对比 FAISS（之前最强的 SSD-ANN）：
...
- [[product-quantization-2011]] —— PQ 压缩，DiskANN 内存层的关键
