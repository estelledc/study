...
- 为什么现代 OLAP 引擎都在抄它的"先估空间再选算法"的元决策思路
...
优化器先数 subgraph：星型图（事实在中心）的连通子图数 ≈ 2^50 / 一些剪枝 = 几十万。**超过 10000 预算**，跳过精确 DP，走 IKKBZ 线性化 + GOO 路径，毫秒级出近最优计划。重点：**不会**因为表数过 12 就硬切到 GA，决策由查询图拓扑驱动。
