...
（用第 k 个素数的「该位置符号编号」次方编码；这里 1、5、7 是符号码，不是位置下标）
...
3. **滥用哥德尔反对 AI**：从 1961 Lucas 到 1989 Penrose，反复有人说"哥德尔证明人脑超越机器"。这个论证逻辑上有缺陷（人脑也未必是一致的形式系统）、经验上也不等于工程障碍（LLM 的瓶颈是数据和对齐，不是哥德尔屏障）。主流学界认为该论证站不住，但不能把它理解成"哥德尔与 AI 无关"。
