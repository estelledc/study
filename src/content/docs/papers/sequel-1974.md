...
2. **关系演算的"英文翻译"**：Codd 1970 的关系代数有 σ（选择）、π（投影）、⋈（连接）等符号。SEQUEL 把每个符号映射到英文词：σ → WHERE、π → SELECT、⋈ → 多表 FROM。**背后仍是数学**，只是皮肤换成了英语。
...
- **1974 年**：Chamberlin 和 Boyce 发表 SEQUEL 论文，把关系演算"翻译"成英语关键词。同年 Boyce 32 岁早逝，他和 Codd 合作的 BCNF 范式也以他名字命名。
