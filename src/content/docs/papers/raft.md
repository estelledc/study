...
更关键的：完整正文 9 页 + 算法伪代码 50 行就讲完了。复现门槛低到工程师周末能写一个原型。
...
领班接到客户端请求，先写到自己的"任务清单"（log），然后并行发给所有副手。**过半副手确认收到**，这条任务才算 commit（可执行）。
...
5. D E 投给 C，C 当选
6. C 接管，把 A 没复制完的 log 继续推给 follower
```
...
- [[lampson-hints-1983]] —— Hints for Computer System Design — Butler Lampson 的系统设计箴言
