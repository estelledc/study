...
实际系统里 RLHF 不止"模型 + 标注员"两端：

- **数据采样器**：从 policy 当前策略中按 entropy / 不确定度采样 pair，避免人看到太相似的两段
- **标注 UI**：给标注员看两段并行视频或回答，要求 1-2 秒内做选择，配快捷键加速
- **质控**：插入"金标准"题，标注员做错就降权或剔除——RLHF 噪声大就来自这一层
- **去偏**：标注员个体偏好差异大，需要做 majority vote 或 ELO ranking 拉平

OpenAI 后来透露，InstructGPT 的标注流水线一共养了 ~40 人全职团队，光 UI 迭代就做了一年。"算法 9 年没变"是真的，但流水线复杂度每年都在涨。

这也是为什么后来 RLAIF（用 AI 标注代替人）成为研究热点——人力开销已经成了 RLHF 最大的瓶颈，必须找替代供给方式。
