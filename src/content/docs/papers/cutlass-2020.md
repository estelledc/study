...
- 为什么 xFormers 等库的部分 attention / GEMM 内核会直接拼 CUTLASS 模板，而不是只调 cuBLAS（官方 FlashAttention 则以手写 CUDA 为主，思路同源）
- 为什么 LLM 推理框架（TensorRT-LLM / vLLM）能融合 GEMM + GELU + bias + dequant 成一个 kernel——常靠 CUTLASS 一类的 epilogue 模板
- 为什么 NVIDIA 每出一代 GPU（Volta / Ampere / Hopper）的新指令（HMMA / IMMA / WGMMA），CUTLASS 都能在几周内吃下，而上层框架几乎不改代码
- 为什么"理解 GEMM 怎么写到 SOTA"绕不开 CUTLASS——它是把硬件细节**显式分层**写清楚的系统化开源 GEMM 模板参考
...
### 案例 2：fused attention 为什么离不开 epilogue

把 attention 分块流式算时，每块要做 `Q·Kᵀ`、softmax、`·V`。关键不是三次独立调库，而是：

1. 第一次 GEMM 的 epilogue **不写回 DRAM**，结果留在寄存器 / 共享内存
2. 原地做 softmax（仍不落盘）
3. 第二次 GEMM 直接读上一步驻留结果，整段融进**一个** kernel

cuBLAS 每发 kernel 各自独立，做不到这种跨 GEMM 共享。CUTLASS（以及同思路的手写 CUDA，如官方 FlashAttention）把 epilogue 暴露出来，fusion 才有抓手。
...
- 学习"GEMM 怎么写到 SOTA"——CUTLASS 是开源里最系统的分层模板参考之一
...
- **2022 年**：Tri Dao 发表 FlashAttention（手写 CUDA 融合 attention）；同期 xFormers 等用 CUTLASS 模板走同类 fusion，证明 epilogue 抽象能重塑 Transformer 内核。
...
- [[flash-attention]] —— FlashAttention 用手工 CUDA 做同类 fusion；对照 CUTLASS epilogue 路线
...
- [[flash-attention]] —— FlashAttention 把 attention 重写成 fused kernel（手写 CUDA；与 CUTLASS epilogue 思路对照）
