...
3. **生命周期**：随时任意一方可以发 `close` 帧（线上常见 1000 正常 / 1001 离开）；若 TCP 被掐、根本没收到 close 帧，端点会**本地合成** 1006（异常）。中间用 `ping/pong` 保活。
...
WebSocket 静默 60 秒后，LB 不知道这条 TCP 还有人用，发个 RST 关掉。浏览器**收不到**对方的 close 帧，于是本地合成 close code **1006**（异常断开；RFC 规定 1006 绝不会出现在线上帧里）。
...
4. **permessage-deflate 不是免费午餐**：像给每封短信都套一层真空袋——压缩扩展（RFC 7692）默认可省约 70% 流量，但每帧都要 CPU 解压；移动端弱网 + 小帧（< 100 字节）反而更慢。

5. **Origin 头要校验**：浏览器自动带 `Origin`，但 curl / 自写客户端可伪造。靠 Origin 防 CSWSH（跨站 WebSocket 劫持，像别人用你的登录态偷偷开电话）时，要清楚这只挡浏览器。
...
- 低延迟（< 100ms）或高频双向（约 ≥1 msg/s）——长轮询撑不住；单机常见量级约 1万–10万 长连接
...
- [[tls-1.3]] —— wss:// 就是 WebSocket over TLS（TLS 之上跑 WebSocket）
