...
- 为什么 **CockroachDB / TiDB / YugabyteDB / FoundationDB** 这一代 NewSQL 数据库都长得像 Spanner——它们都是 OSS 复刻品
...
1. 事务开始，coordinator 调 `TT.now()` 拿 `[t-7ms, t+7ms]`
2. 选 commit timestamp `s = t+7ms`（取区间右端，保守）
3. Paxos 复制写入到 5 个 region 的多数派
4. **commit-wait**：阻塞到 `TT.now().earliest > s`（约 14ms 后）
5. 释放锁、返回客户端 OK
...
**底层做的事**：查询规划器把这条 SQL 拆给跨区域的多个 tablet（分片），每个 tablet 是一个独立的 Paxos group。读取走 **snapshot read**（无锁，只联系本地副本）——比写事务快 5 倍。

### 案例 3：Google AdWords 的全球预算调整

广告主在控制台改预算：

- 旧方案（MySQL Cluster）：预算改完后，**几分钟内**别的 region 才看到，期间可能超预算花钱
- Spanner 方案：改完 100ms 内全球副本一致——超预算风险消失

这是 Spanner 最初的杀手级业务。AdWords 2012 年完成迁移，省下 DBA 每周手动改 schema 的运维时间。

## 踩过的坑

1. **TrueTime 需要专用硬件**：GPS 接收器 + 原子钟。普通公有云用户买不到——CockroachDB 改用纯软件的 HLC（Hybrid Logical Clock），代价是 ε 从 7ms 涨到 250ms（35 倍宽）。

2. **写延迟 > 跨大洲网络往返**：commit-wait 占 ~14ms + 跨 region Paxos quorum ~50-100ms。同 datacenter 部署 Spanner 反而**比 PostgreSQL 慢**——因为 commit-wait 这部分白白等了。
...
- 自建可接受 OSS 复刻 → CockroachDB / TiDB / YugabyteDB

**不适用**：

- 单 region 业务 → PostgreSQL + 异地灾备已足够，别过度工程化
- 极高写吞吐 + 简单 KV → FoundationDB / DynamoDB 更合适
- AP 优先 + 最终一致即可 → Cassandra / DynamoDB 更便宜
- OLAP 列存分析 → Snowflake / ClickHouse 完胜
- 没有 GPS+原子钟硬件的自建场景 → 选 OSS 复刻，别尝试复刻 TrueTime
...
- [[dora-state-of-devops-2023]] —— DORA State of DevOps Report 2023 — 用「餐厅经营」读懂软件交付科学
