...
Plenoxels 论文标题就是宣言：**Radiance Fields without Neural Networks**。它做的对照实验非常硬——同样 200k 步、同样 NeRF synthetic 八物体、同样体渲染公式，把 MLP 替换成"稀疏体素 + SH"，PSNR 反而高 0.4 dB。
...
5. **挑战常识的最佳方式是给个干净对照**：Plenoxels 没有写一篇综述说"我觉得 MLP 可有可无"，而是直接做实验——同样 loss、同样数据集、PSNR 提高 0.4 dB。这是研究最有杀伤力的证据形态
