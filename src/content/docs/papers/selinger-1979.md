...
3. **选择性估算（Selectivity Estimation）**：`WHERE age > 30` 会过滤掉多少行？引擎不知道实际数据，但它有**统计信息**——表的行数、列的最大最小值、直方图。Selinger 用这些数字估算："大概留下 40%"。估得越准，代价模型才越靠谱。
...
Selinger 算法**自动排除 C**，从 A 和 B 里挑更便宜的那条。
...
4. **直方图很粗糙**：选择性估算假设数据均匀分布，遇到**倾斜数据**（90% 的订单来自 10% 的顾客）就严重失准。现代优化器加了多列统计和采样来缓解。
