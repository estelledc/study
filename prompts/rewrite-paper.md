...
### Step 1-3：lr search + MinerU + paper-context

按 `new-paper.md` 的 Step 1-3 走一遍。即使你已读过现有笔记，也要重新拉论文上下文（现有可能基于过时的理解）。全文解析必须走 MinerU；引用上下文必须通过 `paper-context` helper；`lr` 只作为 helper 内部的 search / graph / cite 辅助能力，禁止使用 `lr pdf` 或 arxiv MCP 的 download/read_paper。
